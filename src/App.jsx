...
const SK = {
  ...
  schoolExams:   ["nv-school-exams",  "schoolExams"],
...
// Notifications: stored per-user in Firestore "shared" doc under key "notifs:<user>"
const _notifsKey = (user) => `notifs:${user.replace(/[@.]/g,"_")}`;
const _msgsKey   = (user) => `msgs:${user.replace(/[@.]/g,"_")}`;

// Save notifications to localStorage + Firestore (cross-device)
const saveMyData = (suffix, lsKey, val) => { lsSet(lsKey, val); };

const saveNotifications = async (user, notifs) => {
  lsSet("nv-notifications", notifs);
  dispatchSync();
  if (user) bsSet(_notifsKey(user), notifs).catch(() => {});
};

const saveMessages = async (user, msgs) => {
  lsSet("nv-messages", msgs);
  dispatchSync();
  if (user) bsSet(_msgsKey(user), msgs).catch(() => {});
};

// Load notifications from Firestore and merge with local
const loadNotifications = async (user) => {
  if (!user) return ls("nv-notifications", []);
  try {
    const remote = await bsGet(_notifsKey(user));
  ...
  } catch(e) { console.warn("[Notifs] load failed:", e.message); }
  return ls("nv-notifications", []);
};

const loadMessages = async (user) => {
  if (!user) return ls("nv-messages", []);
  try {
    const remote = await bsGet(_msgsKey(user));
    if (remote && Array.isArray(remote)) {
      lsSet("nv-messages", remote);
      dispatchSync();
      return remote;
    }
  } catch(e) { console.warn("[Msgs] load failed:", e.message); }
  return ls("nv-messages", []);
};

// Class messages (exam result announcements): stored in shared doc
const pushClassMessage = async (classId, msgRecord) => {
  const key = `classmsg_${classId}`;
  const existing = await bsGet(key) || [];
  await bsSet(key, [...existing, msgRecord]);
};

const getClassMessages = async (classId) => {
  const key = `classmsg_${classId}`;
  return await bsGet(key) || [];
};
...
const hydrateFromBackend = async () => {
  ...
  try {
    ...
    // Also hydrate handout notifications (shared across all users)
    const remoteNotifs = doc["handoutNotifications"];
    if (remoteNotifs && Array.isArray(remoteNotifs)) {
      const local = ls("nv-notifications", []);
      const byId = new Map();
      [...remoteNotifs, ...local].forEach(n => { if (!byId.has(n.id)) byId.set(n.id, n); });
      const merged = [...byId.values()].sort((a,b) => b.id - a.id);
      lsSet("nv-notifications", merged);
    }
    dispatchSync();
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const pushNotification = async (item) => {
    ...
    // Save to Firestore so every device sees the notification.
    // We store a shared "handout-notifications" list (all users pull it on sync).
    lsSet("nv-notifications", updated);
    dispatchSync();
    const allNotifs = await bsGet("handoutNotifications") || [];
    await bsSet("handoutNotifications", [notif, ...allNotifs.slice(0, 199)]);
  };
...
function ExamTakingView({ exam, currentUser, onSubmit }) {
  ...
  const handleViolation = useCallback((type) => {
    ...
    setViolations(v => {
      ...
      examBsGet(vKey).then(existing => {
        const list = existing || [];
        examBsSet(vKey, [...list, { type, time: new Date().toLocaleTimeString(), count: next }]);
      });
      if (next >= 3) {
  ...
  const doSubmit = useCallback((autoSubmit = false) => {
    ...
    examBsSet(rKey, result);
    // Update index
    examBsGet(`exam-results-index:${exam.id}`).then(idx => {
      const list = idx || [];
      examBsSet(`exam-results-index:${exam.id}`, [...list.filter(e=>e.student!==currentUser), { student: currentUser, submittedAt: result.submittedAt, pct, score, total: result.total, violations }]);
    });
    ...
    lsSet(attKey, att);
    lsSet(`nv-exam-att-${currentUser}`, att);
    onSubmit(result);
...
function LecturerExamManager({ toast, currentUser }) {
  ...
  // Sync from Firestore on mount so lecturer sees latest state
  useEffect(() => {
    bsGet("schoolExams").then(remote => {
      if (remote) {
        const parsed = typeof remote === "string" ? JSON.parse(remote) : remote;
        lsSet("nv-school-exams", parsed);
        setExams(parsed);
        dispatchSync();
      }
    });
  ...
  const loadResults = async (examId) => {
    ...
    try {
      const idx = await examBsGet(`exam-results-index:${examId}`) || [];
      const enriched = await Promise.all(idx.map(async r => {
        ...
        const [viols, retake] = await Promise.all([
          examBsGet(vKey).then(v => v || []),
          examBsGet(retakeKey).then(v => v || { granted: false }),
        ]);
  ...
  const grantRetake = async (examId, studentEmail) => {
    ...
    await examBsSet(key, { granted: true, grantedAt: new Date().toLocaleString(), grantedBy: currentUser });
  ...
  const revokeRetake = async (examId, studentEmail) => {
    const key = `exam-retake:${examId}:${studentEmail}`;
    await examBsSet(key, { granted: false });
  ...
  const publishResults = async (exam) => {
    ...
    await pushClassMessage(exam.classId, msgRecord);
...
function StudentExamView({ toast, currentUser }) {
  ...
  // Admin/Lecturer writes via saveShared() → _DOC_SHARED; read from same place.
  const refresh = async () => {
    const remote = await bsGet("schoolExams");
    ...
    const att = ls(`nv-exam-att-${currentUser}`, null); const _unused = null;
    ...
    if (completedIds.length > 0) {
      ...
      await Promise.all(completedIds.map(async examId => {
        ...
        const retake = await examBsGet(key);
  ...
  const startExam = async (exam) => {
    ...
    if (retakeGrants[exam.id]) {
      await examBsSet(`exam-retake:${exam.id}:${currentUser}`, { granted: false });
  ...
  useEffect(()=>{
    if (!userClass) return;
    getClassMessages(userClass).then(msgs => { if (msgs) setClassMessages(msgs); });
  },[userClass]);
...
function Messages({ user, toast }) {
  ...
  useEffect(()=>{ loadMessages(user).then(m=>{ if(m&&m.length) setMsgs(m); }); },[user]);
  const [input, setInput] = useState("");
  const [announcements] = useSharedData("nv-announcements", []);
  const send=()=>{if(!input.trim())return;const msg={id:Date.now(),from:user,text:input,time:"Just now",read:true,mine:true};const u=[...msgs,msg];setMsgs(u);saveMessages(user,u);setInput("");};
...
function Notifications({ currentUser, onRead }) {
  ...
  useEffect(() => {
    // Load from Firestore first (cross-device sync), then mark all as read
    loadNotifications(currentUser).then(fresh => {
  ...
  const del = (id) => {
    ...
    setNotifs(u);
    saveNotifications(currentUser, u);
  };
  const clearAll = () => {
    setNotifs([]);
...
export default function App() {
  ...
  useEffect(() => {
    initData(); runSync();
    // Auto-sync every 30 seconds while app is open (faster cross-device updates)
    const interval = setInterval(() => { hydrateFromBackend(); }, 30000);
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k!==CACHE).map(k=>caches.delete(k))))));
  ...
  const login = async () => {
    ...
    if (localUser) {
      ...
      // Refresh all shared data in background
      hydrateFromBackend();
      return;
    }
