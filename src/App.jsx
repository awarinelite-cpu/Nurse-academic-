...
const bsSet = async (key, val, shared = true) => {
  try { await window.storage.set(key, JSON.stringify(val), shared); } catch (e) { console.warn("bsSet failed:", key, e); }
};

// ─── REACTIVE SYNC ───────────────────────────────────────────────────
// After backend hydration, fire this event so all mounted components
// can re-read their freshly-hydrated localStorage data.
...
// Any component using this will automatically refresh after hydration.
function useSharedData(lsKey, fallback) {
  ...
  useEffect(() => {
    ...
    // Also read immediately in case hydration already finished
    handler();
...
// Fires sync event so all subscribed components refresh.
const dbSet = (lsKey, bsKey, val) => {
  lsSet(lsKey, val);
  bsSet(bsKey, val, true).catch(() => {});
  dispatchSync();
};
...
function AdminSchoolPQ({ toast }) {
  ...
  const saveData = (nd) => { setData(nd); saveShared("schoolPQ", nd); };
...
function AdminNursingExams({ toast }) {
  ...
  const saveData = (newData) => {
    setData(newData);
    saveShared("nursingExams", newData);
  };
...
function StudentExamView({ toast, currentUser }) {
  ...
  const refresh = async () => {
    ...
    if (remote) { const parsed = typeof remote === "string" ? JSON.parse(remote) : remote; lsSet("nv-school-exams", parsed); setAllExams(parsed); }
        ...
        <div className="main-area">
          <div className="topbar">
            ...
            <div className="topbar-right">
              ...
              <div className="icon-btn" title="Sync data from server" onClick={()=>{ if(!syncing) runSync().then(()=>toast("✅ Data synced!","success")); }} style={{opacity:syncing?.5:1,cursor:syncing?"wait":"pointer"}}>
