...
const hydrateFromBackend = async () => {
  const defaults = {
    ...
    handouts: [], essayBanks: [], nursingExams: {general:[],midwifery:[],publichealth:[]}, schoolPQ: {},
  };
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const coursesInClass = [...new Set(classHandouts.map(h=>h.course).filter(Boolean))];
  const courseHandouts = classHandouts.filter(h=>h.course===drillCourse);
  const lecturersInCourse = [...new Set(courseHandouts.map(h=>h.lecturerName||h.uploadedBy?.split("@")[0]||"Unknown").filter(Boolean))];
      ...
      {!filter&&(
        <>
          ...
          {!drillClass&&(
            handouts.length===0 ? (
              <div style={{textAlign:"center",padding:"60px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:14}}>
                ...
                <div style={{fontWeight:700}}>No handouts yet</div>
                <div style={{fontSize:12,marginTop:6}}>{isLecturer?"Click \"+ Upload Handout\" to add the first one":"Your lecturers haven't uploaded any materials yet"}</div>
              </div>
            ) : (
              <div className="grid2">
                {classes.filter(c=>handouts.some(h=>h.classId===c.id)).map(c=>{
                  const cnt = handouts.filter(h=>h.classId===c.id).length;
                  const courseCount = new Set(handouts.filter(h=>h.classId===c.id).map(h=>h.course).filter(Boolean)).size;
                  return (
                    <FolderCard key={c.id} icon="📁" label={c.label} sublabel={`${c.desc} · ${courseCount} course${courseCount!==1?"s":""}`}
          ...
          {drillClass&&!drillCourse&&(
            <>
              <div style={{fontWeight:800,fontSize:15,marginBottom:12,color:"var(--accent)"}}>
              ...
              {coursesInClass.length===0 ? (
                ...
                <div className="grid2">
                  {coursesInClass.map(course=>{
                    ...
                    const lecCount = new Set(classHandouts.filter(h=>h.course===course).map(h=>h.lecturerName||h.uploadedBy?.split("@")[0]||"Unknown")).size;
                    return (
          ...
          {drillClass&&drillCourse&&!drillLecturer&&(
            <>
              <div style={{fontWeight:800,fontSize:15,marginBottom:12,color:"var(--accent2)"}}>
          ...
          {drillClass&&drillCourse&&drillLecturer&&(
            <>
              <div style={{fontWeight:800,fontSize:15,marginBottom:12,color:"var(--purple)"}}>
