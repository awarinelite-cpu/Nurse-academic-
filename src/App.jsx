...
const _cache = {};
const _cacheTime = {};
const CACHE_TTL = 4000; // 4s — must expire before the 5-10s sync tick
...
// schoolExams is written by saveShared() → bsSet() → _DOC_SHARED
// All other per-exam keys (results, violations, retakes, class-messages) go to _DOC_EXAMS
...
const hydrateFromBackend = async () => {
  ...
  try {
    ...
    delete _cache[_DOC_SHARED]; // bust cache so next examBsGet("schoolExams") re-fetches
    dispatchSync();
...
function LecturerExamManager({ toast, currentUser }) {
  ...
  const saveExams = async (updated) => {
    setExams(updated);
    delete _cache[_DOC_SHARED]; // ensure next read is fresh for all devices
...
function StudentExamView({ toast, currentUser }) {
  ...
  useEffect(() => {
    refresh();
    // Auto-refresh student exam list every 5-10 s so newly opened exams appear instantly
    let examTimer;
    const scheduleExamRefresh = () => {
      const delay = 5000 + Math.floor(Math.random() * 5000);
      examTimer = setTimeout(async () => { await refresh(); scheduleExamRefresh(); }, delay);
    };
    scheduleExamRefresh();
    const onFocus = () => { clearTimeout(examTimer); refresh().then(scheduleExamRefresh); };
    window.addEventListener("focus", onFocus);
    return () => { clearTimeout(examTimer); window.removeEventListener("focus", onFocus); };
  }, []);
...
export default function App() {
  ...
  useEffect(() => {
    initData(); runSync();
    // Auto-sync every 5-10 s (random spread to avoid all devices hitting Firestore at once)
    let syncTimer;
    const scheduleSync = () => {
      const delay = 5000 + Math.floor(Math.random() * 5000);
    ...
    scheduleSync();
    // Also sync immediately when tab regains focus
