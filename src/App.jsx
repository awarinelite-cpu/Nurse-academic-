...
// ─── IN-MEMORY CACHE (L1 only — all persistence goes to window.storage) ─
// _mem is the sole in-process cache. Reads hit _mem first (zero-cost) and
// fall back to the supplied default. localStorage is NOT used anywhere.
const _mem = {};
const ls = (k, d) => (_mem[k] !== undefined ? _mem[k] : d);
const lsSet = (k, v) => { _mem[k] = v; };
...
// Write: update in-memory cache immediately, persist to backend async.
...
// Pull from backend, populate _mem cache, return value (or in-memory/default fallback)
...
// Shared (admin-managed) storage keys: [_mem cache key, backend key]
const SK = {
  ...
  notifications: ["nv-notifications",  "db:notifications"],
};
// saveShared: write to _mem cache immediately, persist to backend async, notify React subscribers
...
const useHydratedShared = (lsKey, skKey, fallback) => {
  ...
  useEffect(() => {
    ...
    if (skKey) {
      if (!_sharedSubs[skKey]) _sharedSubs[skKey] = new Set();
...
const uKey = (user, suffix) => `nv-user:${user}:${suffix}`;
...
const saveManualGradeToBackend = async (studentEmail, bankId, gradeData) => {
  ...
  // Mirror grade to student private backend storage so they see the result
  const attKey = `nv-essay-att-${studentEmail}`;
  const existingAtt = await bsGet(uKey(studentEmail, "essay-att"), false) || ls(attKey, {});
  const updatedAtt = { ...existingAtt, [String(bankId)]: { ...existingAtt[String(bankId)], manualGrade: gradeData, gradedDate: new Date().toLocaleDateString() } };
  lsSet(attKey, updatedAtt);
  await bsSet(uKey(studentEmail, "essay-att"), updatedAtt, false);
  return updated;
};
// ─── DEFAULT DATA ───────────────────────────────────────────────────
...
// ─── INIT STORAGE ───────────────────────────────────────────────────
// Seeds the backend with defaults on first run (if a key is missing).
// Called inside hydrateFromBackend so window.storage is available.
const ADMIN_DEFAULT = [{username:"admin@gmail.com",password:"admin123",role:"admin",class:"",joined:"System"}];
const initDefaults = async () => {
  const seeds = {
    "db:classes": DEFAULT_CLASSES, "db:drugs": DEFAULT_DRUGS, "db:labs": DEFAULT_LABS,
    "db:pq": DEFAULT_PQ, "db:decks": DEFAULT_DECKS, "db:dict": DEFAULT_DICT,
    "db:skills": DEFAULT_SKILLS, "db:announcements": DEFAULT_ANNOUNCEMENTS,
    "db:users": ADMIN_DEFAULT,
    "db:handouts": [], "db:essay-banks": [], "db:class-exams": [],
    "db:messages": [], "db:nursing-exams": [], "db:notifications": [],
  };
  await Promise.allSettled(Object.entries(seeds).map(async ([bk, def]) => {
    try {
      const existing = await bsGet(bk, true);
      if (existing === null) await bsSet(bk, def, true);
    } catch {}
  }));
};

// Hydrate all shared data from backend into _mem, notify React subscribers.
// Seeds backend with defaults on very first run (if keys are missing).
const SHARED_DEFAULTS = {
  ...
  handouts: [], essayBanks: [], classExams: [], messages: [], nursingExams: [], notifications: [],
};
const hydrateFromBackend = async () => {
  // Seed backend with defaults first (no-op if keys already exist)
  await initDefaults();
  // Load all shared keys into _mem, notify subscribers as each resolves
  await Promise.allSettled(
    Object.keys(SK).map(async key => {
      try { await loadShared(key, SHARED_DEFAULTS[key] || []); notifyKey(key); } catch {}
    })
  );
...
function AdminPanel({ toast, currentUser }) {
  ...
  const TABS = [
    ...
    { key:"nursing", label:"🏥 Nursing Exams" },
  ...
  return (
    <div>
      ...
      {tab==="nursing" && <AdminNursingExams toast={toast} />}
    ...
    <div>
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <button className="btn btn-danger" onClick={()=>{if(confirm("Reset ALL data to defaults? This cannot be undone!")){["nv-classes","nv-drugs","nv-labs","nv-pq","nv-decks","nv-dict","nv-skillsdb","nv-announcements","nv-handouts"].forEach(k=>{ delete _mem[k]; });initDefaults().then(()=>hydrateFromBackend());toast("Data reset to defaults","warn");}}}>🔄 Reset to Defaults</button>
        </div>
      </div>
      <div className="card">
...
function AdminDrugs({ toast }) {
  const [drugs, setDrugs] = useHydratedShared("nv-drugs", "drugs", DEFAULT_DRUGS);
  const [showModal, setShowModal] = useState(false);
  const [edit, setEdit] = useState(null);
  const [pasteMode, setPasteMode] = useState(false);
  const [pasteText, setPasteText] = useState("");
  ...
  const importParsed = () => {
    ...
    const u=[...drugs,...items]; setDrugs(u);
  ...
  const save = () => {
    ...
    setDrugs(u); setShowModal(false); setEdit(null); setForm(blank);
  };

  const del = (id) => { const u=drugs.filter(d=>d.id!==id); setDrugs(u); toast("Deleted","success"); };
...
function AdminLabs({ toast }) {
  const [labs, setLabs] = useHydratedShared("nv-labs", "labs", DEFAULT_LABS);
  const [showModal, setShowModal] = useState(false);
  const [edit, setEdit] = useState(null);
  const [pasteMode, setPasteMode] = useState(false);
  const [pasteText, setPasteText] = useState("");
  ...
  const save = () => {
    ...
    setLabs(u); setShowModal(false); setEdit(null); setForm(blank);
  };

  const del = (id) => { const u=labs.filter(l=>l.id!==id); setLabs(u); toast("Deleted","success"); };
...
function AdminFlashcards({ toast }) {
  const [decks, setDecks] = useHydratedShared("nv-decks", "decks", DEFAULT_DECKS);
  ...
  const importParsed = () => {
    ...
    setDecks(u); toast(`${items.length} cards imported!`,"success"); setPasteText(""); setParsed([]); setPasteMode(false);
  };
  ...
  const saveDeck = () => {
    ...
    setDecks(u); setShowDeckModal(false); setEditDeck(null); setDeckForm({name:""});
  };

  const delDeck = (id) => { if(!confirm("Delete deck?"))return; const u=decks.filter(d=>d.id!==id); setDecks(u); if(selDeck===id)setSelDeck(null); toast("Deleted","success"); };
  ...
  const saveCard = () => {
    ...
    setDecks(u); setShowCardModal(false); setEditCard(null); setCardForm({front:"",back:""});
  };

  const delCard = (deckId, cardIdx) => { const u=decks.map(d=>d.id===deckId?{...d,cards:d.cards.filter((_,i)=>i!==cardIdx)}:d); setDecks(u); toast("Deleted","success"); };
...
function AdminDictionary({ toast }) {
  const [dict, setDict] = useHydratedShared("nv-dict", "dict", DEFAULT_DICT);
  const [showModal, setShowModal] = useState(false);
  const [edit, setEdit] = useState(null);
  const [pasteMode, setPasteMode] = useState(false);
  const [pasteText, setPasteText] = useState("");
  ...
  const save = () => {
    ...
    setDict(u); setShowModal(false); setEdit(null); setForm({term:"",def:""});
  };

  const del = (id) => { const u=dict.filter(d=>d.id!==id); setDict(u); toast("Deleted","success"); };
...
function AdminSkills({ toast }) {
  const [skills, setSkills] = useHydratedShared("nv-skillsdb", "skills", DEFAULT_SKILLS);
  const [showModal, setShowModal] = useState(false);
  const [edit, setEdit] = useState(null);
  const [pasteMode, setPasteMode] = useState(false);
  const [pasteText, setPasteText] = useState("");
  ...
  const importParsed = () => {
    const items = parsed.map(p=>({...p,id:Date.now()+Math.random()}));
    const u=[...skills,...items]; setSkills(u);
  ...
  const save = () => {
    ...
    setSkills(u); setShowModal(false); setEdit(null); setForm({name:""});
  };

  const del = (id) => { const u=skills.filter(s=>s.id!==id); setSkills(u); toast("Deleted","success"); };
...
function AdminExamRetakes({ toast }) {
  ...
  const [userAttemptMap, setUserAttemptMap] = useState({});
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);

  const students = users.filter(u => u.role !== "admin");

  // Load all student attempts from backend on mount and when users change
  useEffect(() => {
    ...
    Promise.allSettled(
      students.map(async u => {
        try {
          const key = `nv-exam-attempts-${u.username}`;
          const remote = await bsGet(uKey(u.username, "mcq-att"), false);
          const data = remote || ls(key, {});
          lsSet(key, data);
          return [u.username, data];
        } catch {
          return [u.username, ls(`nv-exam-attempts-${u.username}`, {})];
        }
      })
    ).then(results => {
      const map = {};
      results.forEach(r => { if (r.status === "fulfilled" && r.value) map[r.value[0]] = r.value[1]; });
      setUserAttemptMap(map);
      setLoading(false);
    });
  }, [users.length]);

  const resetUserExam = async (username, bankId) => {
    const key = `nv-exam-attempts-${username}`;
    const userAttempts = userAttemptMap[username] || ls(key, {});
    const updated = { ...userAttempts };
    delete updated[bankId];
    saveMyData("mcq-att", key, updated);
    setUserAttemptMap(prev => ({ ...prev, [username]: updated }));
    toast(`Retake reset for ${username} on "${banks.find(b => b.id === bankId)?.subject || bankId}"`, "success");
  };

  const resetAllExams = async (username) => {
    if (!confirm(`Reset ALL exam attempts for ${username}?`)) return;
    saveMyData("mcq-att", `nv-exam-attempts-${username}`, {});
    setUserAttemptMap(prev => ({ ...prev, [username]: {} }));
  ...
  return (
    <div>
      ...
      {loading && <div style={{textAlign:"center",padding:"20px",fontFamily:"'DM Mono',monospace",fontSize:12,color:"var(--text3)"}}>⏳ Loading student attempts...</div>}
      ...
      {filtered.map(u => {
        const userAttempts = userAttemptMap[u.username] || ls(`nv-exam-attempts-${u.username}`, {});
                    ...
                    <div key={bankId} className="card2" style={{marginBottom:6,display:"flex",alignItems:"center",gap:10}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace"}}>
                          Date: {data.date || "—"} · Score: {data.pct != null ? `${data.pct}%` : "—"}
                        </div>
                      </div>
      ...
      {filtered.length===0&&!loading&&<div style={{textAlign:"center",padding:"40px",color:"var(--text3)",fontFamily:"'DM Mono',monospace",fontSize:13}}>No students found.</div>}
    </div>
  );
}
...
function AdminEssayExams({ toast }) {
  ...
  const resetStudentEssay = async (username, bankId) => {
    const key = `nv-essay-att-${username}`;
    const att = await bsGet(uKey(username, "essay-att"), false) || ls(key, {});
    const updated = { ...att };
    delete updated[bankId];
    lsSet(key, updated);
    await bsSet(uKey(username, "essay-att"), updated, false);
...
const NURSING_CATEGORIES = [
  { id:"public_health", label:"Public Health Nursing", icon:"🌍", color:"#3E8E95" },
  { id:"midwifery",     label:"Midwifery",             icon:"👶", color:"#a78bfa" },
  { id:"general",       label:"General Nursing",       icon:"🏥", color:"#4ade80" },
];

function AdminNursingExams({ toast }) {
  const [banks, setBanks] = useHydratedShared("nv-nursing-exams", "nursingExams", []);
  const [selCat, setSelCat] = useState(NURSING_CATEGORIES[0].id);
  const [selBank, setSelBank] = useState(null);
  const [inputMode, setInputMode] = useState("single"); // single | paste | answers
  const [pasteText, setPasteText] = useState("");
  const [answerKeyText, setAnswerKeyText] = useState("");
  const [parsed, setParsed] = useState([]);
  const [parseError, setParseError] = useState("");
  const [bankForm, setBankForm] = useState({ title:"", year:"", isPublished:false });
  const [showBankModal, setShowBankModal] = useState(false);
  ...
  useEffect(() => {
    loadShared("nursingExams", []).then(() => notifyKey("nursingExams")).catch(() => {});
  }, []);

  const catBanks = banks.filter(b => b.category === selCat);
  const currentBank = banks.find(b => b.id === selBank);

  const saveBank = () => {
    if (!bankForm.title) return toast("Title required","error");
    let updated;
    if (editBankId) {
      updated = banks.map(b => b.id === editBankId ? { ...b, title:bankForm.title, year:bankForm.year } : b);
      toast("Bank updated","success");
    } else {
      // Parse bulk-pasted questions if provided
      let questions = [];
      if (bankForm.bulkText && bankForm.bulkText.trim()) {
        const result = parseMCQText(bankForm.bulkText);
        if (result.type === "questions" && result.questions.length) {
          questions = result.questions;
          // Apply answer key if provided
          if (bankForm.answerKeyText && bankForm.answerKeyText.trim()) {
            const akText = bankForm.answerKeyText.trim();
            const inline = [...akText.matchAll(/(\d+)[.)]\s*([A-Da-d])/g)];
            let key = [];
            if (inline.length) {
              key = inline.map(m=>({num:+m[1], ans:"ABCD".indexOf(m[2].toUpperCase())}));
            } else {
              const lines = akText.split("\n").map(l=>l.trim()).filter(l=>/^[A-Da-d]$/.test(l));
              if (lines.length) key = lines.map((l,i)=>({num:i+1, ans:"ABCD".indexOf(l.toUpperCase())}));
            }
            if (key.length) questions = applyAnswerKey(questions, key);
          }
        }
      }
      const nb = { id: Date.now(), category: selCat, title:bankForm.title, year:bankForm.year||"", questions, isPublished: false };
      updated = [...banks, nb];
      toast(questions.length > 0 ? `Bank created with ${questions.length} questions ✅` : "Bank created","success");
    }
    setBanks(updated); saveShared("nursingExams", updated);
    setShowBankModal(false); setEditBankId(null); setBankForm({ title:"", year:"", isPublished:false });
  };
  ...
  const togglePublish = (id) => {
    ...
    const newPublished = !bank.isPublished;
    const updated = banks.map(b => b.id === id ? { ...b, isPublished: newPublished } : b);
    setBanks(updated); saveShared("nursingExams", updated);
    toast(newPublished ? "Published — students can now see this exam ✅" : "Unpublished", newPublished ? "success" : "warn");
  };
  ...
  const saveQ = () => {
    ...
    const updated = banks.map(b => {
      if (b.id !== selBank) return b;
      let qs;
      if (editQIdx !== null) { qs = b.questions.map((q,i) => i === editQIdx ? { ...qForm } : q); toast("Updated","success"); }
      else { qs = [...b.questions, { ...qForm }]; toast("Question added","success"); }
      return { ...b, questions: qs };
    });
    setBanks(updated); saveShared("nursingExams", updated);
    setShowQModal(false); setEditQIdx(null); setQForm({ q:"", options:["","","",""], ans:0 });
  };

  const delQ = (qIdx) => {
    if (!confirm("Delete this question?")) return;
    const updated = banks.map(b => b.id === selBank ? { ...b, questions: b.questions.filter((_,i) => i !== qIdx) } : b);
    setBanks(updated); saveShared("nursingExams", updated);
  ...
  const doParse = () => {
    setParseError(""); setParsed([]);
    const result = parseMCQText(pasteText);
    if (!result.questions.length) { setParseError("Could not parse. See format guide below."); return; }
    setParsed(result.questions);
  };
  ...
  const doApplyAnswerKey = () => {
    ...
    if (inline.length) {
      const key = inline.map(m=>({num:+m[1], ans:"ABCD".indexOf(m[2].toUpperCase())}));
      const updated = banks.map(b => b.id === selBank ? { ...b, questions: applyAnswerKey(b.questions, key) } : b);
      setBanks(updated); saveShared("nursingExams", updated);
      toast(`Applied ${key.length} answers!`,"success"); setAnswerKeyText(""); return;
    }
    ...
    if (lines.length) {
      const key = lines.map((l,i)=>({num:i+1, ans:"ABCD".indexOf(l.toUpperCase())}));
      const updated = banks.map(b => b.id === selBank ? { ...b, questions: applyAnswerKey(b.questions, key) } : b);
      setBanks(updated); saveShared("nursingExams", updated);
      toast(`Applied ${key.length} answers!`,"success"); setAnswerKeyText(""); return;
    }
  ...
  const cat = NURSING_CATEGORIES.find(c => c.id === selCat);

  return (
    <div>
      <div className="sec-title">🏥 Nursing Exam Banks</div>
      <div className="sec-sub">Manage Public Health, Midwifery, and General Nursing question banks · publish for students</div>

      {/* Category tabs */}
      <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
        {NURSING_CATEGORIES.map(c => (
          <div key={c.id} onClick={() => { setSelCat(c.id); setSelBank(null); }} style={{
            flex:1,minWidth:140,padding:"12px 16px",borderRadius:11,cursor:"pointer",textAlign:"center",
            border:`2px solid ${selCat===c.id ? c.color : "var(--border)"}`,
            background:selCat===c.id ? `${c.color}18` : "transparent",transition:"all .2s"
          }}>
            <div style={{fontSize:26,marginBottom:4}}>{c.icon}</div>
            <div style={{fontWeight:700,fontSize:13,color:selCat===c.id ? c.color : "var(--text2)"}}>{c.label}</div>
            <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginTop:2}}>
              {banks.filter(b=>b.category===c.id).length} bank(s)
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Bank list + add */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{fontWeight:700,fontSize:15,color:cat.color}}>{cat.icon} {cat.label} Banks</div>
        <button className="btn btn-accent" onClick={()=>{setShowBankModal(true);setEditBankId(null);setBankForm({title:"",year:"",isPublished:false});}}>+ New Bank</button>
      </div>

      <div className="grid2" style={{marginBottom:20}}>
        {catBanks.length === 0 && <div style={{gridColumn:"1/-1",textAlign:"center",padding:"30px",color:"var(--text3)",fontFamily:"'DM Mono',monospace",fontSize:13}}>No banks yet. Create one above.</div>}
        {catBanks.map(b => (
          <div key={b.id} className="card" style={{cursor:"pointer",border:`1px solid ${selBank===b.id ? cat.color : "var(--border)"}`,transition:"border .2s"}}
            onClick={() => { setSelBank(b.id); setInputMode("single"); setParsed([]); setPasteText(""); setAnswerKeyText(""); }}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:15}}>{b.title}</div>
                <div style={{fontSize:12,color:"var(--text3)",marginTop:3}}>{b.year && `${b.year} · `}{b.questions.length} questions</div>
                <span style={{fontSize:10,fontFamily:"'DM Mono',monospace",padding:"2px 8px",borderRadius:4,marginTop:6,display:"inline-block",
                  background:b.isPublished?"rgba(74,222,128,.15)":"rgba(251,146,60,.1)",
                  color:b.isPublished?"var(--success)":"var(--warn)",
                  border:`1px solid ${b.isPublished?"rgba(74,222,128,.3)":"rgba(251,146,60,.25)"}`}}>
                  {b.isPublished ? "✅ Published" : "⏸ Draft"}
                </span>
              </div>
              <div style={{display:"flex",gap:5,flexShrink:0}}>
                <button className="btn btn-sm" onClick={e=>{e.stopPropagation();setEditBankId(b.id);setBankForm({title:b.title,year:b.year||"",isPublished:b.isPublished});setShowBankModal(true);}}>✏️</button>
                <button className={`btn btn-sm ${b.isPublished?"btn-warn":"btn-accent"}`} onClick={e=>{e.stopPropagation();togglePublish(b.id);}}>
      ...
      {/* Question editor */}
      {selBank && currentBank && (
        <div className="card" style={{border:`1px solid ${cat.color}40`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
            <div>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16}}>{currentBank.title}</div>
              <div style={{fontSize:12,color:"var(--text3)"}}>{currentBank.questions.length} questions</div>
            </div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {["single","paste","answers"].map(m=>(
                <button key={m} onClick={()=>{setInputMode(m);setParsed([]);setParseError("");}} style={{
                  padding:"5px 12px",borderRadius:7,border:`1px solid ${inputMode===m?cat.color:"var(--border)"}`,
                  background:inputMode===m?`${cat.color}18`:"transparent",color:inputMode===m?cat.color:"var(--text3)",
                  fontSize:12,cursor:"pointer",fontFamily:"'DM Mono',monospace"
                }}>
                  {m==="single"?"Single Q":m==="paste"?"Bulk Paste":"Apply Answers"}
                </button>
              ))}
            </div>
          </div>
          ...
          {inputMode==="single" && (
            <div style={{marginBottom:20}}>
              <button className="btn btn-accent" onClick={()=>{setShowQModal(true);setEditQIdx(null);setQForm({q:"",options:["","","",""],ans:0});}}>+ Add Question</button>
            </div>
          )}
          ...
          {inputMode==="paste" && (
            <div style={{marginBottom:20}}>
              <div style={{background:"rgba(62,142,149,.07)",border:"1px solid rgba(62,142,149,.2)",borderRadius:9,padding:"10px 14px",fontSize:12,color:"var(--text2)",marginBottom:12,lineHeight:1.7}}>
                <b>Format:</b> Number the questions, list options A–D on new lines.<br/>
                <span style={{fontFamily:"'DM Mono',monospace",fontSize:11}}>1. What is...?{"\n"}A. Option one{"\n"}B. Option two{"\n"}C. Option three{"\n"}D. Option four{"\n\n"}2. Next question...</span>
              </div>
              <textarea className="inp" rows={10} style={{fontFamily:"'DM Mono',monospace",fontSize:12,resize:"vertical"}}
                placeholder={"1. What is the primary role of a public health nurse?\nA. Administer IV medications\nB. Promote community health\nC. Perform surgery\nD. Read ECG\n\n2. Next question..."}
                value={pasteText} onChange={e=>setPasteText(e.target.value)} />
              {parseError && <div style={{color:"var(--danger)",fontSize:12,marginTop:6}}>{parseError}</div>}
              {parsed.length > 0 && <div style={{color:"var(--success)",fontSize:12,margin:"6px 0"}}>✅ {parsed.length} questions parsed — ready to import</div>}
              <div style={{display:"flex",gap:8,marginTop:10}}>
                <button className="btn btn-accent" onClick={doParse}>Parse Preview</button>
                {parsed.length > 0 && <button className="btn btn-purple" onClick={doImport}>Import {parsed.length} Questions</button>}
              </div>
            </div>
          )}

          {/* Apply answer key mode */}
          {inputMode==="answers" && (
            <div style={{marginBottom:20}}>
              <div style={{background:"rgba(167,139,250,.07)",border:"1px solid rgba(167,139,250,.2)",borderRadius:9,padding:"10px 14px",fontSize:12,color:"var(--text2)",marginBottom:12,lineHeight:1.7}}>
                <b>Format:</b> <span style={{fontFamily:"'DM Mono',monospace"}}>1.B 2.C 3.A 4.D</span> or one letter per line: <span style={{fontFamily:"'DM Mono',monospace"}}>B{"\n"}C{"\n"}A</span>
              </div>
              <textarea className="inp" rows={6} style={{fontFamily:"'DM Mono',monospace",fontSize:13,resize:"vertical",letterSpacing:"0.1em"}}
                placeholder="1.B 2.C 3.A 4.D 5.B..." value={answerKeyText} onChange={e=>setAnswerKeyText(e.target.value)} />
              {parseError && <div style={{color:"var(--danger)",fontSize:12,marginTop:6}}>{parseError}</div>}
              <button className="btn btn-purple" style={{marginTop:10}} onClick={doApplyAnswerKey}>Apply Answer Key</button>
            </div>
          )}

          {/* Questions list */}
          {currentBank.questions.length === 0 ? (
            <div style={{textAlign:"center",padding:"30px",color:"var(--text3)",fontFamily:"'DM Mono',monospace",fontSize:13}}>No questions yet.</div>
          ) : (
            <div>
              <div style={{fontSize:13,fontWeight:700,marginBottom:10,color:"var(--text2)"}}>Questions ({currentBank.questions.length})</div>
              {currentBank.questions.map((q, i) => (
                <div key={i} className="card2" style={{marginBottom:8,borderLeft:`3px solid ${cat.color}`}}>
                  <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:cat.color,fontWeight:700,minWidth:24,marginTop:2}}>Q{i+1}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:600,marginBottom:6}}>{q.q}</div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                        {(q.options||[]).map((opt,oi) => opt && (
                          <span key={oi} style={{fontSize:11,padding:"2px 8px",borderRadius:5,fontFamily:"'DM Mono',monospace",
                            background:oi===q.ans?"rgba(74,222,128,.15)":"rgba(255,255,255,.05)",
                            color:oi===q.ans?"var(--success)":"var(--text3)",
                            border:`1px solid ${oi===q.ans?"rgba(74,222,128,.3)":"transparent"}`}}>
                    ...
                    <div style={{display:"flex",gap:4,flexShrink:0}}>
                      <button className="btn btn-sm" onClick={()=>{setEditQIdx(i);setQForm({...q});setShowQModal(true);}}>✏️</button>
      ...
      {showBankModal && (
        <div className="modal-overlay" onClick={()=>setShowBankModal(false)}>
          <div className="modal" style={{maxWidth:560,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <div className="modal-title">{editBankId?"Edit":"New"} Exam Bank — {cat.label}</div>
              <button className="modal-close" onClick={()=>setShowBankModal(false)}>✕</button>
            </div>
            <label className="lbl">Title</label>
            <input className="inp" placeholder="e.g. 2023 Pre-exam Questions" value={bankForm.title} onChange={e=>setBankForm({...bankForm,title:e.target.value})} />
            <label className="lbl">Year / Label (optional)</label>
            <input className="inp" placeholder="e.g. 2023" value={bankForm.year} onChange={e=>setBankForm({...bankForm,year:e.target.value})} />

            {/* Bulk paste questions directly in the create modal */}
            {!editBankId && (<>
              <div style={{margin:"14px 0 6px",borderTop:"1px solid var(--border)",paddingTop:12}}>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:13,marginBottom:4}}>📋 Paste Questions (optional)</div>
                <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginBottom:8}}>
                  Paste numbered MCQ text below — questions will be auto-parsed and added to the bank when you save.
                </div>
                <textarea className="paste-box" rows={10} style={{width:"100%",resize:"vertical"}}
                  placeholder={"1. Which organ produces insulin?\nA) Liver\nB) Kidney\nC) Pancreas\nD) Spleen\nANS: C\n\n2. Normal adult temperature is:\nA) 35.0\u00b0C\nB) 36.1-37.2\u00b0C\nC) 38.5\u00b0C\nD) 40.0\u00b0C\nAnswer: B"}
                  value={bankForm.bulkText||""} onChange={e=>setBankForm({...bankForm,bulkText:e.target.value})} />
                {bankForm.bulkText && (() => {
                  const r = parseMCQText(bankForm.bulkText);
                  const n = r.type==="questions" ? r.questions.length : 0;
                  return <div style={{fontSize:11,fontFamily:"'DM Mono',monospace",color:n>0?"var(--success)":"var(--warn)",marginTop:4}}>
                    {n>0 ? `✅ ${n} question${n!==1?"s":""} detected` : "⚠️ No questions detected — check format"}
                  </div>;
                })()}
                <div style={{margin:"10px 0 4px",borderTop:"1px solid var(--border)",paddingTop:10}}>
                  <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:13,marginBottom:4}}>🔑 Answer Key (optional)</div>
                  <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginBottom:6}}>
                    Paste answers: <code style={{background:"rgba(255,255,255,.07)",padding:"1px 5px",borderRadius:4}}>1.B 2.C 3.A</code> or one letter per line
                  </div>
                  <textarea className="paste-box" rows={3} style={{width:"100%",resize:"vertical"}}
                    placeholder={"1.B 2.C 3.A 4.D\n\n— or —\n\nB\nC\nA\nD"}
                    value={bankForm.answerKeyText||""} onChange={e=>setBankForm({...bankForm,answerKeyText:e.target.value})} />
                </div>
              </div>
            </>)}

            <div style={{display:"flex",gap:8,marginTop:8}}>
              <button className="btn btn-accent" style={{flex:1}} onClick={saveBank}>
                {bankForm.bulkText ? `💾 Save & Import Questions` : "Save"}
              </button>
      ...
      {showQModal && (
        <div className="modal-overlay" onClick={()=>setShowQModal(false)}>
          <div className="modal" style={{maxWidth:540}} onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <div className="modal-title">{editQIdx!==null?"Edit":"Add"} Question</div>
              <button className="modal-close" onClick={()=>setShowQModal(false)}>✕</button>
            </div>
            <label className="lbl">Question Text</label>
            <textarea className="inp" rows={3} style={{resize:"vertical"}} value={qForm.q} onChange={e=>setQForm({...qForm,q:e.target.value})} placeholder="Enter question..." />
            {["A","B","C","D"].map((ltr,i) => (
              <div key={i}>
                <label className="lbl">Option {ltr} {i===qForm.ans?"✓ (Correct)":""}</label>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <input className="inp" style={{flex:1,marginBottom:0,border:`1px solid ${i===qForm.ans?"var(--success)":"var(--border)"}`}}
                    value={qForm.options[i]} onChange={e=>{const o=[...qForm.options];o[i]=e.target.value;setQForm({...qForm,options:o});}} placeholder={`Option ${ltr}`} />
                  <button onClick={()=>setQForm({...qForm,ans:i})} style={{
                    padding:"8px 12px",borderRadius:8,border:`1px solid ${i===qForm.ans?"var(--success)":"var(--border)"}`,
                    background:i===qForm.ans?"rgba(74,222,128,.15)":"transparent",color:i===qForm.ans?"var(--success)":"var(--text3)",
            ...
            <div style={{display:"flex",gap:8,marginTop:16}}>
              <button className="btn btn-accent" style={{flex:1}} onClick={saveQ}>Save Question</button>
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const pushNotification = async (item) => {
    const notifs = await loadShared("notifications", []).catch(() => ls("nv-notifications", []));
    const newNotif = {
    ...
    saveShared("notifications", updatedNotifs);
  };
...
function MCQExamView({ toast, currentUser, banks }) {
  ...
  const submitExam = () => {
    ...
    bsGet(uKey(currentUser,"results"),false).then(prev=>{
      const existing=Array.isArray(prev)?prev:ls("nv-results",[]);
      saveMyData("results","nv-results",[...existing,{id:Date.now(),subject:sel.subject,type:"MCQ Exam",score,total:sel.questions.length,pct,date:new Date().toLocaleDateString()}]);
    }).catch(()=>{ saveMyData("results","nv-results",[...ls("nv-results",[]),{id:Date.now(),subject:sel.subject,type:"MCQ Exam",score,total:sel.questions.length,pct,date:new Date().toLocaleDateString()}]); });
...
{"overallScore":number,"totalMarks":${totalMarks},"overallPct":number,"grade":"A/B/C/D/F","overallComment":"2-3 sentence summary of performance","questions":[{"marksAwarded":number,"maxMarks":number,"strengths":"specific strengths","weaknesses":"specific gaps","feedback":"actionable feedback"}]}`
      ...
      bsGet(uKey(currentUser,"results"),false).then(prev=>{
        const existing=Array.isArray(prev)?prev:ls("nv-results",[]);
        saveMyData("results","nv-results",[...existing,{id:Date.now(),subject:sel.subject,type:"Essay (AI)",score:parsed.overallScore,total:totalMarks,pct:parsed.overallPct,date:new Date().toLocaleDateString()}]);
      }).catch(()=>{ saveMyData("results","nv-results",[...ls("nv-results",[]),{id:Date.now(),subject:sel.subject,type:"Essay (AI)",score:parsed.overallScore,total:totalMarks,pct:parsed.overallPct,date:new Date().toLocaleDateString()}]); });
      notifyUserKey("nv-results");
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",minHeight:260}}>
                ...
                <div style={{padding:"18px 20px",display:"flex",flexDirection:"column",background:"var(--bg2)"}}>
                  ...
                  <textarea
                    ...
                    placeholder={`Write your answer here (aim for ${guide} words)…\n\nTip: Be specific, use nursing terms, and cover all aspects of the question.`}                    value={answers[i]||""}
...
function LecturerHandouts({ toast, currentUser }) {
  ...
  const pushNotification = async (h) => {
    const notifs = await loadShared("notifications", []).catch(() => ls("nv-notifications", []));
    const newNotif = {
      id:Date.now(), type:"handout",
...
function SetExamStudentView({ toast, currentUser, classExams }) {
  ...
  const submitExam = () => {
    ...
    bsGet(uKey(currentUser,"results"),false).then(prev=>{
      const existing=Array.isArray(prev)?prev:ls("nv-results",[]);
      saveMyData("results","nv-results",[...existing,{id:Date.now(),subject:sel.title||sel.subject||"Set Exam",type:"Set Exam",score:correct,total:sel.questions.length,pct,date:new Date().toLocaleDateString()}]);
    }).catch(()=>{ saveMyData("results","nv-results",[...ls("nv-results",[]),{id:Date.now(),subject:sel.title||sel.subject||"Set Exam",type:"Set Exam",score:correct,total:sel.questions.length,pct,date:new Date().toLocaleDateString()}]); });
    notifyUserKey("nv-results");
...
function FlashcardsView() {
  const decks = useShared("decks", DEFAULT_DECKS);
  useEffect(() => { loadShared("decks", DEFAULT_DECKS).then(() => notifyKey("decks")).catch(() => {}); }, []);
...
function DrugGuideView() {
  const drugs = useShared("drugs", DEFAULT_DRUGS);
  useEffect(() => { loadShared("drugs", DEFAULT_DRUGS).then(() => notifyKey("drugs")).catch(() => {}); }, []);
...
function LabReferenceView() {
  const labs = useShared("labs", DEFAULT_LABS);
  useEffect(() => { loadShared("labs", DEFAULT_LABS).then(() => notifyKey("labs")).catch(() => {}); }, []);
  const [search, setSearch] = useState("");
...
function DictionaryView() {
  const dict = useShared("dict", DEFAULT_DICT);
  useEffect(() => { loadShared("dict", DEFAULT_DICT).then(() => notifyKey("dict")).catch(() => {}); }, []);
  const [search, setSearch] = useState("");
  const filtered = dict.filter(d=>d.term.toLowerCase().includes(search.toLowerCase())||d.def.toLowerCase().includes(search.toLowerCase()));
...
function SkillsView() {
  ...
  useEffect(() => { loadShared("skills", DEFAULT_SKILLS).then(() => notifyKey("skills")).catch(() => {}); }, []);
...
// ── Nursing Exams Student View ───────────────────────────────────────
function NursingExamCBT({ exam, currentUser, onBack, toast }) {
  const attKey = `nv-nursing-att-${currentUser}`;
  const [att, setAtt] = useHydratedUser(attKey, "nursing-att", {});
  const attempted = att[String(exam.id)];
  const [started, setStarted] = useState(false);
  const [answers, setAnswers] = useState([]);
  const [qIdx, setQIdx] = useState(0);
  const [done, setDone] = useState(false);
  ...
  const start = () => {
    if (attempted) { toast("You already attempted this exam (1 attempt only)","error"); return; }
    setAnswers(new Array(exam.questions.length).fill(null));
    setQIdx(0); setDone(false); setStarted(true);
  };
  ...
  const submit = () => {
    ...
    const snap = [...answers];
    const score = exam.questions.reduce((s,q,i)=>snap[i]===q.ans?s+1:s,0);
    const pct = Math.round((score/exam.questions.length)*100);
    const newAtt = {...att,[String(exam.id)]:{score,total:exam.questions.length,pct,answers:snap,date:new Date().toLocaleDateString()}};
    setAtt(newAtt);
    bsGet(uKey(currentUser,"results"),false).then(prev=>{
      const existing=Array.isArray(prev)?prev:ls("nv-results",[]);
      saveMyData("results","nv-results",[...existing,{id:Date.now(),subject:exam.title,type:"Nursing CBT",score,total:exam.questions.length,pct,date:new Date().toLocaleDateString()}]);
    }).catch(()=>{ saveMyData("results","nv-results",[...ls("nv-results",[]),{id:Date.now(),subject:exam.title,type:"Nursing CBT",score,total:exam.questions.length,pct,date:new Date().toLocaleDateString()}]); });
    notifyUserKey("nv-results");
    setFinalAnswers(snap); setStarted(false); setDone(true);
  };

  if (done) {
    const score = exam.questions.reduce((s,q,i)=>finalAnswers[i]===q.ans?s+1:s,0);
    const pct = Math.round((score/exam.questions.length)*100);
    return (
      <div>
        <button className="btn btn-sm" style={{marginBottom:16}} onClick={onBack}>← Back</button>
        <div style={{textAlign:"center",padding:"24px 0"}}>
          <div style={{fontSize:52,marginBottom:8}}>{pct>=70?"🎉":pct>=50?"👍":"📚"}</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:22,marginBottom:4}}>CBT Submitted</div>
          <div style={{fontSize:48,fontFamily:"'Syne',sans-serif",fontWeight:800,color:pct>=70?"var(--success)":pct>=50?"var(--warn)":"var(--danger)"}}>{pct}%</div>
          <div style={{fontSize:16,color:"var(--text2)",marginTop:4}}>{score}/{exam.questions.length} correct</div>
        </div>
        <div style={{marginTop:16}}>
          {exam.questions.map((q,i)=>{
            const userAns = finalAnswers[i]; const correct = q.ans;
            const isRight = userAns === correct;
            return (
              <div key={i} className="card" style={{marginBottom:10,borderLeft:`3px solid ${isRight?"var(--success)":"var(--danger)"}`}}>
                <div style={{fontSize:13,fontWeight:600,marginBottom:8}}>Q{i+1}. {q.q}</div>
                {(q.options||[]).map((opt,oi)=>opt&&(
                  <div key={oi} style={{padding:"6px 10px",borderRadius:7,marginBottom:4,fontSize:13,
                    background:oi===correct?"rgba(74,222,128,.12)":oi===userAns&&!isRight?"rgba(248,113,113,.1)":"transparent",
                    border:`1px solid ${oi===correct?"rgba(74,222,128,.3)":oi===userAns&&!isRight?"rgba(248,113,113,.3)":"var(--border)"}`,
                    color:oi===correct?"var(--success)":oi===userAns&&!isRight?"var(--danger)":"var(--text2)"}}>
                    <b>{"ABCD"[oi]}.</b> {opt}
                    {oi===correct&&<span style={{marginLeft:8}}>✓ Correct</span>}
                    {oi===userAns&&!isRight&&<span style={{marginLeft:8}}>✗ Your answer</span>}
                  </div>
                ))}
              </div>
            );
    ...
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:12,color:"var(--text3)"}}>{answers.filter(a=>a!==null).length}/{exam.questions.length} answered</span>
            <button className="btn btn-accent btn-sm" onClick={submit}>Submit ✓</button>
          </div>
        </div>
        <div className="progress-wrap" style={{marginBottom:16,height:5}}>
          <div className="progress-fill" style={{width:`${((qIdx+1)/exam.questions.length)*100}%`}} />
        </div>
        <div className="card" style={{marginBottom:14}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--text3)",marginBottom:8}}>Question {qIdx+1} of {exam.questions.length}</div>
          <div style={{fontSize:15,fontWeight:600,marginBottom:16,lineHeight:1.5}}>{q.q}</div>
          {(q.options||[]).map((opt,oi)=>opt&&(
            <div key={oi} onClick={()=>{const a=[...answers];a[qIdx]=oi;setAnswers(a);}} style={{
              padding:"11px 14px",borderRadius:9,marginBottom:8,cursor:"pointer",fontSize:14,
              border:`1px solid ${answers[qIdx]===oi?"var(--accent)":"var(--border)"}`,
              background:answers[qIdx]===oi?"rgba(62,142,149,.12)":"transparent",transition:"all .15s"
            }}>
              <b style={{color:"var(--accent)",marginRight:8}}>{"ABCD"[oi]}.</b>{opt}
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <button className="btn" disabled={qIdx===0} onClick={()=>setQIdx(i=>i-1)}>← Prev</button>
          <button className="btn btn-accent" disabled={qIdx>=exam.questions.length-1} onClick={()=>setQIdx(i=>i+1)}>Next →</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="card" style={{textAlign:"center",padding:"28px 20px"}}>
        <div style={{fontSize:40,marginBottom:10}}>📋</div>
        <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:20,marginBottom:6}}>{exam.title}</div>
        <div style={{fontSize:13,color:"var(--text2)",marginBottom:16}}>{exam.questions.length} questions · 1 attempt only</div>
        {attempted ? (
          <div style={{color:"var(--text3)",fontFamily:"'DM Mono',monospace",fontSize:13}}>
            ✅ Already attempted — Score: {attempted.pct}% ({attempted.date})
          </div>
        ) : (
          <button className="btn-primary" onClick={start} style={{maxWidth:220,margin:"0 auto"}}>Start CBT Exam →</button>
        )}
      </div>
    </div>
  );
...
function NursingExamReview({ exam, toast }) {
  const [qIdx, setQIdx] = useState(0);
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages:[{role:"user",content:`You are a nursing exam tutor. Explain the answer to this nursing exam question concisely (3-5 sentences). Focus on the clinical rationale.\n\nQuestion: ${q.q}\n\nOptions:\n${(q.options||[]).map((o,i)=>`${String.fromCharCode(65+i)}. ${o}`).join("\n")}\n\nCorrect Answer: ${String.fromCharCode(65+q.ans)}. ${q.options[q.ans]}\n\nExplain why this is correct and why the others are wrong.`}]
        })
      });
      const data = await resp.json();
      const text = (data.content||[]).map(c=>c.text||"").join("");
      setAiExplanation(prev=>({...prev,[qIdx]:text}));
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:15}}>{exam.title} — Review</div>
        <span style={{fontFamily:"'DM Mono',monospace",fontSize:12,color:"var(--text3)"}}>{qIdx+1}/{exam.questions.length}</span>
      </div>
      <div className="progress-wrap" style={{marginBottom:16,height:5}}>
        <div className="progress-fill" style={{width:`${((qIdx+1)/exam.questions.length)*100}%`,background:"var(--accent2)"}} />
      </div>
      <div className="card" style={{marginBottom:14}}>
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--text3)",marginBottom:8}}>Question {qIdx+1}</div>
        <div style={{fontSize:15,fontWeight:600,marginBottom:16,lineHeight:1.5}}>{q.q}</div>
        {(q.options||[]).map((opt,oi)=>opt&&(
          <div key={oi} style={{
            padding:"10px 14px",borderRadius:9,marginBottom:8,fontSize:14,
            border:`1px solid ${showAns&&oi===q.ans?"rgba(74,222,128,.4)":"var(--border)"}`,
            background:showAns&&oi===q.ans?"rgba(74,222,128,.1)":"transparent",
            color:showAns&&oi===q.ans?"var(--success)":"var(--text2)"
          }}>
            <b style={{marginRight:8}}>{"ABCD"[oi]}.</b>{opt}
            {showAns&&oi===q.ans&&<span style={{marginLeft:8,fontSize:12}}>✓ Correct</span>}
          </div>
        ))}
        <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}>
          <button className="btn btn-accent" onClick={()=>setShowAns(s=>!s)}>
            {showAns?"Hide Answer":"Show Answer"}
          </button>
          {showAns&&(
            <button className="btn btn-purple" onClick={getAIExplanation} disabled={loadingAI}>
              {loadingAI?"⏳ Getting AI...":"🤖 AI Explanation"}
            </button>
          )}
        </div>
        {aiExplanation[qIdx]&&(
          <div style={{marginTop:12,background:"rgba(167,139,250,.08)",border:"1px solid rgba(167,139,250,.25)",borderRadius:10,padding:"12px 14px",fontSize:13,color:"var(--text2)",lineHeight:1.6}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"var(--purple)",marginBottom:6,textTransform:"uppercase",letterSpacing:"1px"}}>🤖 AI Explanation</div>
            {aiExplanation[qIdx]}
          </div>
        )}
      </div>
      <div style={{display:"flex",gap:8,justifyContent:"center"}}>
        <button className="btn" disabled={qIdx===0} onClick={()=>{setQIdx(i=>i-1);setShowAns(false);}}>← Prev</button>
        <button className="btn btn-accent" disabled={qIdx>=exam.questions.length-1} onClick={()=>{setQIdx(i=>i+1);setShowAns(false);}}>Next →</button>
      </div>
    </div>
  );
}

function NursingGroupChat({ category, currentUser, toast }) {
  const cat = NURSING_CATEGORIES.find(c=>c.id===category)||NURSING_CATEGORIES[0];
  const roomKey = `db:nursing-chat-${category}`;
  const lsKey = `nv-nursing-chat-${category}`;
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
  const [sending, setSending] = useState(false);
  const [recording, setRecording] = useState(false);
  const [mediaRecorder, setMediaRecorder] = useState(null);
  const [alarm, setAlarm] = useState({enabled:false,time:"",days:[]});
  useEffect(()=>{ bsGet(`db:nursing-alarm-${category}`, false).then(r=>{ if(r) setAlarm(r); }).catch(()=>{}); },[category]);
  const [showAlarm, setShowAlarm] = useState(false);
  const msgsEndRef = useRef(null);
  const fileInputRef = useRef(null);

  useEffect(()=>{
    const fetch=()=>bsGet(roomKey,true).then(r=>{if(r&&Array.isArray(r)){setMsgs(r);}}).catch(()=>{});
    fetch();
  ...
  // Save alarm settings to backend personal storage
  useEffect(()=>{
    bsSet(`db:nursing-alarm-${category}`, alarm, false);
    if(alarm.enabled&&alarm.time){
      ...
      const check=()=>{
        ...
        if(now.getHours()===h&&now.getMinutes()===m&&(alarm.days.length===0||alarm.days.includes(dayName))){
          if(Notification.permission==="granted"){
            new Notification(`📚 Study Time — ${cat.label}`,{body:`Time to study ${cat.label}! Open the app to review.`,icon:"🏥"});
          } else {
            toast(`⏰ Study alarm: Time to study ${cat.label}!`,"info");
          }
        }
      };
  ...
  const sendMsg = async (msgData) => {
    const updated = [...msgs, msgData];
    setMsgs(updated);
    try {
      await bsSet(roomKey, updated, true);
    } catch(e) {
      // If message array is too large (files/images bloat it), trim file data from older msgs
      try {
        const trimmed = updated.map((m,i) => i < updated.length-1 && (m.type==="image"||m.type==="file"||m.type==="voice") ? {...m, fileData:"[file]"} : m);
        await bsSet(roomKey, trimmed, true);
      } catch { toast("Message too large to sync","warn"); }
    }
  };
  ...
  const send = async () => {
    ...
    setSending(true);
    const now = new Date();
    await sendMsg({id:Date.now(),from:currentUser,text:input.trim(),time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString(),type:"text"});
  ...
  const sendFile = async (e) => {
    const file = e.target.files[0]; if(!file) return;
    const maxMB = 4;
    if(file.size > maxMB*1024*1024){ toast(`File too large (max ${maxMB}MB)`,"error"); e.target.value=""; return; }
    setSending(true);
    const reader = new FileReader();
    reader.onerror = () => { toast("Could not read file","error"); setSending(false); };
    reader.onload = async (ev) => {
      try {
        const now = new Date();
        const isImg = file.type.startsWith("image/");
        await sendMsg({id:Date.now(),from:currentUser,type:isImg?"image":"file",
          fileName:file.name,fileType:file.type,fileData:ev.target.result,
          time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString()});
        toast(`${isImg?"Image":"File"} sent ✅`,"success");
      } catch { toast("Failed to send file","error"); }
      setSending(false);
    };
  ...
  const startRecording = async () => {
    try {
      ...
      mr.onstop = async () => {
        ...
        reader.onload = async (ev) => {
          const now = new Date();
          await sendMsg({id:Date.now(),from:currentUser,type:"voice",fileData:ev.target.result,
            time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString()});
          toast("Voice note sent 🎙️","success");
        };
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16,color:cat.color}}>{cat.icon} {cat.label} Group</div>
          <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace"}}>Group chat · all {cat.label} students</div>
        </div>
        <button className="btn btn-sm" onClick={()=>{setShowAlarm(s=>!s);requestAlarmPermission();}} style={{color:alarm.enabled?"var(--warn)":"var(--text3)"}}>
          ⏰ {alarm.enabled?"Alarm ON":"Set Alarm"}
        </button>
      </div>

      {showAlarm&&(
        <div className="card" style={{marginBottom:12,border:"1px solid rgba(251,146,60,.25)",background:"rgba(251,146,60,.06)"}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:10,color:"var(--warn)"}}>⏰ Study Alarm — {cat.label}</div>
          <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:10}}>
            <div>
              <label className="lbl" style={{marginBottom:4}}>Time</label>
              <input type="time" className="inp" style={{width:120,marginBottom:0}} value={alarm.time}
                onChange={e=>setAlarm(a=>({...a,time:e.target.value}))} />
            </div>
            <label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",userSelect:"none",marginTop:16}}>
              <div onClick={()=>setAlarm(a=>({...a,enabled:!a.enabled}))} style={{
                width:20,height:20,borderRadius:5,border:`2px solid ${alarm.enabled?"var(--warn)":"var(--border2)"}`,
                background:alarm.enabled?"var(--warn)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",
                transition:"all .2s",cursor:"pointer"
              }}>{alarm.enabled&&<span style={{fontSize:11,color:"white"}}>✓</span>}</div>
              <span style={{fontSize:13,color:"var(--text2)"}}>Enabled</span>
            </label>
          </div>
          <div style={{marginBottom:8}}>
            <div style={{fontSize:12,color:"var(--text3)",marginBottom:6,fontFamily:"'DM Mono',monospace"}}>REPEAT DAYS (empty = every day)</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {DAYS.map(d=>(
                <div key={d} onClick={()=>setAlarm(a=>({...a,days:a.days.includes(d)?a.days.filter(x=>x!==d):[...a.days,d]}))} style={{
                  padding:"4px 10px",borderRadius:7,cursor:"pointer",fontSize:12,fontFamily:"'DM Mono',monospace",
                  border:`1px solid ${alarm.days.includes(d)?"var(--warn)":"var(--border)"}`,
                  background:alarm.days.includes(d)?"rgba(251,146,60,.15)":"transparent",
                  color:alarm.days.includes(d)?"var(--warn)":"var(--text3)"
                }}>{d}</div>
              ))}
            </div>
          </div>
          {alarm.enabled&&alarm.time&&<div style={{fontSize:11,color:"var(--success)",fontFamily:"'DM Mono',monospace"}}>✅ Alarm set for {alarm.time}{alarm.days.length>0?` on ${alarm.days.join(", ")}`:" daily"}</div>}
        </div>
      )}

      <div className="card" style={{minHeight:260,maxHeight:380,display:"flex",flexDirection:"column",gap:6,padding:12,overflowY:"auto",marginBottom:10}}>
        {msgs.length===0&&<div style={{textAlign:"center",padding:"40px",color:"var(--text3)",fontSize:12,fontFamily:"'DM Mono',monospace"}}>No messages yet. Start the conversation!</div>}
        {msgs.map(m=>{
          ...
          return (
            <div key={m.id} style={{display:"flex",gap:8,alignItems:"flex-end",justifyContent:isMe?"flex-end":"flex-start"}}>
              {!isMe&&<div style={{width:28,height:28,borderRadius:50,background:`${cat.color}30`,border:`1px solid ${cat.color}50`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0,color:cat.color,fontWeight:700}}>{m.from.charAt(0).toUpperCase()}</div>}
              <div style={{maxWidth:"72%"}}>
                {!isMe&&<div style={{fontSize:10,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginBottom:2}}>{m.from} · {m.time}</div>}
                {m.type==="text"&&(
                  <div style={{background:isMe?`linear-gradient(135deg,${cat.color},${cat.color}cc)`:"var(--card2)",borderRadius:isMe?"14px 14px 4px 14px":"14px 14px 14px 4px",padding:"9px 13px",fontSize:13,color:isMe?"white":"var(--text)"}}>
                    {m.text}
                  </div>
                )}
                {m.type==="image"&&(
                  <div style={{borderRadius:10,overflow:"hidden",maxWidth:220,border:"1px solid var(--border)"}}>
                    <img src={m.fileData} alt={m.fileName} style={{width:"100%",display:"block"}} />
                  </div>
                )}
                {m.type==="file"&&(
                  <a href={m.fileData} download={m.fileName} style={{display:"flex",alignItems:"center",gap:8,background:"var(--card2)",borderRadius:10,padding:"10px 14px",textDecoration:"none",color:"var(--text)",border:"1px solid var(--border)"}}>
                    <span style={{fontSize:22}}>📎</span>
                    <div><div style={{fontSize:12,fontWeight:600}}>{m.fileName}</div><div style={{fontSize:10,color:"var(--text3)"}}>Tap to download</div></div>
                  </a>
                )}
                {m.type==="voice"&&(
                  <div style={{background:"var(--card2)",borderRadius:10,padding:"8px 12px",border:"1px solid var(--border)"}}>
                    <audio controls src={m.fileData} style={{height:32,width:180}} />
                    <div style={{fontSize:10,color:"var(--text3)",marginTop:4}}>🎙️ Voice note</div>
                  </div>
                )}
                {isMe&&<div style={{fontSize:10,color:"var(--text3)",textAlign:"right",marginTop:2}}>{m.time}</div>}
              </div>
            </div>
          );
        })}
      ...
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <input ref={fileInputRef} type="file" style={{display:"none"}} onChange={sendFile} accept="*/*" />
        <button className="btn btn-sm" title="Send file" onClick={()=>fileInputRef.current?.click()} style={{flexShrink:0,fontSize:16,padding:"8px 10px"}}>📎</button>
        <button className={`btn btn-sm${recording?" btn-danger":""}`} title={recording?"Stop recording":"Record voice note"}
          onClick={recording?stopRecording:startRecording} style={{flexShrink:0,fontSize:16,padding:"8px 10px"}}>
          {recording?"⏹":"🎙️"}
        </button>
        <input className="inp" style={{flex:1,marginBottom:0}} placeholder="Type a message..." value={input}
          onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} disabled={sending} />
        <button className="btn btn-accent" onClick={send} disabled={sending||!input.trim()} style={{flexShrink:0}}>
          {sending?"...":"Send"}
        </button>
      </div>
    </div>
...
function NursingExamsView({ toast, currentUser }) {
  const [banks] = useHydratedShared("nv-nursing-exams", "nursingExams", []);
  const [selCat, setSelCat] = useState(null);
  const [selBank, setSelBank] = useState(null);
  const [mode, setMode] = useState(null); // null | "cbt" | "review" | "chat"

  if(mode==="cbt"&&selBank){
    return <NursingExamCBT exam={selBank} currentUser={currentUser} toast={toast} onBack={()=>{setMode(null);setSelBank(null);}} />;
  }
  if(mode==="review"&&selBank){
    return (
      <div>
        <button className="btn btn-sm" style={{marginBottom:16}} onClick={()=>{setMode(null);setSelBank(null);}}>← Back</button>
        <NursingExamReview exam={selBank} toast={toast} />
      </div>
    );
  }
  if(mode==="chat"&&selCat){
    return (
      <div>
        <button className="btn btn-sm" style={{marginBottom:16}} onClick={()=>{setMode(null);setSelCat(null);}}>← Back</button>
        <NursingGroupChat category={selCat} currentUser={currentUser} toast={toast} />
      </div>
    );
  }

  const cat = selCat ? NURSING_CATEGORIES.find(c=>c.id===selCat) : null;
  const catBanks = selCat ? banks.filter(b=>b.category===selCat&&b.isPublished) : [];

  return (
    <div>
      <div className="sec-title">🏥 Nursing Exams</div>
      <div className="sec-sub">CBT exam mode · AI-powered review · group study chat</div>

      {/* Category cards */}
      <div className="grid3" style={{marginBottom:24,gap:12}}>
        {NURSING_CATEGORIES.map(c=>{
          const count = banks.filter(b=>b.category===c.id&&b.isPublished).length;
          return (
            <div key={c.id} className="card" style={{
              cursor:"pointer",textAlign:"center",padding:"20px 14px",
              border:`2px solid ${selCat===c.id?c.color:"var(--border)"}`,
              background:selCat===c.id?`${c.color}12`:"transparent",transition:"all .2s"
            }} onClick={()=>setSelCat(s=>s===c.id?null:c.id)}>
              <div style={{fontSize:36,marginBottom:8}}>{c.icon}</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:14,color:selCat===c.id?c.color:"var(--text)"}}>{c.label}</div>
              <div style={{fontSize:11,color:"var(--text3)",marginTop:4,fontFamily:"'DM Mono',monospace"}}>{count} exam{count!==1?"s":""} available</div>
              {selCat===c.id&&<div style={{marginTop:8}}><span style={{fontSize:11,color:c.color}}>▼ selected</span></div>}
            </div>
          );
        })}
      </div>
      ...
      {selCat&&cat&&(
        <div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16,color:cat.color}}>{cat.icon} {cat.label}</div>
            <button className="btn btn-sm" style={{borderColor:cat.color,color:cat.color}} onClick={()=>{setSelCat(selCat);setMode("chat");}}>
              💬 Group Study Chat
            </button>
          </div>

          {catBanks.length===0?(
            <div style={{textAlign:"center",padding:"40px",color:"var(--text3)",fontFamily:"'DM Mono',monospace",fontSize:13}}>
              No published exams for this category yet.
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {catBanks.map(b=>(
                <div key={b.id} className="card" style={{borderLeft:`3px solid ${cat.color}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      <button className="btn btn-accent btn-sm" onClick={()=>{setSelBank(b);setMode("cbt");}}>
                        📝 CBT Mode
                      </button>
                      <button className="btn btn-sm" style={{borderColor:cat.color,color:cat.color}} onClick={()=>{setSelBank(b);setMode("review");}}>
                        🤖 Review + AI
                      </button>
                    </div>
                  </div>
...
function Messages({ user, toast, userClass }) {
  // Support "self" room (note to self) + class room
  const [activeRoom, setActiveRoom] = useState("class"); // "class" | "self"
  const classRoomKey = userClass ? `db:messages-${userClass}` : "db:messages-general";
  const classLsKey  = userClass ? `nv-messages-${userClass}` : "nv-messages-general";
  const selfRoomKey = `db:messages-self-${user}`;
  const selfLsKey   = `nv-messages-self-${user}`;

  const roomKey = activeRoom==="self" ? selfRoomKey : classRoomKey;
  const lsKey   = activeRoom==="self" ? selfLsKey   : classLsKey;
  ...
  useEffect(()=>{
    const fetch=()=>{
      const shared = activeRoom==="self" ? false : true;
      bsGet(roomKey, shared).then(r=>{
        if(r&&Array.isArray(r)&&r.length>0){setMsgs(r);}
      }).catch(()=>{});
    };
    setMsgs([]);
    fetch();
    const t=setInterval(fetch,3000);
    return ()=>clearInterval(t);
  },[roomKey,lsKey,activeRoom]);

  useEffect(()=>{msgsEndRef.current?.scrollIntoView({behavior:"smooth"});},[msgs]);

  const pushMsg = async (msgData) => {
    const updated=[...msgs,msgData];
    setMsgs(updated);
    const shared=activeRoom==="self"?false:true;
    try{
      await bsSet(roomKey,updated,shared);
    } catch {
      try {
        const trimmed = updated.map((m,i) => i < updated.length-1 && (m.type==="image"||m.type==="file"||m.type==="voice") ? {...m, fileData:"[file]"} : m);
        await bsSet(roomKey, trimmed, shared);
      } catch { toast("Message too large to sync","warn"); }
    }
  };
  ...
  const send=async()=>{
    ...
    setSending(true);
    const now=new Date();
    await pushMsg({id:Date.now(),from:user,text:input.trim(),type:"text",
      time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString()});
    setInput(""); setSending(false);
  };
  ...
  const sendFile=async(e)=>{
    const file=e.target.files[0]; if(!file)return;
    if(file.size>4*1024*1024){toast("File too large (max 4MB)","error");e.target.value="";return;}
    setSending(true);
    const reader=new FileReader();
    reader.onerror=()=>{ toast("Could not read file","error"); setSending(false); };
    reader.onload=async(ev)=>{
      try {
        const now=new Date();
        const isImg=file.type.startsWith("image/");
        await pushMsg({id:Date.now(),from:user,type:isImg?"image":"file",
          fileName:file.name,fileType:file.type,fileData:ev.target.result,
          time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString()});
        toast(`${isImg?"Image":"File"} sent ✅`,"success");
      } catch { toast("Failed to send file","error"); }
  ...
  const startRecording=async()=>{
    try{
      ...
      mr.onstop=async()=>{
        ...
        reader.onload=async(ev)=>{
          const now=new Date();
          await pushMsg({id:Date.now(),from:user,type:"voice",fileData:ev.target.result,
            time:now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),date:now.toLocaleDateString()});
          toast("Voice note sent 🎙️","success");
        };
        reader.readAsDataURL(blob);
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div>
          ...
          <div className="sec-sub">{activeRoom==="self"?"📝 Note to Self":"🏫 Class Chat · syncs live"}</div>
        </div>
      </div>

      {/* Room selector */}
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        {[{k:"class",label:"🏫 Class Chat",sub:userClass||"General"},{k:"self",label:"📝 Note to Self",sub:"Private"}].map(r=>(
          <div key={r.k} onClick={()=>setActiveRoom(r.k)} style={{
            flex:1,padding:"10px 14px",borderRadius:10,cursor:"pointer",textAlign:"center",
            border:`1px solid ${activeRoom===r.k?"var(--accent)":"var(--border)"}`,
            background:activeRoom===r.k?"rgba(62,142,149,.1)":"transparent",transition:"all .2s"
          }}>
            <div style={{fontWeight:700,fontSize:13,color:activeRoom===r.k?"var(--accent)":"var(--text2)"}}>{r.label}</div>
            <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginTop:2}}>{r.sub}</div>
          </div>
        ))}
      </div>

      {activeRoom==="class"&&pinnedAnnouncements.map(a=>(
      ...
      <div className="card" style={{minHeight:260,maxHeight:420,display:"flex",flexDirection:"column",gap:6,padding:12,overflowY:"auto",marginBottom:10}}>
        {msgs.length===0&&(
          <div style={{textAlign:"center",padding:"50px 20px",color:"var(--text3)"}}>
            <div style={{fontSize:40,marginBottom:10}}>{activeRoom==="self"?"📝":"💬"}</div>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:12}}>
              {activeRoom==="self"?"Jot down notes to yourself — reminders, ideas, links.":"No messages yet. Say hello!"}
            </div>
          </div>
        )}
        {msgs.map(m=>{
          ...
          return(
            <div key={m.id} style={{display:"flex",gap:8,alignItems:"flex-end",justifyContent:isMe?"flex-end":"flex-start"}}>
              {!isMe&&<div style={{width:28,height:28,borderRadius:50,background:"linear-gradient(135deg,var(--accent),var(--accent2))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0,color:"white",fontWeight:700}}>
              ...
              <div style={{maxWidth:"74%"}}>
                ...
                {m.type==="image"&&(
                  <div style={{borderRadius:10,overflow:"hidden",maxWidth:220,border:"1px solid var(--border)"}}>
                    <img src={m.fileData} alt={m.fileName||"img"} style={{width:"100%",display:"block"}} />
                  </div>
                )}
                {m.type==="file"&&(
                  <a href={m.fileData} download={m.fileName} style={{display:"flex",alignItems:"center",gap:8,background:"var(--card2)",borderRadius:10,padding:"10px 14px",textDecoration:"none",color:"var(--text)",border:"1px solid var(--border)"}}>
                ...
                {m.type==="voice"&&(
                  <div style={{background:"var(--card2)",borderRadius:10,padding:"8px 12px",border:"1px solid var(--border)"}}>
                    <audio controls src={m.fileData} style={{height:32,width:200}} />
                    <div style={{fontSize:10,color:"var(--text3)",marginTop:4}}>🎙️ Voice note</div>
                  </div>
                )}
                {isMe&&<div style={{fontSize:10,color:"var(--text3)",textAlign:"right",marginTop:2}}>{m.time}</div>}
      ...
      <input ref={fileInputRef} type="file" style={{display:"none"}} onChange={sendFile} accept="*/*" />
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <button className="btn btn-sm" title="Attach file" onClick={()=>fileInputRef.current?.click()} style={{flexShrink:0,fontSize:16,padding:"8px 10px"}}>📎</button>
        <button className={`btn btn-sm${recording?" btn-danger":""}`} title={recording?"Stop":"Record voice"}
          onClick={recording?stopRecording:startRecording} style={{flexShrink:0,fontSize:16,padding:"8px 10px"}}>
          {recording?"⏹":"🎙️"}
        </button>
        <input className="inp" style={{flex:1,marginBottom:0}} placeholder={activeRoom==="self"?"Write a note...":"Type a message..."}
          value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} disabled={sending} />
        <button className="btn btn-accent" onClick={send} disabled={sending||!input.trim()} style={{flexShrink:0}}>
          {sending?"...":"Send"}
        </button>
      </div>
...
function Notifications({ currentUser, onRead }) {
  // Notifications are stored in shared backend storage so all students receive them
  const [notifs, setNotifs] = useState([]);
  
  useEffect(() => {
    // Hydrate from backend and subscribe to saveShared("notifications") changes
    loadShared("notifications", []).then(remote => {
      if (Array.isArray(remote)) setNotifs(remote);
    }).catch(() => {});
    const refresh = () => setNotifs(ls("nv-notifications", []));
    if (!_sharedSubs["notifications"]) _sharedSubs["notifications"] = new Set();
    _sharedSubs["notifications"].add(refresh);
    window.addEventListener("nv:shared-hydrated", refresh);
    return () => {
      _sharedSubs["notifications"]?.delete(refresh);
      window.removeEventListener("nv:shared-hydrated", refresh);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    saveShared("notifications", updated);
    setNotifs(updated);
    if (onRead) onRead();
  }, [notifs.length]);
  ...
  const del = (id) => {
    ...
    saveShared("notifications", u);
    setNotifs(u);
  };
  const clearAll = () => {
    saveShared("notifications", []);
    setNotifs([]);
  };
...
export default function App() {
  useEffect(() => {
    hydrateFromBackend();
    // Background sync: re-hydrate all shared data every 60s so changes from
    // other devices (admin edits, new handouts, new exams) appear automatically.
    const bgSync = setInterval(() => { hydrateFromBackend(); }, 60000);
  ...
  const login = async () => {
    ...
    // Timeout after 5s so a hung window.storage call never freezes login
    const withTimeout = (promise, ms = 5000) => Promise.race([promise, new Promise(res => setTimeout(res, ms))]);
    const safeLoad = async (fn) => { try { await withTimeout(fn()); } catch(e) { console.warn("safeLoad:", e); } };

    // 1. Fetch user list from backend (required for cross-device login)
    //    Falls back to _mem cache if backend is unreachable
    ...
    await Promise.allSettled([
      ...
      safeLoad(() => loadUser(u, "nursing-att",   `nv-nursing-att-${u}`,  {})).then(() => notifyUserKey(`nv-nursing-att-${u}`)),
    ]);
    ...
    if (rememberMe) {
      ...
      _mem["nv-remembered-email"] = "";
    }

    // Read unread count from shared backend notifications
    loadShared("notifications", []).then(list => {
      if (Array.isArray(list)) setUnreadNotifs(list.filter(n => !n.read).length);
  ...
  const register = async () => {
    ...
    const users = await bsGet("db:users", true) || ls("nv-users", []);
  ...
  const resetPassword = async () => {
    ...
    // Update password in users list from backend
    const users = await bsGet("db:users", true) || ls("nv-users", []);
  ...
  const renderContent = () => {
    ...
    switch (activeNav) {
      ...
      case "nursingexams": return <NursingExamsView toast={toast} currentUser={currentUser} />;
  ...
  const NAV = [
    ...
    { icon:"🏥", label:"Nursing Exams", key:"nursingexams" },
