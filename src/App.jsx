...
function CbtExamManager({ toast, currentUser }) {
  ...
  const [view, setView] = useState("list"); // list | create | edit | monitor | results
  const [selExam, setSelExam] = useState(null);
  const [loading, setLoading] = useState(true);
  const classes = ls("nv-classes", DEFAULT_CLASSES);
  const users   = ls("nv-users", []);

  const blank = { title:"", subject:"", classId:"", duration:30, questions:[], published:false, allowRetake:false, createdBy:"", createdAt:null, id:null };
  const [form, setForm] = useState({...blank});
  const [qForm, setQForm] = useState({ q:"", options:["","","",""], ans:0 });
  ...
  // Load data
  useEffect(() => {
    const unsub1 = subscribeCbtExams(list => { setExams(list); setLoading(false); });
    const unsub2 = subscribeCbtResults(list => setResults(list));
    return () => { unsub1(); unsub2(); };
  }, []);

  const saveExams = async (list) => {
    setExams(list);
    const ok = await cbtExamsSave(list);
    if (!ok) toast("⚠️ Saved locally but sync failed","warn");
  };

  const saveResults = async (list) => {
    setResults(list);
    const ok = await cbtResultsSave(list);
    if (!ok) toast("⚠️ Results sync failed","warn");
  };

  const createExam = async () => {
    if (!form.title.trim()) return toast("Exam title required","error");
    if (!form.classId) return toast("Select a class","error");
    if (form.questions.length < 1) return toast("Add at least 1 question","error");
    setSyncing(true);
    const exam = { ...form, id: Date.now(), createdBy: currentUser, createdAt: Date.now() };
    const updated = form.id ? exams.map(e=>e.id===form.id?exam:e) : [...exams, exam];
    await saveExams(updated);
    setSyncing(false);
    toast(form.id ? "✅ Exam updated!" : "✅ Exam created!", "success");
  ...
  const publishExam = async (id, val) => {
    const updated = exams.map(e=>e.id===id?{...e,published:val}:e);
    await saveExams(updated);
    toast(val ? "🚀 Exam published! Students can now take it." : "Exam unpublished","success");
  };

  const allowRetakeForStudent = async (examId, studentEmail) => {
    // Remove this student's result so they can retake
    const updated = results.filter(r=>!(r.examId===examId && r.student===studentEmail));
    await saveResults(updated);
    toast(`✅ ${studentEmail} can now retake the exam`,"success");
  };

  const addQuestion = () => {
    if (!qForm.q.trim()) return toast("Question text required","error");
    if (!qForm.options[0]||!qForm.options[1]) return toast("At least options A and B required","error");
    const q = { q:qForm.q.trim(), options:qForm.options.map(o=>o.trim()), ans:qForm.ans };
    let qs;
    if (editQIdx !== null) { qs = form.questions.map((qq,i)=>i===editQIdx?q:qq); setEditQIdx(null); toast("Question updated","success"); }
    else { qs = [...form.questions, q]; toast("Question added","success"); }
  ...
  const importParsed = () => {
    setForm(f=>({...f, questions:[...f.questions,...parsedQs]}));
    setParsedQs([]); setPasteText(""); setPasteAnswers(""); setPasteMode(false);
    toast(`${parsedQs.length} questions imported!`,"success");
  };

  // Monitor: results for selected exam
  const examResults = selExam ? results.filter(r=>r.examId===selExam.id).sort((a,b)=>b.score-a.score) : [];
  const studentsInClass = selExam ? users.filter(u=>u.class===selExam.classId && u.role==="student") : [];
  const notYetTaken = selExam ? studentsInClass.filter(s=>!examResults.find(r=>r.student===s.username)) : [];

  const printResults = () => {
    const examTitle = selExam?.title || "";
    const className = classes.find(c=>c.id===selExam?.classId)?.label || selExam?.classId || "";
    const rows = examResults.map((r,i)=>`
      <tr style="background:${i%2===0?'#f0f8ff':'white'}">
        ...
        <td style="padding:8px 12px;border:1px solid #ccc;text-align:center;font-weight:bold;color:${r.percent>=70?'#16a34a':r.percent>=50?'#b45309':'#dc2626'}">${r.percent}%</td>
    ...
    const html = `
    <!DOCTYPE html><html><head><title>${examTitle} - Results</title>
    <style>body{font-family:'Times New Roman',serif;padding:30px;color:#000}h1{font-size:20px;margin-bottom:4px}p{font-size:13px;color:#444;margin-bottom:20px}table{width:100%;border-collapse:collapse}th{background:#0077b6;color:white;padding:10px 12px;border:1px solid #ccc;text-align:left}@media print{button{display:none}}</style></head>
    <body>
    <h1>📋 ${examTitle}</h1>
    <p>Class: ${className} &nbsp;|&nbsp; Total Questions: ${selExam?.questions?.length} &nbsp;|&nbsp; Duration: ${selExam?.duration} minutes &nbsp;|&nbsp; Generated: ${new Date().toLocaleString()}</p>
    <button onclick="window.print()" style="margin-bottom:16px;padding:8px 18px;background:#0077b6;color:white;border:none;border-radius:6px;cursor:pointer;font-size:14px">🖨️ Print</button>
    <table><thead><tr><th>#</th><th>Student Email</th><th>Score</th><th>Percentage</th><th>Submitted At</th></tr></thead>
    <tbody>${rows}</tbody></table>
    ${notYetTaken.length>0?`<p style="margin-top:20px;color:#b45309">⚠️ ${notYetTaken.length} student(s) have not taken the exam: ${notYetTaken.map(s=>s.username).join(", ")}</p>`:""}
    </body></html>`;
    const w = window.open("","_blank","width=900,height=700");
    w.document.write(html); w.document.close();
  };

  if (loading) return <div style={{textAlign:"center",padding:60,color:"var(--text3)"}}>⏳ Loading CBT exams...</div>;

  // ── Create / Edit view ──
  if (view==="create"||view==="edit") return (
    <div style={{maxWidth:720,margin:"0 auto"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18,flexWrap:"wrap"}}>
        <button className="btn btn-sm" onClick={()=>{setView("list");setForm({...blank});}}>← Back</button>
        <div className="sec-title" style={{marginBottom:0}}>{view==="edit"?"✏️ Edit Exam":"📝 Create New Exam"}</div>
      </div>

      {/* Exam meta */}
      <div className="card" style={{marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:14,marginBottom:12,color:"var(--accent)"}}>📋 Exam Details</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div>
            <label className="lbl">Exam Title *</label>
            <input className="inp" style={{marginBottom:0}} value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="e.g. Midterm Test – Anatomy" />
          </div>
          <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div>
            ...
            <input className="inp" style={{marginBottom:0}} type="number" min="5" max="180" value={form.duration} onChange={e=>setForm(f=>({...f,duration:+e.target.value}))} />
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
          <div style={{fontWeight:800,fontSize:14,color:"var(--accent)"}}>❓ Questions ({form.questions.length})</div>
          <button className={`btn btn-sm${pasteMode?" btn-purple":""}`} onClick={()=>setPasteMode(p=>!p)}>📋 Paste Multiple</button>
        </div>

        {pasteMode&&(
          <div style={{marginBottom:14,background:"var(--bg4)",borderRadius:10,padding:12,border:"1px solid var(--border)"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:8}}>
              <div>
                <div style={{fontSize:11,fontWeight:800,color:"var(--accent)",marginBottom:4}}>📝 Questions (Q: / A: / B: / C: / D:)</div>
                <textarea className="paste-box" rows={8} placeholder={"Q: What is the normal body temperature?\nA: 35.0°C\nB: 36.1–37.2°C\nC: 38.5°C\nD: 40.0°C\n\nQ: Which organ produces insulin?\nA: Liver\nB: Kidney\nC: Pancreas\nD: Spleen"} value={pasteText} onChange={e=>{setPasteText(e.target.value);setParsedQs([]);}} />
              </div>
              <div>
                <div style={{fontSize:11,fontWeight:800,color:"var(--success)",marginBottom:4}}>✅ Answers (one per line: A / B / C / D)</div>
                <textarea className="paste-box" rows={8} placeholder={"B\nC"} value={pasteAnswers} onChange={e=>{setPasteAnswers(e.target.value);setParsedQs([]);}} style={{borderColor:"rgba(34,197,94,.35)"}} />
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-accent" onClick={parsePaste}>🔍 Parse</button>
              {parsedQs.length>0&&<button className="btn btn-success" onClick={importParsed}>✅ Import {parsedQs.length}</button>}
            ...
            {parsedQs.length>0&&(
              <div style={{marginTop:10,border:"1px solid var(--success)",borderRadius:8,overflow:"hidden"}}>
                <div style={{padding:"6px 12px",background:"rgba(34,197,94,.1)",fontSize:12,fontWeight:800,color:"var(--success)"}}>✓ {parsedQs.length} parsed — review then import</div>
                {parsedQs.map((p,i)=>(
                  <div key={i} style={{padding:"6px 12px",borderTop:"1px solid var(--border)",fontSize:12}}>
                    <span style={{fontWeight:700}}>{i+1}. {p.q}</span> <span style={{color:"var(--accent)"}}>ANS: {"ABCD"[p.ans]}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* Single question form */}
        <div style={{background:"var(--bg4)",borderRadius:10,padding:12,border:"1px solid var(--border)",marginBottom:12}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:8,color:"var(--text)"}}>{editQIdx!==null?`✏️ Edit Q${editQIdx+1}`:"➕ Add Question"}</div>
          <label className="lbl">Question *</label>
          <textarea className="inp" rows={2} style={{resize:"vertical"}} value={qForm.q} onChange={e=>setQForm(f=>({...f,q:e.target.value}))} placeholder="Enter question text..." />
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            {["A","B","C","D"].map((l,i)=>(
              <div key={i}>
                <label className="lbl">Option {l}{i<=1?" *":""}</label>
                <input className="inp" style={{marginBottom:0}} value={qForm.options[i]} onChange={e=>setQForm(f=>{const opts=[...f.options];opts[i]=e.target.value;return{...f,options:opts};})} placeholder={`Option ${l}`} />
              </div>
            ))}
          </div>
          <label className="lbl">Correct Answer</label>
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            {["A","B","C","D"].map((l,i)=>(
              <button key={i} onClick={()=>setQForm(f=>({...f,ans:i}))} className="btn btn-sm"
                style={{flex:1,borderColor:qForm.ans===i?"var(--success)":"var(--border)",background:qForm.ans===i?"rgba(34,197,94,.12)":"transparent",color:qForm.ans===i?"var(--success)":"var(--text3)",fontWeight:qForm.ans===i?800:400}}>
                {l}{qForm.ans===i?" ✓":""}
              </button>
            ))}
          </div>
          <div style={{display:"flex",gap:8}}>
            <button className="btn btn-accent" onClick={addQuestion}>{editQIdx!==null?"💾 Update":"➕ Add"}</button>
            {editQIdx!==null&&<button className="btn" onClick={()=>{setEditQIdx(null);setQForm({q:"",options:["","","",""],ans:0});}}>Cancel</button>}
          </div>
        </div>

        {/* Question list */}
        {form.questions.length===0&&<div style={{textAlign:"center",padding:"20px",color:"var(--text3)",fontSize:13,border:"1px dashed var(--border)",borderRadius:10}}>No questions yet. Add above or paste multiple.</div>}
        {form.questions.map((q,i)=>(
          <div key={i} className="card2" style={{marginBottom:8,borderLeft:`3px solid ${editQIdx===i?"var(--accent)":"var(--border)"}`}}>
            <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
              ...
              <div style={{display:"flex",gap:4,flexShrink:0}}>
                <button className="btn btn-sm" onClick={()=>{setEditQIdx(i);setQForm({q:q.q,options:[...q.options],ans:q.ans});}}>✏️</button>
                <button className="btn btn-sm btn-danger" onClick={()=>deleteQ(i)}>🗑️</button>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        <button className="btn btn-accent" style={{flex:1}} onClick={createExam} disabled={syncing}>
          {syncing?"⏳ Saving...":view==="edit"?"💾 Save Changes":"✅ Create Exam"}
        </button>
        <button className="btn" onClick={()=>{setView("list");setForm({...blank});}}>Cancel</button>
      </div>
    </div>
  );

  // ── Monitor / Results view ──
  if ((view==="monitor"||view==="results") && selExam) return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        ...
        <div>
          ...
          <div style={{fontSize:12,color:"var(--text3)"}}>
            {classes.find(c=>c.id===selExam.classId)?.label} · {selExam.questions.length} questions · {selExam.duration} min
          </div>
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:8}}>
          <button className="btn btn-sm" style={{borderColor:"var(--accent)",color:"var(--accent)"}} onClick={printResults}>🖨️ Print Results</button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:18}}>
        {[
          ...
          {icon:"✅",label:"Completed",val:examResults.length,color:"var(--success)"},
          {icon:"⏳",label:"Pending",val:notYetTaken.length,color:"var(--warn)"},
          {icon:"📊",label:"Avg Score",val:examResults.length?Math.round(examResults.reduce((s,r)=>s+r.percent,0)/examResults.length)+"%":"—",color:"var(--purple)"},
        ].map((s,i)=>(
      ...
      {/* Live indicator */}
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,padding:"8px 14px",background:"rgba(34,197,94,.06)",border:"1px solid rgba(34,197,94,.2)",borderRadius:10}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:"var(--success)",animation:"spin 2s linear infinite"}} />
        <span style={{fontSize:12,fontWeight:700,color:"var(--success)"}}>Live — auto-refreshes every 6 seconds</span>
      </div>
      ...
      {examResults.length===0 ? (
        <div className="card" style={{textAlign:"center",padding:"48px 20px",color:"var(--text3)"}}>
          ...
          <div style={{fontSize:12,marginTop:4}}>Results will appear here as students complete the exam.</div>
        </div>
      ) : (
        <div className="card" style={{padding:0,overflow:"hidden",marginBottom:16}}>
          <div style={{padding:"12px 16px",borderBottom:"1px solid var(--border)",fontWeight:800,fontSize:14,background:"var(--bg4)"}}>
          ...
          <table className="tbl">
            ...
            <tbody>
              {examResults.map((r,i)=>{
                ...
                return (
                  <tr key={r.student} style={{background:i===0?"rgba(34,197,94,.04)":""}}>
                    <td style={{textAlign:"center",fontWeight:700,color:i===0?"gold":i===1?"silver":i===2?"#cd7f32":"var(--text3)"}}>
                      {i===0?"🥇":i===1?"🥈":i===2?"🥉":i+1}
                    </td>
                    <td style={{fontWeight:600}}>{r.student}</td>
                    <td style={{fontWeight:700,color:"var(--accent)"}}>{r.score}/{r.total}</td>
                    <td><div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{flex:1,height:6,borderRadius:3,background:"var(--bg3)",overflow:"hidden",minWidth:60}}>
                        <div style={{height:"100%",width:`${r.percent}%`,background:gColor,borderRadius:3,transition:"width .5s"}} />
                      </div>
                      <span style={{fontWeight:700,color:gColor,fontSize:12,minWidth:35}}>{r.percent}%</span>
                    </div></td>
                    ...
                    <td>
                      <button className="btn btn-sm" title="Allow retake" onClick={()=>allowRetakeForStudent(selExam.id, r.student)}>🔄 Retake</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {notYetTaken.length>0&&(
        <div className="card" style={{borderLeft:"3px solid var(--warn)"}}>
          <div style={{fontWeight:800,fontSize:13,marginBottom:8,color:"var(--warn)"}}>⏳ Students Who Haven't Taken the Exam ({notYetTaken.length})</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {notYetTaken.map(s=>(
              <span key={s.username} style={{fontSize:12,padding:"4px 10px",borderRadius:20,background:"rgba(251,146,60,.1)",border:"1px solid rgba(251,146,60,.25)",color:"var(--warn)"}}>
                {s.username}
              </span>
            ))}
          </div>
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:12,color:"var(--text3)",marginTop:2}}>Create, publish and monitor Computer-Based Tests for your students.</div>
        </div>
        <button className="btn btn-accent" onClick={()=>{setForm({...blank});setView("create");}}>+ Create Exam</button>
      </div>

      {exams.filter(e=>e.createdBy===currentUser||true).length===0 ? (
        <div style={{textAlign:"center",padding:"60px 20px",color:"var(--text3)"}}>
          <div style={{fontSize:52,marginBottom:12}}>📝</div>
          <div style={{fontWeight:700,marginBottom:6}}>No exams yet</div>
          <div style={{fontSize:12}}>Create your first CBT exam to get started.</div>
        </div>
      ) : (
        <div style={{display:"grid",gap:12}}>
          {exams.map(e=>{
            ...
            return (
              <div key={e.id} className="card" style={{borderLeft:`4px solid ${e.published?"var(--success)":"var(--border2)"}`}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}>
                  <div style={{flex:1,minWidth:200}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                      <div style={{fontWeight:800,fontSize:15}}>{e.title}</div>
                      <span className={`tag${e.published?" tag-success":""}`} style={{fontSize:10}}>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end"}}>
                    ...
                    <button className="btn btn-sm" onClick={()=>{setForm({...e});setView("edit");}}>✏️ Edit</button>
                    {e.published
                      ? <button className="btn btn-sm" style={{borderColor:"var(--warn)",color:"var(--warn)"}} onClick={()=>publishExam(e.id,false)}>📤 Unpublish</button>
                      : <button className="btn btn-sm btn-success" onClick={()=>publishExam(e.id,true)}>🚀 Publish</button>
                    }
...
function CbtStudentView({ toast, currentUser }) {
  ...
  const [examState, setExamState] = useState("idle"); // idle | taking | done
  const [answers, setAnswers] = useState([]);
  const [qIdx, setQIdx] = useState(0);
  ...
  const submitExam = async () => {
    ...
    const score = answers.reduce((s,a,i)=>s+(a===exam.questions[i].ans?1:0),0);
  ...
  // Available exams for my class
  const available = exams.filter(e=>e.published && e.classId===myClass);
  ...
  if (examState==="taking" && activeExam) {
    ...
    return (
      <div style={{maxWidth:620,margin:"0 auto"}}>
        {/* Header */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:14,flexWrap:"wrap",
          background:"var(--card)",borderRadius:12,padding:"12px 16px",border:"1px solid var(--border)"}}>
          <div>
            <div style={{fontWeight:800,fontSize:14}}>{activeExam.title}</div>
            <div style={{fontSize:11,color:"var(--text3)"}}>{answeredCount}/{activeExam.questions.length} answered</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{
              fontWeight:800,fontSize:18,padding:"6px 14px",borderRadius:8,fontFamily:"'DM Mono',monospace",
              background:urgent?"rgba(239,68,68,.12)":"rgba(0,119,182,.1)",
              color:urgent?"var(--danger)":"var(--accent)",
              border:`2px solid ${urgent?"var(--danger)":"var(--accent)"}`,
              animation:urgent&&timeLeft%2===0?"pulse .5s ease-in-out":"none"
            }}>⏱ {fmtTime(timeLeft)}</div>
            <button className="btn btn-sm btn-danger" onClick={()=>{if(confirm("Submit exam now?"))submitExam();}}>Submit ✓</button>
          </div>
        </div>
        ...
        {/* Progress */}
        <div className="progress-wrap" style={{marginBottom:14}}>
          <div className="progress-fill" style={{width:`${(answeredCount/activeExam.questions.length)*100}%`,background:"var(--accent)"}} />
        </div>

        <div style={{fontSize:11,color:"var(--text3)",marginBottom:6}}>Question {qIdx+1} of {activeExam.questions.length}</div>
        ...
        {q.options.filter(o=>o).map((opt,i)=>(
          <div key={i} onClick={()=>setAnswers(prev=>{const n=[...prev];n[qIdx]=i;return n;})}
            className="quiz-opt" style={{
              borderColor:answers[qIdx]===i?"var(--accent)":"var(--border)",
              background:answers[qIdx]===i?"rgba(0,119,182,.12)":"transparent",
        ...
        <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"space-between"}}>
          <button className="btn btn-sm" disabled={qIdx===0} onClick={()=>setQIdx(q=>q-1)}>← Prev</button>
          {qIdx<activeExam.questions.length-1
            ?<button className="btn btn-sm btn-accent" onClick={()=>setQIdx(q=>q+1)}>Next →</button>
            :<button className="btn btn-sm btn-success" onClick={()=>{if(confirm("Submit exam? You cannot change answers after submission."))submitExam();}}>Submit Exam ✓</button>
          }
        </div>
      </div>
    );
    ...
    return (
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <div className="card" style={{textAlign:"center",padding:"32px 20px",marginBottom:16,borderTop:`4px solid ${gColor}`}}>
          <div style={{fontSize:56,marginBottom:8}}>{myResult.percent>=70?"🎉":myResult.percent>=50?"👍":"😔"}</div>
          <div style={{fontWeight:800,fontSize:22,marginBottom:4}}>Exam Submitted!</div>
          <div style={{fontSize:14,color:"var(--text3)",marginBottom:20}}>{activeExam?.title}</div>
          <div style={{display:"flex",justifyContent:"center",gap:24,flexWrap:"wrap"}}>
            <div><div style={{fontSize:40,fontWeight:800,color:"var(--accent)"}}>{myResult.score}/{myResult.total}</div><div style={{fontSize:12,color:"var(--text3)"}}>Score</div></div>
            <div><div style={{fontSize:40,fontWeight:800,color:gColor}}>{myResult.percent}%</div><div style={{fontSize:12,color:"var(--text3)"}}>Percentage</div></div>
            <div><div style={{fontSize:40,fontWeight:800,color:gColor}}>{grade}</div><div style={{fontSize:12,color:"var(--text3)"}}>Grade</div></div>
          </div>
        </div>
        ...
        {activeExam?.questions.map((q,i)=>{
          const chosen = myResult.answers[i];
          const correct = chosen===q.ans;
          return (
            <div key={i} className="card" style={{marginBottom:10,borderLeft:`3px solid ${correct?"var(--success)":"var(--danger)"}`}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>{i+1}. {q.q}</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {q.options.filter(o=>o).map((opt,oi)=>(
                  <span key={oi} style={{fontSize:11,padding:"3px 9px",borderRadius:6,
                    background:oi===q.ans?"rgba(34,197,94,.15)":oi===chosen&&!correct?"rgba(239,68,68,.1)":"transparent",
        ...
        <button className="btn btn-accent" onClick={()=>{setExamState("idle");setActiveExam(null);setMyResult(null);}}>← Back to Exams</button>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{fontSize:12,color:"var(--text3)",marginBottom:20}}>Computer-Based Tests assigned to your class. You can only take each exam once unless reset by your lecturer.</div>
      ...
      {available.length===0 ? (
        ...
        <div style={{display:"grid",gap:12}}>
          {available.map(e=>{
            ...
            const cls = classes.find(c=>c.id===e.classId);
            return (
              <div key={e.id} className="card" style={{borderLeft:`4px solid ${attempted?"var(--success)":"var(--accent)"}`}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                      <div style={{fontWeight:800,fontSize:15}}>{e.title}</div>
                  ...
                  <div>
                    {attempted
                      ? <button className="btn btn-sm" onClick={()=>{setActiveExam(e);setMyResult(myR);setExamState("done");}}>View Result</button>
      ...
      {myResults.length>0&&(
        <div style={{marginTop:24}}>
          <div style={{fontWeight:800,fontSize:14,marginBottom:10}}>📊 My Past Results</div>
          <div className="card" style={{padding:0,overflow:"hidden"}}>
            <table className="tbl">
              <thead><tr><th>Exam</th><th>Score</th><th>%</th><th>Grade</th><th>Date</th></tr></thead>
              <tbody>
