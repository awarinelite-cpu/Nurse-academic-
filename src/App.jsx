...
export default function App() {
  ...
  const login = async () => {
    ...
    // Check local cache first for instant feedback, verify against backend in parallel
    const localUsers = ls("nv-users", []);
    const localUser = localUsers.find(u => u.username === username && u.password === password);
    // If not found locally, wait for backend — otherwise proceed immediately
    if (!localUser) {
      const fresh = await loadShared("users", [{username:"admin@gmail.com",password:"admin123",role:"admin",class:"",joined:"System"}]);
      const remoteUser = (fresh||[]).find(u => u.username === username && u.password === password);
      if (!remoteUser) return toast("Invalid credentials", "error");
      ...
      // Refresh users from backend in background (non-blocking)
      loadShared("users", localUsers);
    }
    const user = ls("nv-users", localUsers).find(u => u.username === username) || localUser;
    // Enter app immediately using cached data
    setCurrentUserRef(username); setCurrentUser(username);
    setIsAdmin(user.role === "admin"); setIsLecturer(user.role === "lecturer");
    setPage("app");
    toast(`Welcome back! 👋`, "success");
    // Hydrate user private data from backend in background (all in parallel)
    Promise.all([
      loadUser(username, "results",     "nv-results",                  []),
      loadUser(username, "notifications","nv-notifications",            []),
      loadUser(username, "essay-att",   `nv-essay-att-${username}`,    {}),
      loadUser(username, "gpa-courses", "nv-gpa-courses",              []),
      loadUser(username, "skills-done", "nv-skills-done",              {}),
    ]).then(() => {
      const notifs = ls("nv-notifications", []);
