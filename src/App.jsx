...
const SK = {
  ...
  nursingExams:  ["nv-nursing-exams", "db:nursing-exams"],
  schoolPQ:      ["nv-school-pq",     "db:school-pq"],
  schoolExams:   ["nv-school-exams",  "db:school-exams"],
  folders:       ["nv-folders",       "db:folders"],
};
...
// Also sync all private per-user data keys to backend (call after hydration)
const syncUserPrivateData = async (username) => {
  const privKeys = [
    ["results","nv-results",[]],
    ["notifications","nv-notifications",[]],
    [`essay-att`,`nv-essay-att-${username}`,{}],
    ["gpa-courses","nv-gpa-courses",[]],
    ["skills-done","nv-skills-done",{}],
    ["mcq-att",`nv-exam-attempts-${username}`,{}],
  ];
  await Promise.all(privKeys.map(async ([suffix, lsKey, fallback])=>{
    try {
      const remote = await bsGet(uKey(username, suffix), false);
      if (remote !== null) lsSet(lsKey, remote);
      else {
        const local = ls(lsKey, fallback);
        if (JSON.stringify(local) !== JSON.stringify(fallback)) bsSet(uKey(username, suffix), local, false);
      }
    } catch {}
  }));
};
...
const hydrateFromBackend = async () => {
  const defaults = {
    ...
    handouts: [], essayBanks: [], nursingExams: {general:[],midwifery:[],publichealth:[]}, schoolPQ: {}, folders: {},
  };
...
function AdminPQ({ toast }) {
  ...
  const importParsed = () => {
    ...
    toast(`${parsed.length} questions imported!`,"success"); setPasteText(""); setPasteAnswers(""); setParsed([]); setPasteMode(false);
  };
...
function AdminHandouts({ toast }) {
  ...
  const saveFolders = (f) => { setFolders(f); lsSet("nv-folders", f); bsSet("db:folders", f, true); };
    ...
    <div>
        ...
        <div>
          <div style={{background:"rgba(0,119,182,.07)",border:"1px solid rgba(0,119,182,.18)",borderRadius:10,padding:"12px 16px",fontSize:13,color:"var(--accent2)",marginBottom:16,display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div>
              ...
              <span style={{fontSize:12}}>Pre-create course folders for each class so lecturers can find the right location to upload. Each class → course → lecturer folder.</span>
            </div>
          </div>
          <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            <button className="btn btn-accent" onClick={initAllFolders}>⚡ Initialize All Classes</button>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {classes.map(cls=>{
              const classFolders = folders[cls.id]||{};
              const courseCount = Object.keys(classFolders).length;
              return (
                <div key={cls.id} className="card" style={{borderLeft:`4px solid ${cls.color||"var(--accent)"}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:courseCount?10:0}}>
                    <div>
                      <div style={{fontWeight:800,fontSize:14}}>{cls.label}</div>
                      <div style={{fontSize:11,color:"var(--text3)"}}>{cls.desc} · {courseCount} course folder{courseCount!==1?"s":""}</div>
                    </div>
                    <button className="btn btn-sm" onClick={()=>{
                      const existing=folders[cls.id]||{};
                      const newF={...existing};
                      (cls.courses||[]).forEach(c=>{ if(!newF[c]) newF[c]=[]; });
                      saveFolders({...folders,[cls.id]:newF});
                      toast(`✅ Folders for ${cls.label} updated!`,"success");
                    }}>⚡ Sync Courses</button>
                  </div>
                  {courseCount>0&&(
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                      {Object.keys(classFolders).map(course=>{
                        const lecCount=(classFolders[course]||[]).length;
                        const hCount=handouts.filter(h=>h.classId===cls.id&&h.course===course).length;
                        return (
                          <span key={course} style={{fontSize:11,padding:"3px 10px",borderRadius:7,border:"1px solid var(--border2)",color:"var(--accent2)"}}>
                            📂 {course} ({lecCount} lec, {hCount} files)
                          </span>
                        );
                      })}
                    </div>
                  ...
                  {courseCount===0&&(
                    <div style={{fontSize:12,color:"var(--text3)"}}>No folders yet — click "Sync Courses" to create them from class defaults</div>
                  )}
                </div>
              );
            })}
      ...
      {showInitModal&&(
        <div className="modal-overlay" onClick={()=>setShowInitModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-head"><div className="modal-title">⚡ Initialize Course Folders</div><button className="modal-close" onClick={()=>setShowInitModal(false)}>✕</button></div>
            <div style={{fontSize:12,color:"var(--text3)",marginBottom:12}}>This will create empty course folders for all default courses in the selected class.</div>
            <label className="lbl">Select Class</label>
            <select className="inp" value={initClass} onChange={e=>setInitClass(e.target.value)}>
              <option value="">— Choose a class —</option>
              {classes.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const save = () => {
    ...
    const item = {
      id: Date.now(), title: form.title, note: form.note,
      ...
      ...(form.uploadType==="pdf" ? {pdfData:pdfFile,pdfName} : {})
    };
    ...
    // Auto-drill into the new location
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        ...
        {isLecturer && <button className="btn btn-accent" onClick={()=>setShowAdd(p=>!p)}>+ Upload Handout</button>}
      </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:12}}>
            ...
            <div>
              ...
              <select className="inp" style={{marginBottom:0}} value={form.course} onChange={e=>setForm({...form,course:e.target.value})} disabled={!form.classId}>
                ...
                {(classes.find(c=>c.id===form.classId)?.courses||[]).map(c=><option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            {/* Lecturer */}
            <div>
              ...
              {coursesInClass.length===0 ? (
                ...
                <div className="grid2">
                  {coursesInClass.map(course=>{
                    ...
                    return (
                      <FolderCard key={course} icon="📂" label={course}
        ...
        <div className="modal-overlay" onClick={()=>setViewItem(null)}>
          <div className="modal" style={{maxWidth:800,width:"95vw"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              {viewItem.course&&<span className="tag tag-accent">📂 {viewItem.course}</span>}
              {viewItem.lecturerName&&<span className="tag" style={{borderColor:"var(--purple)",color:"var(--purple)"}}>👨‍🏫 {viewItem.lecturerName}</span>}
              {viewItem.pdfName&&<span className="tag" style={{borderColor:"var(--danger)",color:"var(--danger)"}}>📄 PDF</span>}
              ...
              <div>
                <div style={{background:"var(--bg4)",borderRadius:10,padding:"8px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:10,fontSize:12,color:"var(--text3)"}}>
                  ...
                  <a href={viewItem.pdfData} download={viewItem.pdfName}
                    ...
                    onClick={e=>e.stopPropagation()}>⬇️ Download</a>
                </div>
                <iframe src={viewItem.pdfData} style={{width:"100%",height:"65vh",border:"1px solid var(--border)",borderRadius:10,display:"block"}} title={viewItem.title} />
              </div>
            ) : (
      ...
      <div style={{maxWidth:680,margin:"0 auto"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:14}}>
          <div>
            ...
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"var(--text3)"}}>{sel.questions.length} questions · AI graded · {totalWords} words total</div>
          </div>
        </div>
        ...
        {sel.questions.map((q,i)=>(
          <div key={i} className="card" style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:15,flex:1,marginRight:10}}>Q{i+1}. {q.q}</div>
              <span style={{fontSize:11,color:"var(--accent)",fontFamily:"'DM Mono',monospace",flexShrink:0,fontWeight:700}}>[{q.marks||10} marks]</span>
            </div>
            <textarea className="inp" rows={5} style={{resize:"vertical",marginBottom:4}}
...
function AdminSchoolPQ({ toast }) {
  const classes = ls("nv-classes", DEFAULT_CLASSES);
  const [data, setData] = useState(() => ls("nv-school-pq", {}));
  ...
  const saveData = (nd) => { setData(nd); lsSet("nv-school-pq", nd); bsSet("db:school-pq", JSON.stringify(nd), true); };
  ...
  const saveCourse = (cid, course, courseData) => saveData({...data,[courseKey(cid,course)]:courseData});
  ...
  const importMCQ = () => {
    ...
    saveCourse(selClass,selCourse,{...cd,mcq:[...cd.mcq,...parsed.map(p=>({...p,id:Date.now()+Math.random()}))]});
    setParsed([]); setPasteText(""); setPasteAnswers(""); toast(`${parsed.length} MCQ(s) added!`,"success");
  };
...
// ─── NURSING EXAM META (admin-managed, no hardcoded questions) ─────────
...
// ─── ADMIN: Nursing Exams Manager ──────────────────────────────────────
function AdminNursingExams({ toast }) {
  const [data, setData] = useState(() => ls("nv-nursing-exams", {general:[], midwifery:[], publichealth:[]}));
  const [activeExam, setActiveExam] = useState("general");
  ...
  const [editPaperIdx, setEditPaperIdx] = useState(null);

  const meta = NURSING_EXAM_META[activeExam];
  const papers = data[activeExam] || [];

  const saveData = (newData) => {
    setData(newData);
    lsSet("nv-nursing-exams", newData);
    bsSet("db:nursing-exams", JSON.stringify(newData), true);
  };
  ...
  const parsePaste = () => {
    ...
    const items = blocks.map((block, idx) => {
      ...
      lines.forEach(line => {
        ...
        if (m) {
          ...
          if (letter==="Q") q = m[2];
          else if (letter==="A") options[0] = m[2];
          else if (letter==="B") options[1] = m[2];
          else if (letter==="C") options[2] = m[2];
          else if (letter==="D") options[3] = m[2];
        } else if (!q) q = line.replace(/^\d+[.)]\s*/,"");
      });
      // Override with answers column if provided
      if (ansLines[idx]) { const a = "ABCD".indexOf(ansLines[idx][0]?.toUpperCase()); if(a>=0)ans=a; }
    ...
    setParsed(items);
    if (items.length===0) toast("No questions parsed. Check format.","error");
    else toast(`${items.length} question(s) parsed successfully!`,"success");
  };

  const createPaper = () => {
    if (!newPaperTitle.trim()) return toast("Paper title required","error");
    let updated;
    if (editPaperIdx!==null) {
      updated = papers.map((p,i)=>i===editPaperIdx?{...p,title:newPaperTitle.trim()}:p);
      toast("Paper updated","success");
    } else {
      const newPaper = {id:Date.now(), title:newPaperTitle.trim(), questions:[], published:false, publishedAt:null};
      updated = [...papers, newPaper];
      toast("Paper created — add questions then publish","success");
    }
    saveData({...data,[activeExam]:updated});
    setShowPaperModal(false); setNewPaperTitle(""); setEditPaperIdx(null);
  };

  const deletePaper = (pid) => {
    if (!confirm("Delete this entire paper and all its questions?")) return;
    saveData({...data,[activeExam]:papers.filter(p=>p.id!==pid)});
    if (editPaper===pid) setEditPaper(null);
    toast("Paper deleted","success");
  };

  const publishPaper = (pid) => {
    saveData({...data,[activeExam]:papers.map(p=>p.id===pid?{...p,published:true,publishedAt:Date.now()}:p)});
    toast("🚀 Published! Students can see this now. Archives in 24 hours.","success");
  };

  const unpublishPaper = (pid) => {
    saveData({...data,[activeExam]:papers.map(p=>p.id===pid?{...p,published:false,publishedAt:null}:p)});
    toast("Paper unpublished","warn");
  };

  const saveQuestion = (pid) => {
    ...
    const nd = {...data,[activeExam]:papers.map(p=>{
      if (p.id!==pid) return p;
      const qs = editIdx!==null ? p.questions.map((qq,i)=>i===editIdx?q:qq) : [...p.questions,q];
      return {...p,questions:qs};
    })};
    saveData(nd); setSingleForm({q:"",options:["","","",""],ans:0}); setEditIdx(null);
    toast(editIdx!==null?"Question updated":"Question added","success");
  };

  const importParsed = (pid) => {
    if (!parsed.length) return;
    const nd = {...data,[activeExam]:papers.map(p=>p.id===pid?{...p,questions:[...p.questions,...parsed.map(pp=>({...pp}))]}:p)};
    saveData(nd); setParsed([]); setPasteText(""); setPasteAnswers(""); toast(`${parsed.length} questions added!`,"success");
  };

  const deleteQ = (pid, qi) => {
    saveData({...data,[activeExam]:papers.map(p=>p.id===pid?{...p,questions:p.questions.filter((_,i)=>i!==qi)}:p)});
    toast("Question deleted","success");
  };

  const examKeys = Object.keys(NURSING_EXAM_META);

  return (
    <div>
      <div className="sec-title" style={{marginBottom:4}}>🎓 Nursing Exams Manager</div>
      <div style={{fontSize:12,color:"var(--text3)",marginBottom:18}}>Create question papers, add questions, publish instantly. Papers auto-archive after 24 hours into permanent student archive.</div>

      {/* Exam category tabs */}
      <div style={{display:"flex",gap:8,marginBottom:22,flexWrap:"wrap"}}>
        {examKeys.map(key=>{
          const m = NURSING_EXAM_META[key];
          const cnt = (data[key]||[]).length;
          return (
            <div key={key} onClick={()=>{setActiveExam(key);setEditPaper(null);setParsed([]);setPasteText("");setPasteAnswers("");}}
              style={{flex:"1 1 170px",padding:"14px 16px",borderRadius:12,cursor:"pointer",transition:"all .2s",textAlign:"center",
                border:`2px solid ${activeExam===key?m.color:"var(--border)"}`,
                background:activeExam===key?`${m.color}18`:"var(--card)"}}>
              <div style={{fontSize:28,marginBottom:4}}>{m.icon}</div>
              <div style={{fontWeight:800,fontSize:13,color:activeExam===key?m.color:"var(--text)"}}>{m.short}</div>
              <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>{cnt} paper{cnt!==1?"s":""}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontWeight:800,fontSize:16,color:meta.color}}>{meta.icon} {meta.label}</div>
          <div style={{fontSize:12,color:"var(--text3)",marginTop:2}}>{meta.desc}</div>
        </div>
        <button className="btn btn-purple" onClick={()=>{setShowPaperModal(true);setEditPaperIdx(null);setNewPaperTitle("");}}>+ New Paper</button>
      </div>

      {papers.length===0 && (
        <div style={{textAlign:"center",padding:"48px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:14}}>
          <div style={{fontSize:44,marginBottom:10}}>{meta.icon}</div>
          <div style={{fontWeight:700,marginBottom:6}}>No papers yet for {meta.short}</div>
          <div style={{fontSize:12}}>Click "+ New Paper" to create the first question paper.</div>
        </div>
      )}

      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        {papers.map((paper,pi)=>{
          const archived = isPaperArchived(paper);
          const isOpen = editPaper===paper.id;
          const timeLeft = paper.publishedAt&&!archived ? Math.max(0,24*60*60*1000-(Date.now()-paper.publishedAt)) : null;
          const hoursLeft = timeLeft ? Math.floor(timeLeft/3600000) : 0;
          const minsLeft = timeLeft ? Math.floor((timeLeft%3600000)/60000) : 0;

          return (
            <div key={paper.id} className="card" style={{borderLeft:`4px solid ${archived?"var(--text3)":paper.published?meta.color:"var(--border)"}`,padding:0,overflow:"hidden"}}>
              {/* Paper header row */}
              <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 18px",cursor:"pointer",flexWrap:"wrap"}}
                onClick={()=>setEditPaper(isOpen?null:paper.id)}>
                <div style={{fontSize:22}}>{archived?"🗃️":paper.published?"✅":"📋"}</div>
                <div style={{flex:1,minWidth:180}}>
                  <div style={{fontWeight:800,fontSize:15}}>{paper.title}</div>
                  <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>
                    {paper.questions.length} question{paper.questions.length!==1?"s":""}
                    {archived&&" · 🗃️ Archived permanently"}
                    {paper.published&&!archived&&` · ⏳ Archives in ${hoursLeft}h ${minsLeft}m`}
                    {!paper.published&&!archived&&" · 📋 Draft (not visible to students)"}
                  </div>
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}} onClick={e=>e.stopPropagation()}>
                  {!paper.published&&!archived&&(
                    <button className="btn btn-sm btn-accent" style={{background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none"}}
                      onClick={()=>publishPaper(paper.id)}>🚀 Save & Publish</button>
                  )}
                  {paper.published&&!archived&&(
                    <button className="btn btn-sm btn-warn" onClick={()=>unpublishPaper(paper.id)}>⏸ Unpublish</button>
                  )}
                  {archived&&<span className="tag" style={{color:"var(--text3)",borderColor:"var(--text3)"}}>Archived</span>}
                  <button className="btn btn-sm" onClick={()=>{setEditPaperIdx(pi);setNewPaperTitle(paper.title);setShowPaperModal(true);}}>✏️</button>
                  <button className="btn btn-sm btn-danger" onClick={()=>deletePaper(paper.id)}>🗑️</button>
                </div>
              </div>
              ...
              {isOpen&&(
                <div style={{padding:"0 18px 20px",borderTop:"1px solid var(--border)"}}>
                  {/* Mode selector */}
                  <div style={{display:"flex",gap:8,margin:"14px 0 12px",alignItems:"center",flexWrap:"wrap"}}>
                    <div style={{fontWeight:800,fontSize:13,flex:1,color:meta.color}}>Add Questions to this Paper</div>
                    <button className={`btn btn-sm${addMode==="single"?" btn-accent":""}`}
                      style={addMode==="single"?{background:meta.color,border:"none"}:{}}
                      onClick={()=>{setAddMode("single");setEditIdx(null);}}>✏️ Single Question</button>
                    <button className={`btn btn-sm${addMode==="paste"?" btn-accent":""}`}
                      style={addMode==="paste"?{background:meta.color,border:"none"}:{}}
                      onClick={()=>setAddMode("paste")}>📋 Paste Multiple</button>
                  </div>
                  ...
                  {addMode==="single"&&(
                    <div className="card2" style={{marginBottom:14,border:`1px solid ${meta.color}30`}}>
                      <div style={{fontWeight:800,marginBottom:10,fontSize:13,color:meta.color}}>
                        {editIdx!==null?`✏️ Editing Question ${editIdx+1}`:"✏️ New Single Question"}
                      </div>
                      <label className="lbl">Question Text *</label>
                      <textarea className="inp" rows={3} style={{resize:"vertical"}} value={singleForm.q}
                        onChange={e=>setSingleForm({...singleForm,q:e.target.value})}
                        placeholder="Type the full question here..." />
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                        {["A","B","C","D"].map((l,i)=>(
                          <div key={l}>
                            <label className="lbl">Option {l}{i<2?" *":""}</label>
                            <input className="inp" style={{marginBottom:0}} placeholder={`Option ${l}...`}
                              value={singleForm.options[i]}
                              onChange={e=>{const o=[...singleForm.options];o[i]=e.target.value;setSingleForm({...singleForm,options:o});}} />
                          </div>
                        ))}
                      </div>
                      ...
                      <select className="inp" value={singleForm.ans} onChange={e=>setSingleForm({...singleForm,ans:+e.target.value})}>
                        {["A","B","C","D"].map((l,i)=>(
                          <option key={l} value={i}>Option {l}{singleForm.options[i]?`: ${singleForm.options[i]}`:""}</option>
                        ))}
                      </select>
                      <div style={{display:"flex",gap:8}}>
                        <button className="btn btn-accent" style={{background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none"}}
                          onClick={()=>saveQuestion(paper.id)}>
                          {editIdx!==null?"💾 Update Question":"➕ Add Question"}
                        </button>
                        {editIdx!==null&&<button className="btn" onClick={()=>{setEditIdx(null);setSingleForm({q:"",options:["","","",""],ans:0});}}>Cancel</button>}
                      </div>
                    </div>
                  )}
                  ...
                  {addMode==="paste"&&(
                    <div className="card2" style={{marginBottom:14,border:`1px solid ${meta.color}30`}}>
                      <div style={{fontWeight:800,marginBottom:8,fontSize:13,color:meta.color}}>📋 Paste Multiple Questions at Once</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:8}}>
                        <div>
                          <div style={{fontSize:11,fontWeight:800,color:meta.color,marginBottom:4}}>📝 Questions (with A/B/C/D options)</div>
                          <textarea className="paste-box" rows={14}
                            placeholder={"Q: The normal adult temperature is:\nA: 35.0°C\nB: 36.1–37.2°C\nC: 38.5°C\nD: 40.0°C\n\nQ: Which organ produces insulin?\nA: Liver\nB: Kidney\nC: Pancreas\nD: Spleen\n\n1. Normal adult pulse rate is:\nA) 40-60 bpm\nB) 60-100 bpm\nC) 100-120 bpm\nD) 120-140 bpm"}
                            value={pasteText} onChange={e=>{setPasteText(e.target.value);setParsed([]);}} />
                        </div>
                        <div>
                          <div style={{fontSize:11,fontWeight:800,color:"var(--success)",marginBottom:4}}>✅ Answers (one per line: A / B / C / D)</div>
                      ...
                      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                        <button className="btn btn-accent" style={{background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none"}}
                          onClick={parsePaste}>🔍 Auto-Parse Questions</button>
                        {parsed.length>0&&(
                          <button className="btn btn-success" onClick={()=>importParsed(paper.id)}>
                            ✅ Add {parsed.length} Question{parsed.length!==1?"s":""} to Paper
                          </button>
                        )}
                        <button className="btn" onClick={()=>{setParsed([]);setPasteText("");setPasteAnswers("");}}>🗑️ Clear</button>
                      </div>
                      ...
                      {parsed.length>0&&(
                        <div style={{marginTop:12,border:"1px solid var(--success)",borderRadius:8,overflow:"hidden"}}>
                          <div style={{padding:"9px 14px",background:"rgba(34,197,94,.1)",fontSize:12,fontWeight:800,color:"var(--success)"}}>
                            ✓ {parsed.length} question{parsed.length!==1?"s":""} parsed — review then click "Add to Paper"
                          </div>
                          {parsed.map((p,i)=>(
                            <div key={i} style={{padding:"10px 14px",borderTop:"1px solid var(--border)"}}>
                              <div style={{fontWeight:700,fontSize:13,marginBottom:6}}>{i+1}. {p.q}</div>
                              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                                {p.options.filter(o=>o).map((opt,oi)=>(
                                  <span key={oi} style={{padding:"2px 8px",borderRadius:5,fontSize:11,
                                    background:oi===p.ans?"rgba(34,197,94,.15)":"var(--bg4)",
                                    border:`1px solid ${oi===p.ans?"var(--success)":"var(--border)"}`,
                                    color:oi===p.ans?"var(--success)":"var(--text3)",fontWeight:oi===p.ans?800:400
                                  }}>{"ABCD"[oi]}. {opt}{oi===p.ans?" ✓":""}</span>
                  ...
                  <div style={{fontWeight:800,fontSize:13,marginBottom:10,color:"var(--text)"}}>
                    📋 {paper.questions.length} Question{paper.questions.length!==1?"s":""} in Paper
                  </div>
                  {paper.questions.length===0&&(
                    <div style={{textAlign:"center",padding:"20px",color:"var(--text3)",fontSize:13,border:"1px dashed var(--border)",borderRadius:10}}>
                      No questions yet. Add single questions or paste multiple above.
                    </div>
                  )}
                  {paper.questions.map((q,qi)=>(
                    <div key={qi} className="card2" style={{marginBottom:8,borderLeft:`3px solid ${qi===editIdx&&addMode==="single"?meta.color:"var(--border)"}`}}>
                      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                        ...
                        <div style={{display:"flex",gap:4,flexShrink:0}}>
                          <button className="btn btn-sm" onClick={()=>{setSingleForm({q:q.q,options:[...q.options],ans:q.ans});setEditIdx(qi);setAddMode("single");}}>✏️</button>
                          <button className="btn btn-sm btn-danger" onClick={()=>deleteQ(paper.id,qi)}>🗑️</button>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {/* Publish CTA */}
                  {paper.questions.length>0&&!paper.published&&!archived&&(
                    <div style={{marginTop:16,padding:"14px 16px",background:`${meta.color}0e`,borderRadius:10,border:`1px solid ${meta.color}35`,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:800,color:meta.color,marginBottom:2}}>Ready to go live?</div>
                        <div style={{fontSize:11,color:"var(--text3)"}}>Click publish and students will see it instantly. Auto-archives in 24 hours.</div>
                      </div>
                      <button className="btn btn-accent" style={{background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none",padding:"10px 20px",fontSize:14}}
                        onClick={()=>publishPaper(paper.id)}>🚀 Save & Publish →</button>
                    </div>
                  )}
                </div>
      ...
      {showPaperModal&&(
        <div className="modal-overlay" onClick={()=>setShowPaperModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <div className="modal-title">{editPaperIdx!==null?"Edit":"New"} Question Paper — {meta.short}</div>
              <button className="modal-close" onClick={()=>setShowPaperModal(false)}>✕</button>
            </div>
            <label className="lbl">Paper Title *</label>
            <input className="inp" value={newPaperTitle}
              onChange={e=>setNewPaperTitle(e.target.value)}
              placeholder={`e.g. ${meta.short} Mock Exam — June 2025`}
              onKeyDown={e=>e.key==="Enter"&&createPaper()} />
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-purple" style={{flex:1}} onClick={createPaper}>
                {editPaperIdx!==null?"Save Changes":"Create Paper"}
              </button>
              <button className="btn" onClick={()=>setShowPaperModal(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
...
// ─── STUDENT: Nursing Exams View ───────────────────────────────────────
function NursingExamsView({ toast, currentUser }) {
  const [data, setData] = useState(() => ls("nv-nursing-exams", {general:[], midwifery:[], publichealth:[]}));
  const [activeExam, setActiveExam] = useState("general");
  ...
  useEffect(() => {
    bsGet("db:nursing-exams", true).then(remote => {
      if (remote) { const parsed = typeof remote==="string"?JSON.parse(remote):remote; lsSet("nv-nursing-exams",parsed); setData(parsed); }
    }).catch(()=>{});
  }, []);

  const meta = NURSING_EXAM_META[activeExam];
  const allPapers = data[activeExam] || [];
  const visiblePapers = allPapers.filter(p=>p.published||isPaperArchived(p));
  ...
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:22,flexWrap:"wrap"}}>
        {examKeys.map(key=>{
          const m = NURSING_EXAM_META[key];
          const vis = (data[key]||[]).filter(p=>p.published||isPaperArchived(p));
          return (
            <div key={key} onClick={()=>setActiveExam(key)} style={{
              flex:"1 1 160px",padding:"14px 16px",borderRadius:12,cursor:"pointer",transition:"all .2s",textAlign:"center",
              border:`2px solid ${activeExam===key?m.color:"var(--border)"}`,
              background:activeExam===key?`${m.color}15`:"var(--card)"}}>
              <div style={{fontSize:30,marginBottom:4}}>{m.icon}</div>
              <div style={{fontWeight:800,fontSize:13,color:activeExam===key?m.color:"var(--text)"}}>{m.short}</div>
              <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>{vis.length} paper{vis.length!==1?"s":""}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <div style={{width:38,height:38,borderRadius:10,background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{meta.icon}</div>
        <div>
      ...
      {visiblePapers.length===0&&(
        <div style={{textAlign:"center",padding:"56px 20px",color:"var(--text3)"}}>
          ...
          <div style={{fontWeight:700,fontSize:15,marginBottom:6}}>No papers available yet</div>
          <div style={{fontSize:12}}>Your lecturer will publish question papers here soon. Check back later!</div>
        </div>
      )}
      ...
      <div className="grid2" style={{gap:14}}>
        {visiblePapers.map((paper,i)=>{
          ...
          const att = ls(attKey,{})[String(paper.id)];
          const timeLeft = paper.publishedAt&&!archived ? Math.max(0,24*60*60*1000-(Date.now()-paper.publishedAt)) : null;
          const hoursLeft = timeLeft ? Math.floor(timeLeft/3600000) : 0;
          const minsLeft = timeLeft ? Math.floor((timeLeft%3600000)/60000) : 0;

          return (
            <div key={paper.id} className="card" style={{animation:`fadeUp .35s ease ${i*.08}s both`,borderTop:`4px solid ${archived?"var(--text3)":meta.color}`}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:10}}>
                <div style={{fontSize:22}}>{archived?"🗃️":"📋"}</div>
                <div style={{flex:1}}>
                  <div style={{fontWeight:800,fontSize:14}}>{paper.title}</div>
                  <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>
                    {paper.questions.length} question{paper.questions.length!==1?"s":""}
                    {archived&&" · 🗃️ Archived"}
                    {!archived&&timeLeft!==null&&` · ⏳ ${hoursLeft}h ${minsLeft}m remaining`}
                  </div>
                </div>
              </div>
              ...
              {att&&(
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:13,fontWeight:700,marginBottom:4}}>
                    Score: <span style={{color:att.pct>=70?"var(--success)":att.pct>=50?"var(--warn)":"var(--danger)"}}>{att.score}/{att.total} ({att.pct}%)</span>
                  </div>
                  <div className="progress-wrap" style={{marginBottom:3}}>
                    <div className="progress-fill" style={{width:`${att.pct}%`,background:att.pct>=70?"var(--success)":att.pct>=50?"var(--warn)":"var(--danger)"}} />
                  </div>
                  <div style={{fontSize:10,color:"var(--text3)"}}>🔒 Taken on {att.date}</div>
                </div>
              )}

              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {!att&&paper.questions.length>0&&(
                  <button className="btn btn-sm btn-accent" style={{background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none",flex:1}}
                    onClick={()=>{setActivePaper(paper);setMode("exam");}}>📝 Take Exam</button>
                )}
                {paper.questions.length>0&&(
                  <button className="btn btn-sm" style={{flex:att?1:0,borderColor:meta.color,color:meta.color}}
                    onClick={()=>{setActivePaper(paper);setMode("review");}}>📖 Review Mode</button>
                )}
                {paper.questions.length===0&&<span style={{fontSize:12,color:"var(--text3)",fontStyle:"italic"}}>No questions added yet</span>}
              </div>
            </div>
          );
        })}
    ...
    return (
      <div style={{maxWidth:620,margin:"0 auto"}}>
        <div style={{textAlign:"center",padding:"28px 0 20px"}}>
          ...
          <div style={{fontSize:11,color:"var(--text3)"}}>🔒 1 attempt used — contact lecturer to reset</div>
        </div>
...
function SchoolPastQuestionsView({ toast, currentUser }) {
  ...
  useEffect(()=>{
    bsGet("db:school-pq",true).then(remote=>{
      if(remote){const p=typeof remote==="string"?JSON.parse(remote):remote;lsSet("nv-school-pq",p);setData(p);}
    }).catch(()=>{});
  },[]);
...
function NursingExamsStandaloneView({ toast, currentUser }) {
  return (
    <div>
      <div style={{marginBottom:20}}>
      ...
      <NursingExamsView toast={toast} currentUser={currentUser} />
    </div>
  );
}
...
// ── Anti-cheat: full-screen lock + tab/blur detection ──────────────────
function useExamGuard({ active, onViolation }) {
  useEffect(() => {
    if (!active) return;
    const handleVisChange = () => { if (document.hidden) onViolation("tab"); };
    const handleBlur = () => onViolation("blur");
    const handleCtxMenu = (e) => { e.preventDefault(); };
    const handleKeyDown = (e) => {
      // Block F12, Ctrl+Shift+I, Ctrl+U, Alt+Tab etc.
      if (e.key === "F12" || (e.ctrlKey && e.shiftKey && ["I","J","C"].includes(e.key)) ||
          (e.ctrlKey && e.key === "u") || (e.altKey && e.key === "Tab")) {
        e.preventDefault(); onViolation("devtools");
      }
    };
...
// ── The live exam taking UI ──────────────────────────────────────────────
function ExamTakingView({ exam, currentUser, onSubmit }) {
  const [answers, setAnswers] = useState({});
  ...
  const handleViolation = useCallback((type) => {
    if (submitted) return;
    const msgs = {
      tab: "⚠️ You switched tabs or minimised the window!",
      blur: "⚠️ You left the exam window!",
      devtools: "⚠️ Restricted key combination detected!"
    };
    setViolationMsg(msgs[type] || "⚠️ Exam integrity violation detected!");
    setViolations(v => {
      const next = v + 1;
      // Record violation to shared storage
      const vKey = `exam-violations:${exam.id}:${currentUser}`;
      bsGet(vKey, true).then(existing => {
        const list = existing || [];
        bsSet(vKey, [...list, { type, time: new Date().toLocaleTimeString(), count: next }], true);
      });
      if (next >= 3) {
  ...
  }, [submitted, exam.id, currentUser]);

  useExamGuard({ active: !submitted, onViolation: handleViolation });
  ...
  const doSubmit = useCallback((autoSubmit = false) => {
    ...
    // Save to shared backend so lecturer can see it
    const rKey = `exam-result:${exam.id}:${currentUser}`;
    bsSet(rKey, result, true);
    // Update index
    bsGet(`exam-results-index:${exam.id}`, true).then(idx => {
      const list = idx || [];
      bsSet(`exam-results-index:${exam.id}`, [...list.filter(e=>e.student!==currentUser), { student: currentUser, submittedAt: result.submittedAt, pct, score, total: result.total, violations }], true);
    });
    // Mark as attempted in user's private storage
    const attKey = `nv-exam-att-${currentUser}`;
    const att = ls(attKey, {});
    att[exam.id] = { pct, score, total: questions.length, submittedAt: result.submittedAt };
    lsSet(attKey, att);
    bsSet(uKey(currentUser, "exam-att"), att, false);
    onSubmit(result);
  }, [submitted, answers, exam, currentUser, violations, startTime, onSubmit]);
  ...
  if (submitted) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"60vh", gap:20 }}>
      ...
      <div style={{ fontWeight:800, fontSize:22 }}>Exam Submitted!</div>
      <div style={{ color:"var(--text3)", fontSize:14 }}>Your answers have been recorded. Results will be published by your lecturer.</div>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto", position:"relative" }}>
      ...
      {violationMsg && (
        <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:9999, background:"rgba(239,68,68,0.96)", color:"white", padding:"16px 24px", fontSize:15, fontWeight:800, textAlign:"center", animation:"fadeIn .3s" }}>
          {violationMsg} {violations < 3 ? `(Warning ${violations}/3 — 3rd will auto-submit)` : "Auto-submitting..."}
        </div>
      )}

      {/* Header bar */}
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20, padding:"14px 18px", background:"var(--card)", borderRadius:14, border:"1px solid var(--border)", flexWrap:"wrap" }}>
        <div style={{ flex:1, minWidth:180 }}>
          <div style={{ fontWeight:800, fontSize:16 }}>{exam.title}</div>
          <div style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>{exam.subject} · {questions.length} Questions · {answered} answered</div>
        </div>
        <div style={{ display:"flex", gap:12, alignItems:"center", flexWrap:"wrap" }}>
          {violations > 0 && (
            <div style={{ background:"rgba(239,68,68,.12)", border:"1px solid rgba(239,68,68,.3)", borderRadius:8, padding:"4px 10px", fontSize:11, color:"#ef4444", fontWeight:800 }}>
              ⚠️ {violations} Violation{violations!==1?"s":""}
            </div>
          )}
          <div style={{ background: isUrgent ? "rgba(239,68,68,.12)" : "rgba(0,119,182,.1)", border:`1px solid ${isUrgent?"rgba(239,68,68,.3)":"var(--border)"}`, borderRadius:10, padding:"8px 16px", fontFamily:"'DM Mono',monospace", fontWeight:800, fontSize:18, color: isUrgent ? "#ef4444" : "var(--accent)", minWidth:90, textAlign:"center" }}>
            {mm}:{ss}
          </div>
          <button className="btn btn-danger btn-sm" onClick={()=>setShowConfirm(true)}>Submit Exam</button>
        </div>
      </div>

      {/* Question navigator */}
      <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:16, padding:"12px 14px", background:"var(--card2)", borderRadius:12, border:"1px solid var(--border)" }}>
        {questions.map((_, i) => (
          <div key={i} onClick={()=>setCurrentQ(i)} style={{ width:34, height:34, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, cursor:"pointer", transition:"all .15s",
            background: i===currentQ ? "var(--accent)" : flagged[i] ? "rgba(251,146,60,.2)" : answers[i]!==undefined ? "rgba(34,197,94,.15)" : "var(--bg4)",
            border: `2px solid ${i===currentQ ? "var(--accent)" : flagged[i] ? "var(--warn)" : answers[i]!==undefined ? "var(--success)" : "var(--border)"}`,
            color: i===currentQ ? "white" : flagged[i] ? "var(--warn)" : answers[i]!==undefined ? "var(--success)" : "var(--text3)"
          }}>{i+1}</div>
        ))}
        <div style={{ marginLeft:"auto", fontSize:11, color:"var(--text3)", alignSelf:"center" }}>
          <span style={{ color:"var(--success)" }}>■</span> Answered &nbsp;
          <span style={{ color:"var(--warn)" }}>■</span> Flagged &nbsp;
          <span style={{ color:"var(--text3)" }}>■</span> Unanswered
        </div>
      </div>
      ...
      {q && (
        <div className="card" style={{ marginBottom:16, padding:"24px 28px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
            <div style={{ fontSize:12, color:"var(--text3)", fontFamily:"'DM Mono',monospace" }}>Question {currentQ+1} of {questions.length}</div>
            <button onClick={()=>setFlagged(f=>({...f,[currentQ]:!f[currentQ]}))} style={{ background: flagged[currentQ] ? "rgba(251,146,60,.15)" : "transparent", border:`1px solid ${flagged[currentQ]?"var(--warn)":"var(--border)"}`, borderRadius:7, padding:"4px 10px", cursor:"pointer", color: flagged[currentQ] ? "var(--warn)" : "var(--text3)", fontSize:12, fontWeight:700, transition:"all .15s" }}>
              {flagged[currentQ] ? "🚩 Flagged" : "🏳 Flag"}
            </button>
          </div>
          <div style={{ fontWeight:800, fontSize:16, lineHeight:1.6, marginBottom:24, color:"var(--text)" }}>{q.q}</div>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {q.options.filter(o=>o).map((opt, oi) => (
              <div key={oi} onClick={()=>setAnswers(a=>({...a,[currentQ]:oi}))} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderRadius:12, cursor:"pointer", transition:"all .2s",
                background: answers[currentQ]===oi ? "rgba(0,119,182,.1)" : "var(--bg4)",
                border: `2px solid ${answers[currentQ]===oi ? "var(--accent)" : "var(--border)"}`,
                transform: answers[currentQ]===oi ? "translateX(4px)" : "none"
              }}>
                <div style={{ width:32, height:32, borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:800, flexShrink:0,
                  background: answers[currentQ]===oi ? "var(--accent)" : "var(--card)",
                  color: answers[currentQ]===oi ? "white" : "var(--text3)",
                  border: `1px solid ${answers[currentQ]===oi ? "var(--accent)" : "var(--border)"}`
                }}>{"ABCD"[oi]}</div>
                <div style={{ fontSize:14, fontWeight: answers[currentQ]===oi ? 800 : 600, color: answers[currentQ]===oi ? "var(--accent)" : "var(--text)" }}>{opt}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display:"flex", gap:10, justifyContent:"space-between" }}>
        <button className="btn" onClick={()=>setCurrentQ(q=>Math.max(0,q-1))} disabled={currentQ===0}>← Previous</button>
        ...
        {currentQ < questions.length-1
          ? <button className="btn btn-accent" onClick={()=>setCurrentQ(q=>Math.min(questions.length-1,q+1))}>Next →</button>
          : <button className="btn btn-success" onClick={()=>setShowConfirm(true)}>Review & Submit →</button>
        }
      </div>
      ...
      {showConfirm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div className="card" style={{ maxWidth:440, width:"100%", padding:"28px 32px", textAlign:"center" }}>
            <div style={{ fontSize:48, marginBottom:12 }}>📋</div>
            <div style={{ fontWeight:800, fontSize:18, marginBottom:8 }}>Submit Your Exam?</div>
            <div style={{ fontSize:13, color:"var(--text3)", marginBottom:20, lineHeight:1.6 }}>
              You have answered <b style={{color:"var(--accent)"}}>{answered}</b> of <b>{questions.length}</b> questions.
              {answered < questions.length && <span style={{color:"var(--warn)"}}> {questions.length-answered} unanswered question{questions.length-answered!==1?"s":""} will be marked wrong.</span>}
              <br/>You cannot change your answers after submission.
            </div>
            <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
              <button className="btn" onClick={()=>setShowConfirm(false)}>← Go Back</button>
              <button className="btn btn-danger" onClick={()=>doSubmit(false)}>✅ Yes, Submit Now</button>
            </div>
          </div>
        </div>
      )}
...
// ── Lecturer: exam creation & management panel ─────────────────────────
function LecturerExamManager({ toast, currentUser }) {
  const [exams, setExams] = useState(() => ls("nv-school-exams", []));
  ...
  // Exam form state
  const [form, setForm] = useState({ title:"", subject:"", classId:"", duration:60, passMark:50, instructions:"", shuffleQ:false, showResults:false });
  ...
  const saveExams = (updated) => { setExams(updated); saveShared("schoolExams", updated); };

  const loadResults = async (examId) => {
    setLoadingResults(true);
    try {
      const idx = await bsGet(`exam-results-index:${examId}`, true) || [];
      const enriched = await Promise.all(idx.map(async r => {
        const vKey = `exam-violations:${examId}:${r.student}`;
        const retakeKey = `exam-retake:${examId}:${r.student}`;
        const [viols, retake] = await Promise.all([
          bsGet(vKey, true).then(v => v || []),
          bsGet(retakeKey, true).then(v => v || { granted: false }),
        ]);
        return { ...r, violationDetails: viols, retakeGranted: retake.granted === true };
      }));
      setResults(enriched);
    } catch { setResults([]); }
  ...
  const startCreate = (examToEdit = null, idx = null) => {
    if (examToEdit) {
      setForm({ title:examToEdit.title, subject:examToEdit.subject||"", classId:examToEdit.classId||"", duration:examToEdit.duration||60, passMark:examToEdit.passMark||50, instructions:examToEdit.instructions||"", shuffleQ:examToEdit.shuffleQ||false, showResults:examToEdit.showResults||false });
      setQuestions([...(examToEdit.questions||[])]);
      setEditExamIdx(idx);
    } else {
      setForm({ title:"", subject:"", classId:"", duration:60, passMark:50, instructions:"", shuffleQ:false, showResults:false });
      setQuestions([]); setEditExamIdx(null);
    }
    setView("create"); setParsedQs([]); setPasteText(""); setPasteAnswers(""); setPasteMode(false); setEditQIdx(null);
  };

  const saveExam = () => {
    if (!form.title.trim()) return toast("Exam title required","error");
    if (!form.classId) return toast("Select a class","error");
    if (questions.length < 1) return toast("Add at least 1 question","error");
    const exam = { ...form, id: editExamIdx!==null ? exams[editExamIdx].id : Date.now(), questions, createdBy: currentUser, status:"draft", createdAt: new Date().toLocaleDateString(), publishedAt: null, resultsPublished: false };
    let updated;
    if (editExamIdx!==null) { updated = exams.map((e,i)=>i===editExamIdx?{...e,...exam}:e); toast("Exam updated","success"); }
    else { updated = [...exams, exam]; toast("Exam created! You can now open it for students.","success"); }
    saveExams(updated); setView("list"); setEditExamIdx(null);
  };

  const deleteExam = (id) => {
    if (!confirm("Delete this exam? All student results will be lost.")) return;
    saveExams(exams.filter(e=>e.id!==id)); if (selExam===id) setSelExam(null);
    toast("Exam deleted","success");
  };

  const openExam = (id) => {
    const updated = exams.map(e=>e.id===id?{...e,status:"open",openedAt:Date.now()}:e);
    saveExams(updated); toast("✅ Exam is now OPEN — students can start taking it","success");
  };

  const closeExam = (id) => {
    const updated = exams.map(e=>e.id===id?{...e,status:"closed"}:e);
    saveExams(updated); toast("Exam closed","warn");
  };

  const grantRetake = async (examId, studentEmail) => {
    const key = `exam-retake:${examId}:${studentEmail}`;
    await bsSet(key, { granted: true, grantedAt: new Date().toLocaleString(), grantedBy: currentUser }, true);
    toast(`✅ Retake granted to ${studentEmail.split("@")[0]}`, "success");
    // Reload results to reflect
    await loadResults(examId);
  };

  const revokeRetake = async (examId, studentEmail) => {
    const key = `exam-retake:${examId}:${studentEmail}`;
    await bsSet(key, { granted: false }, true);
    toast(`Retake revoked for ${studentEmail.split("@")[0]}`, "warn");
    await loadResults(examId);
  };

  const publishResults = async (exam) => {
    if (!publishMsg.trim()) return toast("Enter a message to publish with results","error");
    // Mark results as published
    const updated = exams.map(e=>e.id===exam.id?{...e,resultsPublished:true,resultsMsg:publishMsg,resultsPublishedAt:new Date().toLocaleString()}:e);
    saveExams(updated);
    // Send group message to the class
    const classObj = classes.find(c=>c.id===exam.classId);
    const className = classObj?.label || "your class";
    const msgBody = `📊 *Exam Results Published: ${exam.title}*\n${publishMsg}\nResults are now visible in your exam history. Check "My Exams" tab.`;
    const msgRecord = { id: Date.now(), from: currentUser, text: msgBody, time: new Date().toLocaleTimeString(), class: exam.classId, examId: exam.id, isExamResult: true };
    // Store as shared class message
    const classmsgsKey = `class-messages:${exam.classId}`;
    const existing = await bsGet(classmsgsKey, true) || [];
    await bsSet(classmsgsKey, [...existing, msgRecord], true);
    toast(`✅ Results published to ${className} group!`, "success");
    setPublishMsg(""); setSelExam(null);
  };
  ...
  const parsePaste = () => {
    ...
    const items = blocks.map((block,idx)=>{
      ...
      let q="",options=["","","",""],ans=0;
      lines.forEach(line=>{
        const ansM=line.match(/^(?:ANS|ANSWER|Ans)[.:)]\s*([A-Da-d])/i);
        if(ansM){ans="ABCD".indexOf(ansM[1].toUpperCase());if(ans<0)ans=0;return;}
        const m=line.match(/^([QqAaBbCcDd])[.):\s]\s*(.+)$/);
        if(m){
          const l2=m[1].toUpperCase();
  ...
  const importParsed = () => { setQuestions(q=>[...q,...parsedQs]); setParsedQs([]); setPasteText(""); setPasteAnswers(""); setPasteMode(false); toast(`${parsedQs.length} questions added!`,"success"); };
  const addQ = () => {
    if(!qForm.q.trim()) return toast("Question text required","error");
    if(!qForm.options[0]||!qForm.options[1]) return toast("Options A & B required","error");
    if(editQIdx!==null){ setQuestions(qs=>qs.map((q,i)=>i===editQIdx?{...qForm}:q)); setEditQIdx(null); toast("Question updated","success"); }
    else { setQuestions(qs=>[...qs,{...qForm,options:[...qForm.options]}]); toast("Question added","success"); }
    setQForm({q:"",options:["","","",""],ans:0});
  };
  const delQ = (i) => setQuestions(qs=>qs.filter((_,idx)=>idx!==i));
  const editQ = (i) => { setQForm({...questions[i],options:[...questions[i].options]}); setEditQIdx(i); setPasteMode(false); };

  if (view==="create") return (
    <div>
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:20,flexWrap:"wrap"}}>
        <button className="btn btn-sm" onClick={()=>setView("list")}>← Back</button>
        <div className="sec-title" style={{marginBottom:0}}>{editExamIdx!==null?"✏️ Edit Exam":"📝 Create New Exam"}</div>
      </div>

      {/* Exam settings */}
      <div className="card" style={{marginBottom:16}}>
        <div style={{fontWeight:800,fontSize:14,color:"var(--accent)",marginBottom:14}}>📋 Exam Settings</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          <div>
            <label className="lbl">Exam Title *</label>
            <input className="inp" style={{marginBottom:0}} placeholder="e.g. Mid-Semester Test — Pharmacology" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} />
          </div>
          <div>
            <label className="lbl">Subject / Course</label>
            <input className="inp" style={{marginBottom:0}} placeholder="e.g. Pharmacology" value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})} />
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
          <div>
            <label className="lbl">Class *</label>
            <select className="inp" style={{marginBottom:0}} value={form.classId} onChange={e=>setForm({...form,classId:e.target.value})}>
              <option value="">— Select class —</option>
              {classes.map(c=><option key={c.id} value={c.id}>{c.label} — {c.desc}</option>)}
            </select>
          </div>
          <div>
            ...
            <input className="inp" style={{marginBottom:0}} type="number" min="5" max="360" value={form.duration} onChange={e=>setForm({...form,duration:+e.target.value})} />
          </div>
          <div>
            <label className="lbl">Pass Mark (%)</label>
            <input className="inp" style={{marginBottom:0}} type="number" min="0" max="100" value={form.passMark} onChange={e=>setForm({...form,passMark:+e.target.value})} />
          </div>
        </div>
        <div style={{marginBottom:12}}>
          <label className="lbl">Instructions to Students (optional)</label>
          <textarea className="inp" rows={2} style={{resize:"vertical",marginBottom:0}} value={form.instructions} onChange={e=>setForm({...form,instructions:e.target.value})} placeholder="Any special instructions for students before they start..." />
        </div>
        <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}>
            <input type="checkbox" checked={form.shuffleQ} onChange={e=>setForm({...form,shuffleQ:e.target.checked})} style={{width:16,height:16}} />
            Shuffle question order per student
          </label>
          <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}>
            <input type="checkbox" checked={form.showResults} onChange={e=>setForm({...form,showResults:e.target.checked})} style={{width:16,height:16}} />
            Show score to student immediately after submission
          </label>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
          <div style={{fontWeight:800,fontSize:14,color:"var(--accent)"}}>❓ Questions ({questions.length})</div>
          <div style={{display:"flex",gap:8}}>
            <button className={`btn btn-sm${!pasteMode?" btn-accent":""}`} onClick={()=>{setPasteMode(false);setEditQIdx(null);}}>✏️ Single</button>
            <button className={`btn btn-sm${pasteMode?" btn-accent":""}`} onClick={()=>{setPasteMode(true);setEditQIdx(null);}}>📋 Paste</button>
          </div>
        </div>
        ...
        {!pasteMode&&(
          <div className="card2" style={{marginBottom:14}}>
            <div style={{fontWeight:800,fontSize:13,color:"var(--accent)",marginBottom:10}}>{editQIdx!==null?`✏️ Edit Q${editQIdx+1}`:"➕ New Question"}</div>
            <textarea className="inp" rows={3} style={{resize:"vertical"}} value={qForm.q} onChange={e=>setQForm({...qForm,q:e.target.value})} placeholder="Question text..." />
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              {["A","B","C","D"].map((l,i)=>(
                <div key={l}>
                  <label className="lbl">Option {l}{i<2?" *":""}</label>
                  <input className="inp" style={{marginBottom:0}} placeholder={`Option ${l}...`} value={qForm.options[i]} onChange={e=>{const o=[...qForm.options];o[i]=e.target.value;setQForm({...qForm,options:o});}} />
                </div>
              ))}
            </div>
            <label className="lbl">Correct Answer *</label>
            <select className="inp" value={qForm.ans} onChange={e=>setQForm({...qForm,ans:+e.target.value})}>
              {["A","B","C","D"].map((l,i)=><option key={l} value={i}>Option {l}{qForm.options[i]?`: ${qForm.options[i]}`:""}</option>)}
            </select>
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-accent" onClick={addQ}>{editQIdx!==null?"💾 Update":"➕ Add Question"}</button>
              {editQIdx!==null&&<button className="btn" onClick={()=>{setEditQIdx(null);setQForm({q:"",options:["","","",""],ans:0});}}>Cancel</button>}
            </div>
          </div>
        )}
        ...
        {pasteMode&&(
          <div className="card2" style={{marginBottom:14}}>
            <div style={{fontWeight:800,fontSize:13,color:"var(--accent)",marginBottom:8}}>📋 Paste Multiple Questions</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:8}}>
              <div>
                <div style={{fontSize:11,fontWeight:800,color:"var(--accent)",marginBottom:4}}>📝 Questions (with A/B/C/D options)</div>
                <textarea className="paste-box" rows={10} placeholder={"Q: Normal adult temperature is:\nA: 35.0°C\nB: 36.1–37.2°C\nC: 38.5°C\nD: 40.0°C\n\nQ: Which organ produces insulin?\nA: Liver\nB: Kidney\nC: Pancreas\nD: Spleen"} value={pasteText} onChange={e=>{setPasteText(e.target.value);setParsedQs([]);}} />
              </div>
              <div>
                <div style={{fontSize:11,fontWeight:800,color:"var(--success)",marginBottom:4}}>✅ Answers (one per line: A / B / C / D)</div>
                <textarea className="paste-box" rows={10} placeholder={"B\nC"} value={pasteAnswers} onChange={e=>{setPasteAnswers(e.target.value);setParsedQs([]);}} style={{borderColor:"rgba(34,197,94,.35)"}} />
              </div>
            </div>
            <div style={{display:"flex",gap:8,marginBottom:parsedQs.length?10:0}}>
              <button className="btn btn-accent" onClick={parsePaste}>🔍 Parse</button>
              {parsedQs.length>0&&<button className="btn btn-success" onClick={importParsed}>✅ Add {parsedQs.length} Questions</button>}
              <button className="btn" onClick={()=>{setPasteMode(false);setParsedQs([]);setPasteText("");setPasteAnswers("");}}>Cancel</button>
            </div>
            {parsedQs.length>0&&<div className="parse-preview">{parsedQs.map((p,i)=><div key={i} className="parse-item"><span className="parse-check">✓</span><span style={{flex:1}}>{p.q}</span><span style={{color:"var(--accent)"}}>ANS: {"ABCD"[p.ans]}</span></div>)}</div>}
          </div>
        )}

        {/* Questions list */}
        {questions.length===0&&<div style={{textAlign:"center",padding:20,color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:10}}>No questions yet. Add above.</div>}
        {questions.map((q,i)=>(
          <div key={i} className="card2" style={{marginBottom:8,borderLeft:`3px solid ${i===editQIdx?"var(--accent)":"var(--border)"}`}}>
            <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
              <div style={{width:24,height:24,borderRadius:7,background:"rgba(0,119,182,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"var(--accent)",flexShrink:0}}>{i+1}</div>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:13,marginBottom:5}}>{q.q}</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                  {q.options.filter(o=>o).map((opt,oi)=>(
                    <span key={oi} style={{fontSize:11,padding:"2px 8px",borderRadius:5,background:oi===q.ans?"rgba(34,197,94,.12)":"transparent",border:`1px solid ${oi===q.ans?"var(--success)":"var(--border)"}`,color:oi===q.ans?"var(--success)":"var(--text3)",fontWeight:oi===q.ans?800:400}}>{"ABCD"[oi]}. {opt}{oi===q.ans?" ✓":""}</span>
                  ))}
                </div>
              </div>
              <div style={{display:"flex",gap:4,flexShrink:0}}>
                <button className="btn btn-sm" onClick={()=>{editQ(i);setPasteMode(false);}}>✏️</button>
                <button className="btn btn-sm btn-danger" onClick={()=>delQ(i)}>🗑️</button>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",flexWrap:"wrap"}}>
        ...
        <button className="btn btn-accent" onClick={saveExam}>💾 {editExamIdx!==null?"Save Changes":"Create Exam"}</button>
      </div>
    </div>
  );

  if (view==="results" && selExam) {
    const exam = exams.find(e=>e.id===selExam);
    if (!exam) return null;
    const classObj = classes.find(c=>c.id===exam.classId);
    const students = allUsers.filter(u=>(u.role==="student"||!u.role)&&(u.class===exam.classId||!exam.classId));
    const avg = results.length ? Math.round(results.reduce((s,r)=>s+r.pct,0)/results.length) : 0;
    const passed = results.filter(r=>r.pct>=exam.passMark).length;
    return (
      <div>
        <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:20,flexWrap:"wrap"}}>
          <button className="btn btn-sm" onClick={()=>{setView("list");setSelExam(null);setResults([]);}}>← Back</button>
          <div>
            <div className="sec-title" style={{marginBottom:0}}>📊 {exam.title} — Results</div>
            <div style={{fontSize:12,color:"var(--text3)"}}>{classObj?.label} · {exam.subject} · Pass mark: {exam.passMark}%</div>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:20}}>
          {[
            {label:"Submitted",val:results.length,icon:"📥"},
            {label:"Average Score",val:`${avg}%`,icon:"📊"},
            {label:"Passed",val:passed,icon:"✅"},
            {label:"Failed",val:results.length-passed,icon:"❌"},
          ].map(s=>(
            <div key={s.label} className="card2" style={{textAlign:"center",padding:"14px 10px"}}>
              <div style={{fontSize:22,marginBottom:4}}>{s.icon}</div>
              <div style={{fontWeight:800,fontSize:20,color:"var(--accent)"}}>{s.val}</div>
        ...
        {/* Publish results */}
        {!exam.resultsPublished && results.length > 0 && (
          <div className="card" style={{marginBottom:16,border:"1px solid rgba(34,197,94,.3)"}}>
            <div style={{fontWeight:800,fontSize:14,color:"var(--success)",marginBottom:10}}>📢 Publish Results to Class</div>
            <textarea className="inp" rows={3} style={{resize:"vertical"}} value={publishMsg} onChange={e=>setPublishMsg(e.target.value)} placeholder={`e.g. "Well done everyone! Average was ${avg}%. Top score was ${results.length?Math.max(...results.map(r=>r.pct)):0}%. Certificates will be issued next week."`} />
            <button className="btn btn-success" onClick={()=>publishResults(exam)}>📢 Publish Results & Notify Class</button>
          </div>
        )}
        {exam.resultsPublished && (
          <div style={{background:"rgba(34,197,94,.08)",border:"1px solid rgba(34,197,94,.25)",borderRadius:12,padding:"12px 16px",marginBottom:16,fontSize:13,color:"var(--success)",fontWeight:800}}>
            ✅ Results published to {classObj?.label} — {exam.resultsPublishedAt}
          </div>
        )}

        {/* Results table */}
        {loadingResults ? (
          <div style={{textAlign:"center",padding:40,color:"var(--text3)"}}>Loading results...</div>
        ) : results.length===0 ? (
          <div style={{textAlign:"center",padding:40,color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:12}}>No submissions yet. Students will appear here once they take the exam.</div>
        ) : (
          <div className="card" style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead>
                <tr style={{borderBottom:"2px solid var(--border)"}}>
                  {["#","Student","Score","Percentage","Status","Violations","Time","Submitted","Retake"].map(h=>(
                    <th key={h} style={{padding:"10px 12px",textAlign:"left",fontWeight:800,color:"var(--text3)",fontSize:11,whiteSpace:"nowrap"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[...results].sort((a,b)=>b.pct-a.pct).map((r,i)=>(
                  <tr key={r.student} style={{borderBottom:"1px solid var(--border)",transition:"background .15s"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(0,119,182,.04)"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                    <td style={{padding:"10px 12px",fontFamily:"'DM Mono',monospace",color:"var(--text3)"}}>{i+1}</td>
                    <td style={{padding:"10px 12px",fontWeight:700}}>{r.student.split("@")[0]}<div style={{fontSize:10,color:"var(--text3)",fontWeight:400}}>{r.student}</div></td>
                    <td style={{padding:"10px 12px",fontFamily:"'DM Mono',monospace",fontWeight:800}}>{r.score}/{r.total}</td>
                    <td style={{padding:"10px 12px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{width:60,height:6,borderRadius:4,background:"var(--bg4)",overflow:"hidden"}}>
                          <div style={{width:`${r.pct}%`,height:"100%",background:r.pct>=exam.passMark?"var(--success)":"var(--danger)",transition:"width .4s"}} />
                        </div>
                        <span style={{fontFamily:"'DM Mono',monospace",fontWeight:800,color:r.pct>=exam.passMark?"var(--success)":"var(--danger)"}}>{r.pct}%</span>
                      </div>
                    </td>
                    <td style={{padding:"10px 12px"}}>
                      <span style={{background:r.pct>=exam.passMark?"rgba(34,197,94,.15)":"rgba(239,68,68,.12)",color:r.pct>=exam.passMark?"var(--success)":"var(--danger)",borderRadius:6,padding:"3px 8px",fontSize:11,fontWeight:800}}>
                        {r.pct>=exam.passMark?"PASS":"FAIL"}
                      </span>
                    </td>
                    <td style={{padding:"10px 12px"}}>
                      {r.violations>0 ? <span style={{color:"var(--warn)",fontWeight:800,fontSize:12}}>⚠️ {r.violations}</span> : <span style={{color:"var(--text3)",fontSize:12}}>—</span>}
                    </td>
                    <td style={{padding:"10px 12px",fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--text3)"}}>{r.timeTaken?`${Math.floor(r.timeTaken/60)}m ${r.timeTaken%60}s`:"—"}</td>
                    <td style={{padding:"10px 12px",fontSize:11,color:"var(--text3)"}}>{r.submittedAt}</td>
                    <td style={{padding:"10px 12px"}}>
                      {r.retakeGranted ? (
                        <div style={{display:"flex",flexDirection:"column",gap:3}}>
                          <span style={{fontSize:11,color:"var(--success)",fontWeight:800}}>✅ Granted</span>
                          <button className="btn btn-sm" style={{fontSize:10,padding:"2px 7px",color:"var(--danger)",borderColor:"var(--danger)"}} onClick={()=>revokeRetake(selExam,r.student)}>Revoke</button>
                        </div>
                      ) : (
                        <button className="btn btn-sm" style={{fontSize:11,whiteSpace:"nowrap"}} onClick={()=>grantRetake(selExam,r.student)}>🔄 Grant Retake</button>
                      )}
                    </td>
                  </tr>
                ))}
  ...
  // Default: exam list view
  const myExams = exams.filter(e=>e.createdBy===currentUser);
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <div>
          <div className="sec-title">📝 My Exams</div>
          <div style={{fontSize:12,color:"var(--text3)"}}>Create and manage mid-semester tests and exams for your students.</div>
        </div>
        <button className="btn btn-accent" onClick={()=>startCreate()}>+ Create Exam</button>
      </div>
      ...
      {myExams.length===0 && (
        <div style={{textAlign:"center",padding:"60px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:14}}>
          ...
          <div style={{fontSize:12,marginBottom:20}}>Create your first exam to get started</div>
          <button className="btn btn-accent" onClick={()=>startCreate()}>+ Create Your First Exam</button>
        </div>
      )}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        {myExams.map((exam,i)=>{
          const classObj = classes.find(c=>c.id===exam.classId);
          const statusColor = exam.status==="open" ? "var(--success)" : exam.status==="closed" ? "var(--text3)" : "var(--warn)";
          const statusIcon = exam.status==="open" ? "🟢" : exam.status==="closed" ? "🔒" : "📋";
          return (
            <div key={exam.id} className="card" style={{borderLeft:`4px solid ${statusColor}`,padding:0,overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",gap:14,padding:"16px 20px",flexWrap:"wrap"}}>
                <div style={{fontSize:24}}>{statusIcon}</div>
                <div style={{flex:1,minWidth:180}}>
                  <div style={{fontWeight:800,fontSize:15}}>{exam.title}</div>
                  <div style={{fontSize:12,color:"var(--text3)",marginTop:3}}>
                    {classObj?.label||"No class"} · {exam.subject||"No subject"} · {exam.questions?.length||0} questions · {exam.duration} mins · Pass: {exam.passMark}%
                  </div>
                  <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>Created {exam.createdAt}</div>
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {exam.status==="draft"&&<button className="btn btn-sm btn-success" onClick={()=>openExam(exam.id)}>🟢 Open for Students</button>}
                  {exam.status==="open"&&<button className="btn btn-sm btn-warn" onClick={()=>closeExam(exam.id)}>🔒 Close Exam</button>}
                  <button className="btn btn-sm btn-accent" onClick={async()=>{setSelExam(exam.id);setView("results");await loadResults(exam.id);}}>📊 Results</button>
                  {exam.status==="draft"&&<button className="btn btn-sm" onClick={()=>startCreate(exam,myExams.indexOf(exam))}>✏️ Edit</button>}
                  <button className="btn btn-sm btn-danger" onClick={()=>deleteExam(exam.id)}>🗑️</button>
                </div>
              </div>
              {exam.status==="open"&&(
                <div style={{padding:"8px 20px",background:"rgba(34,197,94,.07)",borderTop:"1px solid rgba(34,197,94,.15)",fontSize:12,color:"var(--success)",fontWeight:800}}>
                  ✅ LIVE — Students can take this exam now. Opened: {exam.openedAt ? new Date(exam.openedAt).toLocaleTimeString() : "Unknown"}
                </div>
              )}
              {exam.resultsPublished&&(
                <div style={{padding:"8px 20px",background:"rgba(0,119,182,.05)",borderTop:"1px solid var(--border)",fontSize:12,color:"var(--accent)"}}>
                  📢 Results published to class — {exam.resultsPublishedAt}
                </div>
              )}
            </div>
          );
...
// ── Student: see and take exams ────────────────────────────────────────
function StudentExamView({ toast, currentUser }) {
  const [allExams, setAllExams] = useState(() => ls("nv-school-exams", []));
  const [activeExam, setActiveExam] = useState(null); // exam object being taken
  const [examResult, setExamResult] = useState(null); // completed result
  const [tab, setTab] = useState("available"); // available | history
  const attKey = `nv-exam-att-${currentUser}`;
  const [attempts, setAttempts] = useState(() => ls(attKey, {}));
  const [retakeGrants, setRetakeGrants] = useState({}); // examId -> true if granted
  const allUsers = ls("nv-users", []);
  const userObj = allUsers.find(u=>u.username===currentUser);
  const userClass = userObj?.class || "";
  const classes = ls("nv-classes", DEFAULT_CLASSES);

  // Refresh exams from shared storage + check retake grants
  const refresh = async () => {
    const remote = await bsGet("db:school-exams", true);
    if (remote) { const parsed = typeof remote === "string" ? JSON.parse(remote) : remote; setAllExams(parsed); lsSet("nv-school-exams", parsed); }
    const att = await bsGet(uKey(currentUser, "exam-att"), false);
    if (att) { setAttempts(att); lsSet(attKey, att); }
    // Check retake grants for all completed exams
    const currentAttempts = att || ls(attKey, {});
    const completedIds = Object.keys(currentAttempts);
    if (completedIds.length > 0) {
      const grants = {};
      await Promise.all(completedIds.map(async examId => {
        const key = `exam-retake:${examId}:${currentUser}`;
        const retake = await bsGet(key, true);
        if (retake?.granted === true) grants[examId] = true;
      }));
  ...
  // Filter to exams for this student's class
  const myClassExams = allExams.filter(e => (!e.classId || e.classId===userClass) && (e.status==="open"||e.status==="closed"));
  const openExams = myClassExams.filter(e=>e.status==="open"&&(!attempts[e.id]||retakeGrants[e.id]));
  const completedExams = myClassExams.filter(e=>attempts[e.id]);
  const missedExams = myClassExams.filter(e=>e.status==="closed"&&!attempts[e.id]);

  const startExam = async (exam) => {
    if (attempts[exam.id] && !retakeGrants[exam.id]) return toast("You have already completed this exam","error");
    if (exam.status!=="open") return toast("This exam is not currently open","error");
    // Clear retake grant so it can't be used again
    if (retakeGrants[exam.id]) {
      await bsSet(`exam-retake:${exam.id}:${currentUser}`, { granted: false }, true);
      setRetakeGrants(g => { const next = {...g}; delete next[exam.id]; return next; });
    }
    setActiveExam(exam);
    setExamResult(null);
  };
  ...
  const handleSubmit = (result) => {
    ...
    setAttempts(a=>({...a,[result.examId]:{pct:result.pct,score:result.score,total:result.total,submittedAt:result.submittedAt}}));
    if (result.autoSubmitted) toast("Exam auto-submitted due to integrity violations","warn");
    else toast("Exam submitted successfully! 🎉","success");
    setTab("history");
  };
  ...
  useEffect(()=>{
    if (!userClass) return;
    bsGet(`class-messages:${userClass}`, true).then(msgs => { if (msgs) setClassMessages(msgs); });
  },[userClass]);

  if (activeExam) return (
    <div>
      <div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.25)",borderRadius:12,padding:"10px 16px",marginBottom:16,fontSize:12,color:"var(--danger)",fontWeight:800}}>
        🔒 EXAM IN PROGRESS — Do NOT switch tabs, minimise the window, or use browser tools. Violations are recorded.
      </div>
      <ExamTakingView exam={activeExam} currentUser={currentUser} onSubmit={handleSubmit} />
    </div>
  );
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        <div className="sec-title">📝 My Exams</div>
        <div style={{fontSize:12,color:"var(--text3)",marginBottom:14}}>Exams set by your lecturers. Each exam can only be taken once.</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[
            {key:"available",label:`Available (${openExams.length})`,icon:"📋"},
            {key:"history",label:`Completed (${completedExams.length})`,icon:"✅"},
            {key:"messages",label:`Announcements (${classMessages.filter(m=>m.isExamResult).length})`,icon:"📢"},
          ].map(t=>(
            <div key={t.key} onClick={()=>setTab(t.key)} style={{flex:"1 1 140px",padding:"10px 14px",borderRadius:10,cursor:"pointer",transition:"all .2s",textAlign:"center",border:`2px solid ${tab===t.key?"var(--accent)":"var(--border)"}`,background:tab===t.key?"rgba(0,119,182,.08)":"var(--card)"}}>
              <div style={{fontSize:18,marginBottom:2}}>{t.icon}</div>
              <div style={{fontSize:12,fontWeight:800,color:tab===t.key?"var(--accent)":"var(--text)"}}>{t.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {examResult && (
        <div style={{marginBottom:16,padding:"16px 20px",borderRadius:12,background: examResult.pct>=(allExams.find(e=>e.id===examResult.examId)?.passMark||50)?"rgba(34,197,94,.1)":"rgba(239,68,68,.08)",border:`1px solid ${examResult.pct>=(allExams.find(e=>e.id===examResult.examId)?.passMark||50)?"rgba(34,197,94,.3)":"rgba(239,68,68,.25)"}`}}>
          <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>
            {examResult.pct>=(allExams.find(e=>e.id===examResult.examId)?.passMark||50)?"🎉 Congratulations! You passed!":"📚 Keep studying — you can do better!"}
          </div>
          <div style={{fontSize:13}}><b>{examResult.examTitle}</b>: {examResult.score}/{examResult.total} ({examResult.pct}%)</div>
          {examResult.violations>0&&<div style={{fontSize:12,color:"var(--warn)",marginTop:4}}>⚠️ {examResult.violations} integrity violation{examResult.violations!==1?"s":""} were recorded.</div>}
        </div>
      )}

      {tab==="available" && (
        <div>
          {openExams.length===0&&missedExams.length===0&&(
            <div style={{textAlign:"center",padding:"48px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:12}}>
              <div style={{fontSize:44,marginBottom:10}}>📋</div>
              <div style={{fontWeight:700}}>No exams available right now</div>
              <div style={{fontSize:12,marginTop:6}}>Your lecturer will notify you when an exam is opened</div>
            </div>
          )}
          {openExams.map(exam=>{
            const classObj = classes.find(c=>c.id===exam.classId);
            return (
              <div key={exam.id} className="card" style={{marginBottom:12,borderLeft:`4px solid ${retakeGrants[exam.id]?"var(--warn)":"var(--success)"}`}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:14,flexWrap:"wrap"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                      <div style={{fontWeight:800,fontSize:15}}>📝 {exam.title}</div>
                      {retakeGrants[exam.id] && <span style={{background:"rgba(251,146,60,.15)",border:"1px solid var(--warn)",borderRadius:6,padding:"2px 8px",fontSize:11,color:"var(--warn)",fontWeight:800}}>🔄 Retake</span>}
                    </div>
                    <div style={{fontSize:12,color:"var(--text3)",marginBottom:8}}>
                      {classObj?.label} · {exam.subject||"General"} · {exam.questions?.length||0} questions · {exam.duration} minutes
                    </div>
                    {exam.instructions&&<div style={{fontSize:12,background:"rgba(0,119,182,.06)",borderRadius:8,padding:"8px 12px",border:"1px solid rgba(0,119,182,.15)",marginBottom:8,color:"var(--text2)"}}>📌 {exam.instructions}</div>}
                    <div style={{fontSize:11,color:"var(--warn)",fontWeight:800}}>⚠️ Once started, you cannot pause, switch tabs, or leave. Complete in one sitting.</div>
                  </div>
                  <button className="btn btn-success" style={{flexShrink:0}} onClick={()=>startExam(exam)}>Start Exam →</button>
                </div>
              </div>
            );
          })}
          {missedExams.length>0&&(
            <div style={{marginTop:16}}>
              <div style={{fontWeight:800,fontSize:13,color:"var(--text3)",marginBottom:8}}>🔒 Closed Exams (missed)</div>
              {missedExams.map(exam=>(
                <div key={exam.id} className="card2" style={{marginBottom:8,opacity:0.7}}>
                  <div style={{fontWeight:700}}>{exam.title}</div>
                  <div style={{fontSize:12,color:"var(--text3)"}}>{exam.subject} · Closed — you did not submit</div>
                </div>
              ))}
            </div>
          )}
      ...
      {tab==="history" && (
        <div>
          {completedExams.length===0&&<div style={{textAlign:"center",padding:"48px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:12}}><div style={{fontSize:44,marginBottom:10}}>✅</div><div style={{fontWeight:700}}>No completed exams yet</div></div>}
          {completedExams.map(exam=>{
            const att = attempts[exam.id];
            const passed = att?.pct >= exam.passMark;
            const published = exam.resultsPublished;
            return (
              <div key={exam.id} className="card" style={{marginBottom:12,borderLeft:`4px solid ${passed?"var(--success)":"var(--danger)"}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                      <div style={{fontWeight:800,fontSize:15}}>{exam.title}</div>
                      {retakeGrants[exam.id] && <span style={{background:"rgba(251,146,60,.15)",border:"1px solid var(--warn)",borderRadius:6,padding:"2px 8px",fontSize:11,color:"var(--warn)",fontWeight:800}}>🔄 Retake Available</span>}
                    </div>
                    <div style={{fontSize:12,color:"var(--text3)",marginBottom:6}}>{exam.subject} · Submitted {att?.submittedAt}</div>
                    {published ? (
                      <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontFamily:"'DM Mono',monospace",fontWeight:800,fontSize:18,color:passed?"var(--success)":"var(--danger)"}}>{att?.score}/{att?.total} ({att?.pct}%)</span>
                        <span style={{background:passed?"rgba(34,197,94,.15)":"rgba(239,68,68,.12)",color:passed?"var(--success)":"var(--danger)",borderRadius:6,padding:"3px 10px",fontSize:12,fontWeight:800}}>{passed?"PASS":"FAIL"}</span>
                      </div>
                    ) : (
                      <div style={{fontSize:12,color:"var(--text3)",fontStyle:"italic"}}>⏳ Results not yet published by lecturer</div>
                    )}
                  </div>
                  {published&&(
                    <div style={{textAlign:"center",padding:"10px 16px",background: passed?"rgba(34,197,94,.08)":"rgba(239,68,68,.06)",borderRadius:10,border:`1px solid ${passed?"rgba(34,197,94,.25)":"rgba(239,68,68,.2)"}`}}>
                      <div style={{fontFamily:"'DM Mono',monospace",fontWeight:800,fontSize:22,color:passed?"var(--success)":"var(--danger)"}}>{att?.pct}%</div>
                      <div style={{fontSize:10,color:"var(--text3)"}}>Your Score</div>
                    </div>
                  )}
                </div>
              </div>
      ...
      {tab==="messages" && (
        <div>
          {classMessages.filter(m=>m.isExamResult).length===0&&(
            <div style={{textAlign:"center",padding:"48px 20px",color:"var(--text3)",border:"1px dashed var(--border)",borderRadius:12}}>
              <div style={{fontSize:44,marginBottom:10}}>📢</div>
              <div style={{fontWeight:700}}>No announcements yet</div>
              <div style={{fontSize:12,marginTop:6}}>Results announcements from your lecturer will appear here</div>
            </div>
          )}
          {[...classMessages].filter(m=>m.isExamResult).reverse().map((msg,i)=>(
            <div key={i} className="card" style={{marginBottom:12,borderLeft:"4px solid var(--accent)"}}>
              <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginBottom:6}}>{msg.from?.split("@")[0]} · {msg.time}</div>
              <div style={{fontSize:13,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{msg.text}</div>
            </div>
          ))}
        </div>
      )}
...
// ── Main router: shows lecturer manager or student view ─────────────────
function SchoolExamsView({ toast, currentUser, isLecturer }) {
  return isLecturer
    ? <LecturerExamManager toast={toast} currentUser={currentUser} />
    : <StudentExamView toast={toast} currentUser={currentUser} />;
}
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k!==CACHE).map(k=>caches.delete(k))))));
  ...
  // ── Forgot Password via Claude AI email simulation ──
  const sendResetCode = async () => {
    ...
    _setResetCode(code);
    // Store code in backend so it works cross-device
    await bsSet(`reset:${forgotEmail.trim()}`, {code, expires: Date.now()+600000}, true);
    // Use Claude API to "send" the email (simulated — shows code in a toast since we can't send real email)
    try {
      await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          model:"claude-sonnet-4-20250514",max_tokens:100,
          messages:[{role:"user",content:`Generate a brief password reset notification message for ${forgotEmail} with code ${code} for Nursing Academic Hub. Just the body text, no subject.`}]
        })
      });
    } catch {}
    setForgotLoading(false);
    setForgotMode("code");
    toast(`📧 Reset code sent! Your code is: ${code} (valid 10 min)`, "success");
  };
  ...
  const verifyResetCode = async () => {
    ...
    setForgotLoading(true);
    // Check code from backend
    const stored = await bsGet(`reset:${forgotEmail.trim()}`);
    ...
    // Clear reset code
    await bsSet(`reset:${forgotEmail.trim()}`,null,true);
    setForgotLoading(false);
  ...
  const renderContent = () => {
    ...
    switch (activeNav) {
      ...
      case "nursingexams": return <NursingExamsStandaloneView toast={toast} currentUser={currentUser} />;
      case "schoolexams": return <SchoolExamsView toast={toast} currentUser={currentUser} isLecturer={isLecturer||isAdmin} />;
  ...
  const NAV = [
    ...
    { icon:"🎓", label:"Nursing Council Exams", key:"nursingexams" },
    ...(isLecturer||isAdmin ? [{ icon:"📝", label:"School Exams", key:"schoolexams" }] : [{ icon:"📝", label:"My Exams", key:"schoolexams" }]),
