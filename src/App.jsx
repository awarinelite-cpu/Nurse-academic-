...
// Check if backend storage is actually available
const storageAvailable = () => {
  try { return typeof window !== "undefined" && typeof window.storage === "object" && typeof window.storage.get === "function"; }
  catch { return false; }
};
...
const bsGet = async (key, shared = true) => {
  ...
  try {
    ...
    const v = r.value;
    // Handle both raw JSON strings and already-parsed values
    if (typeof v === "string") {
      try { return JSON.parse(v); } catch { return v; }
    }
    return v;
  } catch (e) {
    console.warn("[bsGet] failed for key:", key, e);
    return null;
  }
};
...
const bsSet = async (key, val, shared = true) => {
  if (!storageAvailable()) {
    console.warn("[bsSet] window.storage not available");
    return false;
  }
  try {
    const serialized = JSON.stringify(val);
    // Warn if value is very large (approaching 5MB limit)
    if (serialized.length > 3 * 1024 * 1024) {
      console.warn(`[bsSet] Large value for key "${key}": ${(serialized.length/1024/1024).toFixed(1)}MB`);
    }
    await window.storage.set(key, serialized, shared);
    return true;
  } catch (e) {
    console.error("[bsSet] FAILED for key:", key, "Error:", e?.message || e);
    return false;
  }
};
...
// Hook: subscribe to backend sync and return live data from localStorage.
...
// Dual-write: localStorage first (instant UI) + backend async
const dbSet = async (lsKey, bsKey, val) => {
  lsSet(lsKey, val);
  dispatchSync(); // Update UI immediately
  const ok = await bsSet(bsKey, val, true);
  if (!ok) console.error(`[dbSet] Backend write failed for ${bsKey}`);
  return ok;
};
...
const dbLoad = async (lsKey, bsKey, fallback, shared = true) => {
  try {
    const remote = await bsGet(bsKey, shared);
    if (remote !== null && remote !== undefined) {
      lsSet(lsKey, remote);
      return remote;
    }
  } catch (e) {
    console.warn("[dbLoad] failed for", bsKey, e);
  }
...
// saveShared: returns a Promise<boolean> - true if backend write succeeded
...
// Storage health check - call once on startup
let _storageHealthy = null; // null=unchecked, true=ok, false=broken
const checkStorageHealth = async () => {
  if (!storageAvailable()) {
    _storageHealthy = false;
    console.error("[SYNC] window.storage is NOT available — cross-device sync disabled");
    return false;
  }
  try {
    const testKey = "nv-health-check";
    await window.storage.set(testKey, "ok", true);
    const r = await window.storage.get(testKey, true);
    _storageHealthy = r?.value === "ok";
    if (!_storageHealthy) console.error("[SYNC] Storage health check FAILED — read-back mismatch");
    else console.log("[SYNC] Storage health check PASSED ✅");
    return _storageHealthy;
  } catch (e) {
    _storageHealthy = false;
    console.error("[SYNC] Storage health check FAILED:", e);
    return false;
  }
};

// Hydrate all shared data from backend (called once on app mount).
// After all keys are fetched and written to localStorage, fires the
// reactive sync event so every mounted component re-reads fresh data.
const hydrateFromBackend = async () => {
  if (_storageHealthy === null) await checkStorageHealth();
  if (!_storageHealthy) {
    console.warn("[SYNC] Skipping hydration — storage not available");
    return;
  }
  const defaults = {
  ...
  // Notify all subscribed components that fresh data is in localStorage
  dispatchSync();
};
...
function HandoutViewModal({ item, onClose }) {
  ...
  useEffect(() => {
    if (!pdfSrc && item.hasPdf && item.pdfKey) {
      setLoading(true);
      bsGet(item.pdfKey, true).then(data => {
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const save = async () => {
    ...
    if (form.uploadType==="pdf" && pdfFile) {
      const pdfOk = await bsSet(`handout-pdf:${itemId}`, pdfFile, true);
      if (!pdfOk) {
...
function AdminNursingExams({ toast }) {
  const [data, setData] = useSharedData("nv-nursing-exams", {general:[], midwifery:[], publichealth:[]});
...
function LecturerExamManager({ toast, currentUser }) {
  const [exams, setExams] = useSharedData("nv-school-exams", []);
  ...
  const saveExams = async (updated) => {
    setExams(updated);
    const ok = await saveShared("schoolExams", updated);
    if (!ok) toast("⚠️ Exam saved locally but failed to sync to server — check connection","warn");
    return ok;
  };
  ...
  const saveExam = async () => {
    ...
    let updated;
    if (editExamIdx!==null) { updated = exams.map((e,i)=>i===editExamIdx?{...e,...exam}:e); }
    else { updated = [...exams, exam]; }
    const ok = await saveExams(updated);
    if (ok) {
      toast(editExamIdx!==null ? "✅ Exam updated & synced!" : "✅ Exam created & synced! You can now open it for students.","success");
    }
    setView("list"); setEditExamIdx(null);
  };
  ...
  const openExam = async (id) => {
    ...
    const ok = await saveExams(updated);
    if (ok) toast("✅ Exam is now OPEN — students can start taking it","success");
  };

  const closeExam = async (id) => {
    const updated = exams.map(e=>e.id===id?{...e,status:"closed"}:e);
    const ok = await saveExams(updated);
    if (ok) toast("Exam closed","warn");
  };
...
function StudentExamView({ toast, currentUser }) {
  const [allExams, setAllExams] = useSharedData("nv-school-exams", []);
  ...
  const refresh = async () => {
    ...
    if (remote) { const parsed = typeof remote === "string" ? JSON.parse(remote) : remote; lsSet("nv-school-exams", parsed); setAllExams(parsed); dispatchSync(); }
...
export default function App() {
  ...
  const runSync = async () => {
    ...
    if (healthy) await hydrateFromBackend();
    setSyncing(false);
    return healthy;
  };
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k!==CACHE).map(k=>caches.delete(k))))));
  ...
  const verifyResetCode = async () => {
    ...
    try { await window.storage.delete(`reset:${forgotEmail.trim()}`, true); } catch {}
    setForgotLoading(false);
        ...
        <div className="main-area">
          <div className="topbar">
            ...
            <div className="topbar-right">
              ...
              <div className="icon-btn" title={syncError?"⚠️ Sync unavailable — tap to retry":"Sync data from server"}
                onClick={()=>{ if(!syncing) runSync().then(ok=>ok?toast("✅ Data synced!","success"):toast("❌ Sync failed — check your connection","error")); }}
