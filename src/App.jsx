      ...
      {selPaper&&(()=>{
        ...
        return (
          <div className="card" style={{borderTop:`4px solid ${meta.color}`,animation:"fadeUp .3s ease"}}>
            ...
            {selPaper==="osce"&&(
              <button className="btn btn-accent" style={{width:"100%",background:`linear-gradient(135deg,${meta.color},${meta.color}bb)`,border:"none",fontWeight:800,fontSize:14,padding:"13px"}}
...
// ── Daily Mock Exam (generates fresh 20-question mock daily) ─────────────
const DAILY_MOCK_QUESTIONS = [
  // General Nursing
  {q:"The normal adult respiratory rate is:", options:["8-12 breaths/min","12-20 breaths/min","20-28 breaths/min","28-36 breaths/min"], ans:1, cat:"General"},
  {q:"Which position is used for a patient in shock?", options:["Fowler's","Trendelenburg","Lithotomy","Sims"], ans:1, cat:"General"},
  {q:"The antidote for heparin overdose is:", options:["Vitamin K","Protamine sulfate","Naloxone","Flumazenil"], ans:1, cat:"Pharmacology"},
  {q:"Normal urine output per hour in an adult is:", options:["10-20 mL/hr","30-60 mL/hr","80-100 mL/hr","100-150 mL/hr"], ans:1, cat:"General"},
  {q:"Which cranial nerve controls eye movement (medial rectus)?", options:["CN III","CN IV","CN VI","CN VII"], ans:0, cat:"Anatomy"},
  {q:"The pH of normal blood is:", options:["7.15-7.25","7.25-7.35","7.35-7.45","7.45-7.55"], ans:2, cat:"Physiology"},
  {q:"Insulin is produced by which cells of the pancreas?", options:["Alpha cells","Beta cells","Delta cells","Acinar cells"], ans:1, cat:"Physiology"},
  {q:"Which drug is first-line for anaphylaxis?", options:["Hydrocortisone","Chlorpheniramine","Adrenaline (Epinephrine)","Salbutamol"], ans:2, cat:"Pharmacology"},
  {q:"The normal Glasgow Coma Scale (GCS) score is:", options:["10","12","13","15"], ans:3, cat:"General"},
  {q:"Which vitamin is essential for blood clotting?", options:["Vitamin A","Vitamin B12","Vitamin C","Vitamin K"], ans:3, cat:"Physiology"},
  {q:"Cheyne-Stokes breathing is characterised by:", options:["Rapid deep breathing","Slow shallow breathing","Alternating apnea and hyperpnea","Prolonged expiration"], ans:2, cat:"General"},
  {q:"The term 'nosocomial infection' refers to:", options:["Infection acquired in hospital","Infection acquired in community","Infection from insects","Congenital infection"], ans:0, cat:"General"},
  {q:"Which solution is isotonic?", options:["0.45% NaCl","0.9% NaCl","3% NaCl","5% NaCl"], ans:1, cat:"Pharmacology"},
  {q:"Apgar score is assessed at:", options:["1 minute only","1 and 5 minutes","5 and 10 minutes","Birth only"], ans:1, cat:"Midwifery"},
  {q:"Normal CVP range is:", options:["0-2 cmH2O","2-8 cmH2O","8-12 cmH2O","12-18 cmH2O"], ans:1, cat:"General"},
  {q:"The hepatitis B vaccine is given at birth and at:", options:["2 and 6 weeks","6 and 14 weeks","6 weeks and 6 months","3 and 6 months"], ans:1, cat:"Public Health"},
  {q:"Which position is best for a patient post-tonsillectomy?", options:["Supine","Prone","Lateral (sims) position","Trendelenburg"], ans:2, cat:"General"},
  {q:"The APTT test monitors which anticoagulant therapy?", options:["Warfarin","Aspirin","Heparin","Clopidogrel"], ans:2, cat:"Pharmacology"},
  {q:"Oliguria is defined as urine output less than:", options:["200 mL/day","400 mL/day","500 mL/day","600 mL/day"], ans:1, cat:"General"},
  {q:"Which stage of labour involves placental delivery?", options:["First stage","Second stage","Third stage","Fourth stage"], ans:2, cat:"Midwifery"},
  // Additional questions pool
  {q:"A patient with COPD is at risk of oxygen-induced hypercapnia. Target SpO2 is:", options:["88-92%","94-98%","98-100%","85-88%"], ans:0, cat:"General"},
  {q:"The normal serum potassium level is:", options:["2.5-3.5 mEq/L","3.5-5.0 mEq/L","5.0-6.5 mEq/L","6.5-8.0 mEq/L"], ans:1, cat:"Physiology"},
  {q:"Kernig's sign is associated with:", options:["Appendicitis","Meningitis","Pancreatitis","Peritonitis"], ans:1, cat:"General"},
  {q:"Which infection control precaution is used for TB?", options:["Contact","Droplet","Airborne","Standard only"], ans:2, cat:"Public Health"},
  {q:"Lochia rubra lasts for approximately:", options:["1-2 days","3-4 days","5-6 days","7-10 days"], ans:1, cat:"Midwifery"},
  {q:"The drug of choice for status epilepticus is:", options:["Phenytoin IV","Diazepam IV","Phenobarbitone IM","Carbamazepine oral"], ans:1, cat:"Pharmacology"},
  {q:"Tachycardia in adults is defined as heart rate above:", options:["80 bpm","90 bpm","100 bpm","110 bpm"], ans:2, cat:"General"},
  {q:"Which hormone stimulates uterine contractions?", options:["Prolactin","Oestrogen","Oxytocin","Progesterone"], ans:2, cat:"Physiology"},
  {q:"The five rights of drug administration include all EXCEPT:", options:["Right patient","Right drug","Right route","Right diagnosis"], ans:3, cat:"Pharmacology"},
  {q:"Pulse oximetry measures:", options:["PaO2","SaO2","SpO2","FiO2"], ans:2, cat:"General"},
];

function getDailyMockQuestions() {
  // Use date as seed to get consistent 20 questions per day
  const today = new Date();
  const seed = today.getFullYear()*10000 + (today.getMonth()+1)*100 + today.getDate();
  const shuffled = [...DAILY_MOCK_QUESTIONS].sort((a,b)=>{
    const ha = (seed * (DAILY_MOCK_QUESTIONS.indexOf(a)+1)) % 997;
    const hb = (seed * (DAILY_MOCK_QUESTIONS.indexOf(b)+1)) % 997;
    return ha - hb;
  });
...
const NC_EXAM_META = {
  general:    { key:"general",    label:"General Nursing (GNC)",          short:"GNC",         icon:"🏥", color:"#3a5f25", desc:"General Nursing Council exam — Med-Surg, Fundamentals, Pharmacology" },
  midwifery:  { key:"midwifery",  label:"Midwifery Council Exam",         short:"Midwifery",   icon:"👶", color:"#5a4a8c", desc:"Maternal and newborn care, labour & delivery, antenatal/postnatal." },
  publichealth:{ key:"publichealth",label:"Public Health Nursing Exam",   short:"Public Health",icon:"🌍", color:"#1e5f74", desc:"Epidemiology, disease surveillance, health promotion, immunisation." },
  paediatric: { key:"paediatric", label:"Paediatric Nursing Exam",        short:"Paediatrics", icon:"🧒", color:"#c05621", desc:"Child health, growth & development, paediatric pharmacology, IMCI." },
  psychiatric:{ key:"psychiatric",label:"Psychiatric Nursing Exam",       short:"Psychiatric", icon:"🧠", color:"#6b21a8", desc:"Mental health nursing, psychiatric disorders, therapeutic communication." },
  critical:   { key:"critical",   label:"Critical Care Nursing Exam",     short:"Critical Care",icon:"💓", color:"#991b1b", desc:"ICU nursing, haemodynamics, ventilator management, ACLS protocols." },
};

function NcDailyMockExam({ toast, currentUser, onBack }) {
  const [phase, setPhase] = useState("intro"); // intro | exam | result
  const [questions] = useState(getDailyMockQuestions);
  ...
  if (phase==="intro") return (
    <div style={{maxWidth:600,margin:"0 auto"}}>
      <div className="nc-card" style={{textAlign:"center",padding:"32px 28px"}}>
        ...
        <div style={{fontSize:13,color:"#6b8a52",marginBottom:20}}>{today} · 20 Questions · Mixed Specialties</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:24}}>
          {[{icon:"❓",label:"20 Questions"},{icon:"⏱","label":"No time limit"},{icon:"📊",label:"Score tracked"}].map((s,i)=>(
        ...
        <div style={{background:"rgba(74,122,46,.07)",borderRadius:10,padding:"14px 16px",marginBottom:22,textAlign:"left"}}>
          <div style={{fontWeight:800,fontSize:13,color:"#2d4a1e",marginBottom:8}}>Topics covered today:</div>
          {["General Nursing","Pharmacology","Physiology","Midwifery","Public Health"].map(t=>(
            <div key={t} style={{fontSize:12,color:"#4a7a2e",marginBottom:4}}>✅ {t}</div>
          ))}
        </div>
        <div style={{display:"flex",gap:10}}>
          <button className="nc-btn" style={{flex:1}} onClick={onBack}>← Back</button>
          <button className="nc-btn nc-btn-primary" style={{flex:2,fontSize:15}} onClick={()=>setPhase("exam")}>▶ Start Daily Mock</button>
        </div>
      </div>
    </div>
  );
...
function NcSpecialtyExams({ toast, currentUser }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:12,marginBottom:20}}>
        {Object.values(NC_EXAM_META).map(m=>(
...
function NursingCouncilSite({ currentUser, isAdmin, onSwitchToSchool, toast, themeMode, setThemeMode }) {
  ...
  const renderContent = () => {
    switch(activeNav) {
      ...
      case "daily": return <NcDailyMockExam toast={toast} currentUser={currentUser} onBack={()=>setActiveNav("dashboard")} />;
      case "specialty": return <NcSpecialtyExams toast={toast} currentUser={currentUser} />;
    ...
    <>
      ...
      <div className="nc-shell">
        ...
        <div className={`nc-sidebar${sidebarOpen?" open":""}`}>
          ...
          {isAdmin&&(
            <>
              ...
              <div className="nc-nav-item" onClick={()=>{setActiveNav("admin");setSidebarOpen(false);}}>
        ...
        <div className="nc-main-area">
          ...
          <div className="nc-page-content">{activeNav==="admin"?<AdminNursingExams toast={toast} />:renderContent()}</div>
        </div>
      </div>
      <Toasts list={[]} />
    </>
