...
function Dashboard({ user, onNavigate }) {
  ...
  return (
    <div>
      ...
      {/* ── Nursing Council Exam Quick-Access ── */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4,flexWrap:"wrap",gap:8}}>
        <div className="sec-title" style={{marginBottom:0}}>🎓 Nursing Council Exams</div>
        <span onClick={()=>onNavigate("nursingexams")} style={{fontSize:12,color:"var(--accent)",cursor:"pointer",fontWeight:600}}>View all →</span>
      </div>
      <div style={{fontSize:12,color:"var(--text3)",marginBottom:14}}>Tap an exam to start practising past papers</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12,marginBottom:28}}>
        {Object.values(NURSING_EXAM_META).map((m,i)=>(
          <div key={m.key}
            onClick={()=>onNavigate("nursingexams", null, m.key)}
            style={{
              cursor:"pointer",borderRadius:16,overflow:"hidden",
              border:`2px solid ${m.color}35`,
              background:`var(--card)`,
              boxShadow:`0 2px 12px ${m.color}12`,
              transition:"transform .18s, box-shadow .18s",
              animationDelay:`${i*.07}s`,
            }}
            onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow=`0 12px 32px ${m.color}30`;}}
            onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow=`0 2px 12px ${m.color}12`;}}
          >
            {/* Coloured top banner */}
            <div style={{background:`linear-gradient(135deg,${m.color},${m.color}cc)`,padding:"18px 16px 14px",textAlign:"center"}}>
              <div style={{fontSize:40,marginBottom:4,filter:"drop-shadow(0 2px 4px rgba(0,0,0,.2))"}}>{m.icon}</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:15,color:"white",lineHeight:1.2}}>{m.short}</div>
            </div>
            {/* Body */}
            <div style={{padding:"12px 14px 16px"}}>
              <div style={{fontSize:11,color:"var(--text3)",lineHeight:1.6,marginBottom:12,minHeight:48}}>{m.desc}</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{fontSize:11,fontWeight:700,color:m.color,background:`${m.color}15`,padding:"4px 10px",borderRadius:20,border:`1px solid ${m.color}30`}}>
                  Start Practising
                </span>
...
function NursingExamsView({ toast, currentUser, initialExam }) {
  const [data, setData] = useSharedData("nv-nursing-exams", {general:[], midwifery:[], publichealth:[]});
  const [activeExam, setActiveExam] = useState(initialExam||"general");
