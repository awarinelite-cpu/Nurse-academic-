...
:root{
  --bg:#1a2e08;--bg2:#16280a;--bg3:#11200a;--bg4:#0d1a07;
  --card:#1e3a0c;--card2:#243f10;
  --accent:#A4C639;--accent2:#6B8E23;--accent3:#c8dc7a;
  --warn:#fb923c;--danger:#f87171;--success:#4ade80;--purple:#a78bfa;
  --border:rgba(255,255,255,0.09);--border2:rgba(255,255,255,0.18);
  --text:#eef5d8;--text2:#b8cc80;--text3:#6a8a3a;
  --radius:14px;--radius2:10px;
  --admin:#7c3aed;--admin2:#6d28d9;
}
body{font-family:'Instrument Sans',sans-serif;background:linear-gradient(180deg,#A4C639 0%,#6B8E23 100%) fixed;min-height:100vh;overflow-x:hidden;color:var(--text);}
body.light{
  --bg:#f0f7d4;--bg2:#e4f0b8;--bg3:#d4e89a;--bg4:#c4dc7c;
  --card:#e8f5c0;--card2:#dced9e;
  --border:rgba(60,90,0,0.15);--border2:rgba(60,90,0,0.28);
  --text:#1a2e08;--text2:#3a5a10;--text3:#6a8a3a;
}
...
/* AUTH */
.auth-page{min-height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(ellipse at 30% 20%,rgba(164,198,57,.25),transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(107,142,35,.2),transparent 50%),linear-gradient(180deg,#A4C639 0%,#6B8E23 100%);padding:20px;}
.auth-card{background:rgba(17,32,10,0.88);backdrop-filter:blur(16px);border:1px solid var(--border2);border-radius:22px;padding:38px 34px;width:100%;max-width:420px;animation:fadeUp .5s ease;box-shadow:0 40px 80px rgba(0,0,0,.4);}
...
.sidebar{width:240px;min-width:240px;background:rgba(13,26,7,0.82);backdrop-filter:blur(12px);border-right:1px solid var(--border);display:flex;flex-direction:column;overflow-y:auto;padding:0 0 20px;z-index:10;transition:transform .3s;}
...
.topbar{padding:13px 22px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border);background:rgba(13,26,7,0.75);backdrop-filter:blur(10px);flex-shrink:0;gap:10px;}
...
/* CARDS / COMMON */
.card{background:rgba(30,58,12,0.80);backdrop-filter:blur(8px);border:1px solid var(--border);border-radius:var(--radius);padding:18px;}
.card2{background:rgba(36,63,16,0.75);backdrop-filter:blur(6px);border:1px solid var(--border);border-radius:var(--radius2);padding:14px;}
.grid2{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;}
.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;}
.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}
.grid5{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;}
.stat-card{background:rgba(30,58,12,0.80);backdrop-filter:blur(8px);border:1px solid var(--border);border-radius:var(--radius);padding:16px;animation:fadeUp .4s ease both;}
...
.modal{background:rgba(17,32,10,0.94);backdrop-filter:blur(16px);border:1px solid var(--border2);border-radius:18px;padding:26px;width:100%;max-width:540px;max-height:88vh;overflow-y:auto;animation:fadeUp .3s ease;}
...
/* TT */
.tt-badge{display:inline-block;padding:3px 9px;border-radius:6px;font-size:11px;font-family:'DM Mono',monospace;font-weight:600;}.fc-lbl{font-family:'DM Mono',monospace;font-size:9px;color:var(--text3);margin-bottom:8px;text-transform:uppercase;letter-spacing:.08em;}
    ...
    <>
      ...
      <div className="app-shell">
        ...
        <div className={`sidebar${sidebarOpen?" open":""}`}>
          ...
          {[
            { key:"bnsc", label:"BNSc", ids:["bnsc1","bnsc2","bnsc3","bnsc4","bnscf"] },
            { key:"ndhnd", label:"ND / HND", ids:["nd1","nd2","hnd1","hnd2"] },
            { key:"cn", label:"Community Nursing", ids:["cn1","cn2"] },
          ].map(group => {
            const groupClasses = classes.filter(c => group.ids.includes(c.id));
            const isOpen = openGroup === group.key;
            return (
              <div key={group.key}>
                <div className="nav-item" style={{justifyContent:"space-between"}} onClick={()=>setOpenGroup(isOpen ? null : group.key)}>
                  <span style={{display:"flex",alignItems:"center",gap:9}}>
                    <span className="nav-icon">🎓</span>{group.label}
                  </span>
                  <span style={{fontSize:10,color:"var(--text3)",transition:"transform .2s",transform:isOpen?"rotate(180deg)":"rotate(0deg)"}}>▾</span>
                </div>
                {isOpen && groupClasses.map(c=>(
                  <div key={c.id} className="nav-item" style={{paddingLeft:32}} onClick={()=>navigate("handouts",c)}>
