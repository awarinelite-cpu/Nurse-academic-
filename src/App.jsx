...
const _cache = {};
const _cacheTime = {};
const CACHE_TTL = 4000; // 4 seconds — expires before next 5–10s sync cycle
...
function StudentExamView({ toast, currentUser }) {
  ...
  useEffect(() => {
    refresh();
    // Auto-synchronize school exams every 5–10 seconds across devices
    const EXAM_SYNC_MS = 5000 + Math.floor(Math.random() * 5000);
    let examSyncTimer;
    const scheduleExamSync = () => {
      examSyncTimer = setTimeout(async () => { await refresh(); scheduleExamSync(); }, EXAM_SYNC_MS);
    };
    scheduleExamSync();
    return () => clearTimeout(examSyncTimer);
  }, []);
...
export default function App() {
  ...
  useEffect(() => {
    initData(); runSync();
    // Auto-sync every 5–10 seconds while app is open (random to stagger devices)
    const SYNC_MS = 5000 + Math.floor(Math.random() * 5000); // 5000–9999 ms
    let syncTimer;
    const schedulSync = () => {
      syncTimer = setTimeout(async () => {
        await hydrateFromBackend();
        schedulSync(); // reschedule after each sync
      }, SYNC_MS);
    };
    schedulSync();
    // Also sync immediately when tab regains focus (user switches back from another device)
