...
const _cache = {};
const _cacheTime = {};
const CACHE_TTL = 4000; // 4 s — always stale before next 5-10s sync tick
...
// schoolExams list lives in _DOC_SHARED (written by saveShared/bsSet).
// All other per-exam sub-keys (results, violations, retakes, pdfs) live in _DOC_EXAMS.
const examBsGet = async (key) => {
  if (key === "schoolExams") {
    // Lecturers write this via saveShared("schoolExams") which calls bsSet -> _DOC_SHARED
    const doc = await _getDoc(_DOC_SHARED);
    return doc ? (doc["schoolExams"] ?? null) : null;
  }
...
const hydrateFromBackend = async () => {
  ...
  try {
    ...
    // Bust the shared-doc cache so the next examBsGet("schoolExams") re-fetches
    delete _cache[_DOC_SHARED];
    dispatchSync();
...
function LecturerExamManager({ toast, currentUser }) {
  ...
  const saveExams = async (updated) => {
    setExams(updated);
    // Bust shared-doc cache so the write is immediately visible on next read
    delete _cache[_DOC_SHARED];
    ...
    else toast("✅ Exam synced to all devices","success");
    return ok;
  };
  ...
  const publishResults = async (exam) => {
    ...
    const existing = await examBsGet(classmsgsKey) || [];
    await examBsSet(classmsgsKey, [...existing, msgRecord]);
...
function StudentExamView({ toast, currentUser }) {
  ...
  const refresh = async () => {
    const remote = await examBsGet("schoolExams");
  ...
  useEffect(() => {
    refresh();
    // Auto-synchronize school exams every 5-10 s across devices
    let examSyncTimer;
    const scheduleExamSync = () => {
      const delay = 5000 + Math.floor(Math.random() * 5000);
      examSyncTimer = setTimeout(async () => { await refresh(); scheduleExamSync(); }, delay);
    };
    scheduleExamSync();
    // Also refresh immediately on focus so opening the tab shows latest exams instantly
    const onFocus = () => { clearTimeout(examSyncTimer); refresh().then(scheduleExamSync); };
    window.addEventListener("focus", onFocus);
    return () => { clearTimeout(examSyncTimer); window.removeEventListener("focus", onFocus); };
  }, []);
  ...
  useEffect(()=>{
    if (!userClass) return;
    examBsGet(`class-messages:${userClass}`).then(msgs => { if (msgs) setClassMessages(msgs); });
  },[userClass]);
...
export default function App() {
  ...
  useEffect(() => {
    initData(); runSync();
    // Auto-sync every 5-10 s (randomised to stagger devices hitting Firestore)
    let syncTimer;
    const scheduleSync = () => {
      const delay = 5000 + Math.floor(Math.random() * 5000);
      syncTimer = setTimeout(async () => { await hydrateFromBackend(); scheduleSync(); }, delay);
    };
    scheduleSync();
    // Sync immediately when tab regains focus
    const onFocus = () => { clearTimeout(syncTimer); hydrateFromBackend().then(scheduleSync); };
    window.addEventListener("focus", onFocus);
    return () => { clearTimeout(syncTimer); window.removeEventListener("focus", onFocus); };
  }, []);
