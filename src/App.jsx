...
const SHARED_DEFAULTS = {
  ...
  handouts: [], essayBanks: [], classExams: [], messages: [], nursingExams: [],
};
...
function Handouts({ selectedClass, toast, currentUser, isLecturer }) {
  ...
  const pushNotification = async (item) => {
    const notifs = await bsGet("db:notifications", true) || ls("nv-notifications", []);
    const newNotif = {
    ...
    await bsSet("db:notifications", updatedNotifs, true);
...
function NursingExamCBT({ exam, currentUser, onBack, toast }) {
  ...
  const submit = () => {
    ...
    setAtt(newAtt);
    const results = ls("nv-results",[]);
    saveMyData("results","nv-results",[...results,{id:Date.now(),subject:exam.title,type:"Nursing CBT",score,total:exam.questions.length,pct,date:new Date().toLocaleDateString()}]);
    notifyUserKey("nv-results");
...
function NursingGroupChat({ category, currentUser, toast }) {
  ...
  const [msgs, setMsgs] = useState(()=>ls(lsKey,[]));
...
function Notifications({ currentUser, onRead }) {
  ...
  useEffect(() => {
    ...
    // Also listen for local pushes (from saveShared / notifyKey)
  ...
  useEffect(() => {
    ...
    // Mark all as read - save back to shared backend storage
...
export default function App() {
  ...
  const login = async () => {
    ...
    bsGet("db:notifications", true).then(sharedNotifs => {
      const list = Array.isArray(sharedNotifs) ? sharedNotifs : [];
