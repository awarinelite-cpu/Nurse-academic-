...
function AdminNursingExams({ toast }) {
  ...
  const importParsed = (pid) => {
    ...
    saveData(nd); setParsed([]); setPasteText(""); toast(`${parsed.length} questions added!`,"success");
  };
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:22,flexWrap:"wrap"}}>
        {examKeys.map(key=>{
          ...
          return (
            <div key={key} onClick={()=>{setActiveExam(key);setEditPaper(null);setParsed([]);setPasteText("");}}
                ...
                <div style={{padding:"0 18px 20px",borderTop:"1px solid var(--border)"}}>
                  ...
                  {addMode==="paste"&&(
                    <div className="card2" style={{marginBottom:14,border:`1px solid ${meta.color}30`}}>
                      ...
                      <div style={{fontSize:11,color:"var(--text3)",marginBottom:10,padding:"10px 12px",background:"rgba(0,119,182,.06)",borderRadius:8,border:"1px solid rgba(0,119,182,.15)",lineHeight:1.8}}>
                        <b style={{color:"var(--accent)"}}>Format A (Q: prefix):</b><br/>
                        <span style={{fontFamily:"monospace",fontSize:10}}>Q: Question text here{"\n"}A: First option &nbsp;&nbsp; B: Second option &nbsp;&nbsp; C: Third option &nbsp;&nbsp; D: Fourth option{"\n"}ANS: B</span><br/><br/>
                        <b style={{color:"var(--accent)"}}>Format B (numbered):</b><br/>
                        <span style={{fontFamily:"monospace",fontSize:10}}>1. Question text here{"\n"}A) Option A &nbsp;&nbsp; B) Option B &nbsp;&nbsp; C) Option C &nbsp;&nbsp; D) Option D{"\n"}Answer: C</span><br/><br/>
                        <b>Separate each question with a blank line.</b>
                      </div>
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        ...
        <div style={{fontSize:12,color:"var(--text3)",marginBottom:16}}>Each exam: 1 attempt only, tracked per student</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[
            {key:"nursing",icon:"🎓",label:"Nursing Exams",sub:"GNC · Midwifery · Public Health"},
            {key:"mcq",icon:"📝",label:"MCQ Question Banks",sub:"Multiple choice · Admin-uploaded"},
            {key:"essay",icon:"✍️",label:"Essay Exams",sub:"Long answer · AI or manual grading"}
          ].map(t=>(
            <div key={t.key} onClick={()=>setTab(t.key)} style={{
              flex:1,minWidth:150,padding:"12px 16px",borderRadius:11,cursor:"pointer",transition:"all .2s",
              border:`1px solid ${tab===t.key?"var(--accent)":"var(--border)"}`,
              background:tab===t.key?"rgba(0,119,182,.10)":"transparent",textAlign:"center"
            }}>
              <div style={{fontSize:22,marginBottom:4}}>{t.icon}</div>
              <div style={{fontWeight:700,fontSize:14,color:tab===t.key?"var(--accent)":"var(--text2)"}}>{t.label}</div>
              <div style={{fontSize:11,color:"var(--text3)",marginTop:2}}>{t.sub}</div>
            </div>
          ))}
        </div>
      </div>
